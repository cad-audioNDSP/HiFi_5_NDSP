...
void imgfast_gs8_rot0_16(void* pScr, void* outImg, imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz, int fillColor)
{
  ...
  __Pragma("no_reorder");
  // copy
  src = (const ae_int8x16 *)inImg;
  dst0 = (ae_int16x8 *)((uintptr_t)outImg + (PADDING*ostride + (ALIGNMENT / 2))*sizeof(int16_t));
  ...
  for (i = 0; i<h; i++)
  {
    ...
    dst0 = (      ae_int16x8 *)((uintptr_t)dst0 + (ostride - (w&~15))*sizeof(int16_t));
  }
  __Pragma("no_reorder");
  for (i = 0; i<(w & 15); i++)
  {
