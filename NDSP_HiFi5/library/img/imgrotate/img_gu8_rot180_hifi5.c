...
void img_gu8_rot180 (void* pScr, void* outImg, const imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz)
{
  ...
  istride = inSz->stride;
  src = (const ae_int8x16 *)((uintptr_t)inImg + ((h - 1)*istride + w)*sizeof(int8_t));
  aD = AE_ZALIGN128();
  dst = (ae_int8x16 *)((uintptr_t)dst);
  /* flip by 180 degrees */
  if (w >= 16)
  {
    for (i=0; i<h; i++)
    {    
      ...
      dst = (      ae_int8x16*)((int8_t*)dst - 16 + sh);
      for (j = 0; j<((w >> 4) - 1); j++)
      {
        AE_L8X8X2_XP(x0, x1, src, -2 * (int)sizeof(ae_int16x4));
      ...
      {
        ae_int8x8 p;
        AE_L8_IP(p, castxcc(ae_int8, src), -(int)sizeof(int8_t));
        AE_S8_0_IP(p, castxcc(ae_int8, dst), sizeof(int8_t));
      }
      src = (const ae_int8x16 *)((uintptr_t)src - (istride- w - 1)*sizeof(int8_t));
  ...
  for (i = 0; i<h; i++)
  {
    for (j = 0; j<w; j++)
    {
      out[i*ostride + j] = in[(h - 1 - i)*istride + w - 1 - j];
    }
  }
#endif
} /* img_gu8_rot180() */
