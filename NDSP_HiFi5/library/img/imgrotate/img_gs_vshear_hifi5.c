...
{
#if ALG==0
  static const int16_t ALIGN(ALIGNMENT) dsel_tbl[] = { 4, 5, 0, 1};
  ...
  if (alpha > 0)
  {
    ...
    for (j = 0; j<(w >> 1); j++)
    {
      ...
      ae_int16x4 t0, t1, t2, t3, h0, h1, h2;
      int k0, k1;
      xtbool2 bk_1;
      ...
      src1 = (const ae_int16x4 *)((uintptr_t)src + (2 * j + stride)* sizeof(int16_t));

      AE_L32_XP(w0, castxcc(ae_int32, src0), stride * sizeof(int16_t));
      t0 = AE_MOVINT16X4_FROMF32X2(w0);
      AE_L32_XP(w1, castxcc(ae_int32, src0), stride * sizeof(int16_t));
      t1 = AE_MOVINT16X4_FROMF32X2(w1);
      h0 = AE_SEL16X4(t0, t1, dsel0);
      __Pragma("loop_count min=3")
      for (i = 0; i<((h - 1)>>1); i++)//5/2
      {
        ae_int32x2 y0, y1;
        AE_L32_XP(w2, castxcc(ae_int32, src1), stride * sizeof(int16_t));
        t2 = AE_MOVINT16X4_FROMF32X2(w2);
        AE_L32_XP(w3, castxcc(ae_int32, src1), stride * sizeof(int16_t));
        t3 = AE_MOVINT16X4_FROMF32X2(w3);
        h1 = AE_SEL16X4(t1, t2, dsel0);
        h2 = AE_SEL16X4(t2, t3, dsel0);

        AE_MULF2P32X16X4RAS(y0, y1, c0_01, c0_01, h0);
        AE_MULAF2P32X16X4RAS(y0, y1, c1_01, c1_01, h1);
        AE_MULAF2P32X16X4RAS(y0, y1, c2_01, c2_01, h2);
      ...
      for (i = 0; i<((h - 1)& 1); i++) 
      {
        ...
        y0 = AE_MULFP32X16X2RAS_H(c0_01, h0);
        AE_MULAFP32X16X2RAS_H(y0, c1_01, t1);
        AE_MULAFP32X16X2RAS_H(y0, c2_01, t2);
      ...
      for (i = 0; i<(k1-k0); i++)
      {
        ...
        x0 = AE_ROUND16X4F32SASYM(y0, y0);
        AE_S16_0_I(x0, (ae_int16 *)dst0, sizeof(int16_t));
        t3 = AE_SEL16_2301(t1, t1);
        AE_S16_0_IP(t3, castxcc(ae_int16, dst0), 2 * sizeof(int16_t));
      ...
      }
      {
        ...
        AE_S16_0_I(t1, (ae_int16 *) dst0, sizeof(int16_t)); 
        x0 = AE_SEL16_4321(t1, t1);
        AE_S16_0_IP(x0, castxcc(ae_int16, dst0), 2 * sizeof(int16_t));
      }
  ...
  else
  {
    ...
    for (j = 0; j<(w >> 1); j++)
    {
      ...
      for (i = 0; i<((h - 1)>>1); i++) 
      {
        ...
        AE_MULF2P32X4RAS(y0, y1, c0_01, c0_01, w0, w1);
        AE_MULAF2P32X4RAS(y0, y1, c1_01, c1_01, w1, w2);
        AE_MULAF2P32X4RAS(y0, y1, c2_01, c2_01, w2, w3);
  ...
  for (j = 0; j<(w>>2); j++)
  {
    ...
    for (i = 0; i<(h - 1); i++) //5/2
    {
      ...
      AE_MULF2P32X16X4RAS(y0, y1, aQ31_01, aQ31_23, x0);
      AE_MULAF2P32X16X4RAS(y0, y1, bQ31_01, bQ31_23, x1);
      x0 = AE_ROUND16X4F32SASYM(y0, y1);
