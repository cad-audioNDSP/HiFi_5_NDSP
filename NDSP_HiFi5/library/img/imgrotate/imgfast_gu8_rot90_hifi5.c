...
void imgfast_gu8_rot90 (void* pScr, void* outImg, const imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz)
{
  ...
  for (i=0; i<(w>>3); i++)
  {
    ...
    dst0 = (ae_int8x8 *)((uintptr_t)outImg + (((h & 15) + 0)* ostride)*sizeof(int8_t)) + i;
  ...
  for (j = 0; j<(h & 15); j++)
  {
     ...
     dst0 = (ae_int8x8 *)((uintptr_t)outImg + (j * ostride)*sizeof(int8_t));
     for (i = 0; i<(w >> 3); i++) 
     {
  ...
  for (i = 0; i<(w & 7); i++)
  {
    ...
    dst0 = (      ae_int8x8 *)((uintptr_t)outImg + (((h & 7) + 0)* ostride + (w - 1 - i))*sizeof(int8_t));
    ...
    dst0 = (      ae_int8x8 *)((uintptr_t)outImg + (w - 1 - i) *sizeof(int8_t));
    for (j = 0; j<(h & 7); j++)
    {
