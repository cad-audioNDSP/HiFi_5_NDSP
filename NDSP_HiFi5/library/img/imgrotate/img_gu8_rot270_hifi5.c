...
void img_gu8_rot270 (void* pScr, void* outImg, const imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz)
{
  ...
  for (i=0; i<(w>>3); i++)
  {
    ...
    dst0 = (ae_int8 *)((uintptr_t)outImg + ((0)* ostride)*sizeof(int8_t)) + 8 * i;
    dst1 = (ae_int8 *)((uintptr_t)outImg + ((1)* ostride)*sizeof(int8_t)) + 8 * i;
    dst2 = (ae_int8 *)((uintptr_t)outImg + ((2)* ostride)*sizeof(int8_t)) + 8 * i;
    dst3 = (ae_int8 *)((uintptr_t)outImg + ((3)* ostride)*sizeof(int8_t)) + 8 * i;
    dst4 = (ae_int8 *)((uintptr_t)outImg + ((4)* ostride)*sizeof(int8_t)) + 8 * i;
    dst5 = (ae_int8 *)((uintptr_t)outImg + ((5)* ostride)*sizeof(int8_t)) + 8 * i;
    dst6 = (ae_int8 *)((uintptr_t)outImg + ((6)* ostride)*sizeof(int8_t)) + 8 * i;
    dst7 = (ae_int8 *)((uintptr_t)outImg + ((7)* ostride)*sizeof(int8_t)) + 8 * i;

    for (j=0; j<(h>>4); j++) //38
    {

      aX = AE_LA128_PP(src0); AE_LA8X8X2_IP(x0, z0, aX, castxcc(ae_int8x16, src0)); src0 += -16 - 2 * istride;
      aX = AE_LA128_PP(src1); AE_LA8X8X2_IP(x1, z1, aX, castxcc(ae_int8x16, src1)); src1 += -16 - 2 * istride;
      aX = AE_LA128_PP(src0); AE_LA8X8X2_IP(x2, z2, aX, castxcc(ae_int8x16, src0)); src0 += -16 - 2 * istride;
      aX = AE_LA128_PP(src1); AE_LA8X8X2_IP(x3, z3, aX, castxcc(ae_int8x16, src1)); src1 += -16 - 2 * istride;
      aX = AE_LA128_PP(src0); AE_LA8X8X2_IP(x4, z4, aX, castxcc(ae_int8x16, src0)); src0 += -16 - 2 * istride;
      aX = AE_LA128_PP(src1); AE_LA8X8X2_IP(x5, z5, aX, castxcc(ae_int8x16, src1)); src1 += -16 - 2 * istride;
      aX = AE_LA128_PP(src0); AE_LA8X8X2_IP(x6, z6, aX, castxcc(ae_int8x16, src0)); src0 += 3 * 2 * istride;
      aX = AE_LA128_PP(src1); AE_LA8X8X2_IP(x7, z7, aX, castxcc(ae_int8x16, src1)); src1 += 3 * 2 * istride;
      ...
      AE_SA8X8_IP(y1, aY, castxcc(ae_int8x8, dst0));  AE_SA64POS_FP(aY, dst0); dst0 += 8 * ostride - 8;
      AE_SA8X8_IP(y0, aY, castxcc(ae_int8x8, dst1));  AE_SA64POS_FP(aY, dst1); dst1 += 8 * ostride - 8;
      AE_SA8X8_IP(y3, aY, castxcc(ae_int8x8, dst2));  AE_SA64POS_FP(aY, dst2); dst2 += 8 * ostride - 8;
      AE_SA8X8_IP(y2, aY, castxcc(ae_int8x8, dst3));  AE_SA64POS_FP(aY, dst3); dst3 += 8 * ostride - 8;
      AE_SA8X8_IP(y5, aY, castxcc(ae_int8x8, dst4));  AE_SA64POS_FP(aY, dst4); dst4 += 8 * ostride - 8;
      AE_SA8X8_IP(y4, aY, castxcc(ae_int8x8, dst5));  AE_SA64POS_FP(aY, dst5); dst5 += 8 * ostride - 8;
      AE_SA8X8_IP(y7, aY, castxcc(ae_int8x8, dst6));  AE_SA64POS_FP(aY, dst6); dst6 += 8 * ostride - 8;
      AE_SA8X8_IP(y6, aY, castxcc(ae_int8x8, dst7));  AE_SA64POS_FP(aY, dst7); dst7 += 8 * ostride - 8;
  ...
  for (j = 0; j<(h & 15); j++)
  {
     ...
     dst0 = (ae_int8 *)((uintptr_t)outImg + ((h - 1 - j) * ostride)*sizeof(int8_t));
     for (i = 0; i<(w >> 3); i++) 
     {
  ...
  for (i = 0; i<(w & 7); i++)
  {
    ...
    dst0 = (      ae_int8 *)((uintptr_t)outImg + ((7)* ostride + (w - 1 - i))*sizeof(int8_t) );
    for (j = 0; j<(h >> 3); j++)
    {
      aY = AE_LA64_PP(src0);
      AE_LA8X8_IP(x0, aY, castxcc(ae_int8x8, src0));
    ...
    dst0 = (      ae_int8 *)((uintptr_t)outImg +  ((h - 1)* ostride + (w - 1 - i)) *sizeof(int8_t));
    for (j = 0; j<(h & 7); j++)
    {
