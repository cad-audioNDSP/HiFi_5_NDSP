...
void img_gu8_rot270_16(void* pScr,void* outImg, imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz, int fillColor)
{
  ...
  for (i=0; i<(w>>3); i++)
  {
    ...
    src2 = (const ae_int8 *)((uintptr_t)inImg + ((w - 1 - 8 * i - 2) * istride) * sizeof(int8_t));
    src3 = (const ae_int8 *)((uintptr_t)inImg + ((w - 1 - 8 * i - 3) * istride) * sizeof(int8_t));
    src4 = (const ae_int8 *)((uintptr_t)inImg + ((w - 1 - 8 * i - 4) * istride) * sizeof(int8_t));
    src5 = (const ae_int8 *)((uintptr_t)inImg + ((w - 1 - 8 * i - 5) * istride) * sizeof(int8_t));
    src6 = (const ae_int8 *)((uintptr_t)inImg + ((w - 1 - 8 * i - 6) * istride) * sizeof(int8_t));
    src7 = (const ae_int8 *)((uintptr_t)inImg + ((w - 1 - 8 * i - 7) * istride) * sizeof(int8_t));
    for (j=0; j<(h>>4); j++) //48
    {
