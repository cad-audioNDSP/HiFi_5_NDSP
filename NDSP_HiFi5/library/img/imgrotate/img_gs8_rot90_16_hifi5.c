...
void img_gs8_rot90_16(void* pScr,void* outImg, imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz, int fillColor)
{
  ...
  for (i=0; i<(w>>3); i++)
  {
    ...
    src0 = (const ae_int8 *)((uintptr_t)inImg + ((0 + 8 * i) * istride + (h)-(h & 7)) * sizeof(int8_t)) - 8;
    src1 = (const ae_int8 *)((uintptr_t)inImg + ((1 + 8 * i) * istride + (h)-(h & 7)) * sizeof(int8_t)) - 8;
    ...
    for (j=0; j<(h>>3); j++) //28
    {
      ...
      aX = AE_LA64_PP(src0); AE_LA8X8_IP(x0, aX, castxcc(ae_int8x8, src0)); src0 += 2 * istride - 8;
      aX = AE_LA64_PP(src1); AE_LA8X8_IP(x1, aX, castxcc(ae_int8x8, src1)); src1 += 2 * istride - 8;
      aX = AE_LA64_PP(src0); AE_LA8X8_IP(x2, aX, castxcc(ae_int8x8, src0)); src0 += 2 * istride - 8;
      aX = AE_LA64_PP(src1); AE_LA8X8_IP(x3, aX, castxcc(ae_int8x8, src1)); src1 += 2 * istride - 8;
      aX = AE_LA64_PP(src0); AE_LA8X8_IP(x4, aX, castxcc(ae_int8x8, src0)); src0 += 2 * istride - 8;
      aX = AE_LA64_PP(src1); AE_LA8X8_IP(x5, aX, castxcc(ae_int8x8, src1)); src1 += 2 * istride - 8;
      aX = AE_LA64_PP(src0); AE_LA8X8_IP(x6, aX, castxcc(ae_int8x8, src0)); src0 += -3 * 2 * istride - 16;
      aX = AE_LA64_PP(src1); AE_LA8X8_IP(x7, aX, castxcc(ae_int8x8, src1)); src1 += -3 * 2 * istride - 16;
  ...
  for (i = 0; i<(w & 7); i++)
  {
    ...
    for (j = 0; j<(h & 7); j++)
    {
      ...
      AE_CVTI16X4X2F8S(y0, y0, x0, 7);
      /*signed->unsigned*/
      y0 = AE_ADD16(y0, 0x4000);
