...
void imgfasthist_gs16   ( imghist_t * restrict res, const void * restrict inImg, const imgsize_t* sz, int M)
{
    ...
    {
        pIn2 = (const ae_int16x8 *)(in + i*istride);
        pIn3 = pIn2 + 1;
        for (j = 0; j < (w >> 4); j++)
        {
            AE_L16X4X2_IP(x0, x1, pIn2, 16 * sizeof(int16_t));
            AE_MULAAAA2Q16(mean64, mean64_, x0, x1, 1, 1);
            AE_L16X4X2_IP(x2, x3, pIn3, 16 * sizeof(int16_t));                     
            AE_MULAAAA2Q16(mean64_2, mean64_3, x2, x3, 1, 1);
        }
        if (w&8)
        {
            AE_L16X4X2_IP(x0, x1, pIn2, 8 * sizeof(int16_t));
            AE_MULAAAA2Q16(mean64, mean64_, x0, x1, 1, 1);
        }
        if (w & 4)       
        {
            pIn = (const ae_int16x4 *)(pIn2);
    ...
    {
        pIn = (const ae_int16x4 *)(in + 2*i*istride);
        for (j = 0; j < (w>>2); j++)
        {            
            AE_L16X4_IP(x, pIn, 4 * sizeof(int16_t));
        ...
        pIn = (const ae_int16x4 *)(in + (2*i+1)*istride);
        for (j = 0; j < (w >> 2); j++)
        {
            AE_L16X4_IP(x, pIn, 4 * sizeof(int16_t));
            AE_MUL16X4(d0, d1, x, (M - 1));
    ...
    for (i = 0; i < (int)sz->height; i++)
    {
        ...
        if (w & 7)       
        {
            alw2 = AE_LA128_PP(pIn2);
            AE_LAV16X4X2_XP(x0, x1, alw2, pIn2, ((w&7)<<1));
            AE_MULAAAA2Q16(mean64, mean64_, x0, x1, 1, 1);
        }
    }
    mean64 += mean64_;
