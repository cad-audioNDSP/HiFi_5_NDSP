...
void imgnorm_gs16(void * restrict outImg, const void * restrict inImg, const imgsize_t* sz, int minInt, int maxInt)
{
    ...
    // find min/max values
    {
        ...
        for (i = 0; i<h; i++)
        {
            ...
            for (j = 0; j<(K>>1); j++)
            {
                AE_LA16X4X2_IP(p, p0, al_inw, pInw);
    ...
    for (i = 0; i < h; i++)
    {
        ...
        for (j = 0; j < (w >> 3); j++)
        {
            ...
            ae_int32x2 ch, cl;
            AE_LA16X4X2_IP(p0, p1, al_inw, pInw);
            ...
            AE_SA16X4X2_IP(x0, x1, al_outw, pOutw);
        }
    ...
    if (w&4)
    {
        ...
        for (i = 0; i < h; i++)
        {
            pIn = (const ae_int16x4*)(in + i*istride + (w&~7));
            al_in = AE_LA64_PP(pIn);
            pOut = (ae_int16x4*)(out + i*istride + (w&~7));
            ae_int16x4 p;
            ae_int16x4 x, cQ15;
            ae_int32x2 ch, cl;
            AE_LA16X4_IP(p, al_in, pIn);
    ...
    {    
        for (i = 0; i < h; i++)
        {
            pIn = (const ae_int16x4*)(in + i*istride + ((w>>2)<<2));
            al_in = AE_LA64_PP(pIn);
            pOut = (ae_int16x4*)(out + i*istride + ((w >> 2) << 2));
            int off1, off2;
