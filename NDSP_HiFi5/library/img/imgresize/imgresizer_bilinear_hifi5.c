...
#define IMAGE_RESIZER_MAGIC 0x23764323
...
static const imgresizer_api_t* imgresizer_bilinear_getApi(const imgsize_t* in,const imgsize_t* out)
{
    NASSERT(in->stride==out->stride);   // in-place only!!!
    if (in->width==out->width)
    {   // vertical
        if (in->height==2*out->height) return &imgresizer_api_dn2xv;
        if (2*in->height==out->height) return &imgresizer_api_up2xv;
        if (in->height<2*out->height && in->height>out->height) return &imgresizer_api_dnxv;
        if (out->height<2*in->height && out->height>in->height) return &imgresizer_api_upxv;
    }
    else
    {
        NASSERT(in->height==out->height);
        // horizontal
        if (in->width==2*out->width) return &imgresizer_api_dn2xh;
        if (2*in->width==out->width) return &imgresizer_api_up2xh;
        if (in->width<2*out->width && in->width>out->width) return &imgresizer_api_dnxh;
        if (out->width<2*in->width && out->width>in->width) return &imgresizer_api_upxh;
    }
    NASSERT(0); // some error in the upper level logic!
    return NULL;
}

static size_t imgresizer_bilinear_alloc(const imgsize_t* in,const imgsize_t* out)
{
    size_t sz;
    sz=ALIGNMENT;
    const imgresizer_api_t* api=imgresizer_bilinear_getApi(in,out);
    sz+=(sizeof(imgresizer_bilinear_t)+(ALIGNMENT-1))&~(ALIGNMENT-1);
    sz+=(api->getCoefSz(in,out)+(ALIGNMENT-1))&~(ALIGNMENT-1);
    return sz;
}
static size_t imgresizer_bilinear_getScratchSize(const imgsize_t* in,const imgsize_t* out)
{
    return imgresizer_bilinear_getApi(in,out)->getScratchSize(in,out);
}
static struct tag_imgresizer_t* imgresizer_bilinear_init(void* objmem,const imgsize_t* in,const imgsize_t* out)
{
    ...
    pResizer->api=imgresizer_bilinear_getApi(in,out);
    pResizer->in=*in;
    pResizer->out=*out;
...
static void imgresizer_bilinear_process(struct tag_imgresizer_t* pResizer, void *pScr, const void* imgIn, void* imgOut, int fast)
{
    NASSERT(pResizer);
...
static void imgresizer_bilinear_getnext(imgsize_t* currentOutSz,const imgsize_t* inSz,const imgsize_t* outSz)
{
    /*upsampling*/
...
static const imgresize_method_api_t bilinear=
{
    "",
    imgresizer_bilinear_getApi,
    imgresizer_bilinear_alloc,
    imgresizer_bilinear_getScratchSize,
    imgresizer_bilinear_init,
    imgresizer_bilinear_process,
    imgresizer_bilinear_getnext,
    1
};

const imgresize_method_t imgresize_method_bilinear=(const imgresize_method_t)&bilinear;
