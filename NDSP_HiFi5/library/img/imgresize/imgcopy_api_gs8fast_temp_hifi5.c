...
#else
{
    ...
    for (n = 0; n < h; n++)
    {
        ...
        for (m = 0; m < (w >> 4); ++m)
        {
            ...
            AE_L8X8X2_IP(s0, s1, castxcc(ae_int8x16, src), sizeof(ae_int8x16));

            s0 = AE_INT8X8_XOR_INT8X8(s0, _128);
            s1 = AE_INT8X8_XOR_INT8X8(s1, _128);
...
const imgcopy_api imgcopy_api_gs8fast_temp={getScratchSize,copy};
