...
static void process(void *pScr, void* pCoef, const void* img, void*imgOut, const imgsize_t* in, const imgsize_t* out, int fast)
{
    ...
    for (n = 0; n < h; n++)
    {
        ...
        for (m = 0; m < ((win-1)&3); m++)
        {
            ...
            y0 = AE_MULFD16X16X4RAS(x0, x1, c0, c1);
            y1 = AE_MULFD16X16X4RAS(x0, x1, c1, c0);
        ...
        for (m = 0; m < ((win-1)/4); m++)
        {
            ...
            t0 = AE_MULFD16X16X4RAS(x0,x1,c1,c0);
            t1 = AE_MULFD16X16X4RAS(x0,x1,c0,c1);

            AE_DSEL16X4(y0,y1,t0,t1,sel);
