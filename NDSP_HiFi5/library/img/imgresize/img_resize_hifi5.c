...
inline_ void imgresize_getTempImageSz(imgsize_t* sz, const imgresize_params_t * params)
{
    ...
    sz->stride = ((sz->width + (ALIGNMENT - 1))&~(ALIGNMENT - 1))+2*IMGRESIZE_PADDING;  // 16-bit aligned image with PADDING for two 16x4x2 loads
}
                                    ...
                                    const imgcopy_api* fncopy_input,const imgcopy_api* fncopy_output, int fast)
{
    ...
    if (method->inplace == 1)
        pResize->tempImage = (void*)((uintptr_t)pResize->tempImage + sizeof(int16_t)*IMGRESIZE_PADDING);
...
void imgresize_process ( imgresize_handle_t handle, void * restrict pScr, void * restrict outImg, const void * restrict inImg)
{
    ...
    if ((method->inplace == 1) || (sz == 0))
    {
        ...
        {
          int stride = pResize->tempImageSz.stride; 
          int h = pResize->tempImageSz.height;
          int w = pResize->tempImageSz.width;
          pImg = ((int16_t*)tempImg) + w;
          ae_valignx2 aI;
          ...
          for (m = 0; m < h; m++)
          {
             /* for (n = 0; n < (IMGRESIZE_PADDING>>3); n++) */
            AE_SA16X4X2_IP(_0, _0, aI, castxcc(ae_int16x8, pImg));
            AE_SA16X4X2_IP(_0, _0, aI, castxcc(ae_int16x8, pImg));
            AE_SA128POS_FP(aI, pImg);
            pImg += stride - IMGRESIZE_PADDING;
          }          
        }
