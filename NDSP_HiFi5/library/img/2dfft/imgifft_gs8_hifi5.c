               ...
               int fftscale)
{
    ...
    ae_int16x8 * restrict px = (ae_int16x8*)tmpXre;
    ae_int8x8  * restrict py = (ae_int8x8  *)(z);
    ae_valign al_y = AE_ZALIGN64();
...
#else
        ae_int16x4 _0x8080 = AE_MOVDA16(0x8080);
        __Pragma("loop_count min=1"); 
        for (i = 0; i < (N >> 3); i++)
        {
            AE_L16X4X2_IP(tmp0, tmp1, px, 2*sizeof(tmp0));
            tmp0 = AE_SLAA16S( AE_MULFP16X4RAS(tmp0, f), lshift);
            tmp0 = AE_ADD16S(tmp0, dcBias); 
            tmp1 = AE_SLAA16S( AE_MULFP16X4RAS(tmp1, f), lshift);
            tmp1 = AE_ADD16S(tmp1, dcBias); 
            /* Sature negative values to zero */
            tmp0 = AE_MAX16(AE_ZERO16(), tmp0);
            tmp0 = AE_SRAA16RS(tmp0, 7); 
            tmp1 = AE_MAX16(AE_ZERO16(), tmp1);
            tmp1 = AE_SRAA16RS(tmp1, 7);
            /* Pack and store 8 bytes */
            y0 = AE_SEL8X8I(AE_MOVINT8X8_FROMINT16X4(tmp0), AE_MOVINT8X8_FROMINT16X4(tmp1), 25);
            y0 = AE_MOVINT8X8_FROMINT16X4(AE_XOR16(_0x8080, AE_MOVINT16X4_FROMINT8X8(y0)));
            AE_SA8X8_IP(y0, al_y, py);
        }
        AE_SA64POS_FP(al_y, py);
        py = (ae_int8x8  *)( stride - N + (uintptr_t)py );
#endif
    }
#endif
}
