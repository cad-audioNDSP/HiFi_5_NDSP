               ...
               int fftscale)
{
    ...
    */
    ...
    lshift = expCols + expRows + fftscale - log2MN;
    ...
    for ( j = 0; j < M; j++)
    {
        ...
        for (i = 0; i < N; i++)
        {
            z[j*stride + i] = S_add_ss(S_shl_s(S_mpy_ss(tmpXre[k], f), lshift), dcBias);
            k++;
        }
#else
        __Pragma("loop_count min=1");
        for (i = 0; i < (N >> 3); i++)
        {
            ...
            AE_S16X4X2_IP(tmp0, tmp1, pz, 2*sizeof(tmp0)); 
        }
        pz = (ae_int16x8*)((stride - N)*sizeof(int16_t) + (uintptr_t)pz); 
#endif
    }
#endif
}
