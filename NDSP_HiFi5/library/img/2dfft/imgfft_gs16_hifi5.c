               ...
               int32_t mean, const imgsize_t* sz)
{
    ...
    */
    ...
    ae_int16x8 * restrict px = (ae_int16x8 *)img;
    ae_int16x8 * restrict py = (ae_int16x8 *)tmpX2;
    ...
    for (i = 0; i < M; i++)
    {
        ...
        px = (ae_int16x8*)((stride - N)*sizeof(int16_t)+(uintptr_t)px); 
        py = (ae_int16x8*)( N*sizeof(int16_t) + (uintptr_t)py);
    }
