...
int ApplyRFFT_toRows(complex_fract16* scr, complex_fract16 *y, int16_t *x, int M, int N)
{
    ...
    *(int32_t*)&scrY[N / 2 + 1] = 0;
    ...
    for (i = 0; i < M; i++)
    {
        ...
        for (j = 0; j < (N>>3); j++)
        {
            AE_L16X4X2_IP(y0, y1, px, 2*sizeof(y0));
            AE_S16X4X2_IP(y0, y1, py, 2*sizeof(y0));
        }
        
        {
            AE_L16X4_IP(y0, castxcc(ae_int16x4,px), sizeof(y0));
            AE_S16X4_IP(y0, castxcc(ae_int16x4,py), sizeof(y0));
        }
    }
    __Pragma("loop_count min=1"); 
    for (i = 0; i < M; i++)
...
int ApplyFFT_toCols(complex_fract16* scr, complex_fract16 *y, complex_fract16 *x, int M, int N)
{
    ...
    ae_int32x2 x0, x1;
    ae_int16x4 sel0 = AE_MOVINT16X4_FROMINT64(0x0705060403010200);
    ...
    for (j = 0; j < N / 2; j+=2)
    {
        ae_int16x4 x10, x32, x10_, x32_, y10, y32, y10_, y32_;
        ...
        for (i = 0; i < M; i+=4)
        {
            ...
            AE_DSEL16X4(y10, y32, x10, x32, sel0);

            AE_L16X4_XP(x10, px0, N * sizeof(complex_fract16));
            AE_L16X4_XP(x32, px0, N * sizeof(complex_fract16));

            AE_DSEL16X4(y10_, y32_, x10, x32, sel0);

            AE_S16X4X2_IP(y10, y10_, castxcc(ae_int16x8, py0), 4 * sizeof(complex_fract16));
            AE_S16X4X2_IP(y32, y32_, castxcc(ae_int16x8, py1), 4 * sizeof(complex_fract16));
        ...
        for (i = 0; i < M; i+=4)
        {
            ...
            y[j + (i + 1)*N] = scrY0[(i + 1)];
#else
            AE_L16X4X2_IP(x10, x10_, castxcc(ae_int16x8, px0), 4 * sizeof(complex_fract16));
            AE_L16X4X2_IP(x32, x32_, castxcc(ae_int16x8, px1), 4 * sizeof(complex_fract16));
            ...
            AE_DSEL16X4(y10, y32, x10_, x32_, sel0);
