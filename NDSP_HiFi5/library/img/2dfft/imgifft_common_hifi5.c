...
int ApplyIFFT_toCols(complex_fract16* scr, complex_fract16 *y, complex_fract16 *x, int M, int N)
{
    ...
    for (j = 0; j < N / 2; j += 2)
    {
        ...
        ae_int16x4 x10_, x32_, y10_, y32_;
...
int ApplyRIFFT_toRows(complex_fract16* scr, int16_t *y, complex_fract16 *x, int M, int N)
{
    ...
    for (i = 0; i < M; i++)
    {
        ...
        for (j = 0; j < (N >> 3); j++)
        {
            ...
            AE_S16X4X2_IP(tmp0, tmp1, py, 2*sizeof(tmp0));
        }
    }
    ...
    for (i = 0; i < M; i++)
    {
        ...
        {
            ae_int16x4 t0, t1;
            AE_L16X4X2_IP(t0, t1, px, 2*sizeof(t0)); 
            t0 = AE_SRAA16RS(t0, shift); 
            t1 = AE_SRAA16RS(t1, shift); 
            AE_S16X4X2_IP(t0, t1, py, 2*sizeof(t0));
        }
    }
