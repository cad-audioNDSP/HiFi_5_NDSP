                        ...
                        const imgsize_t* sz)
{
    ...
    for (i = 0; i < h; i++)
    {
        ...
        for (j = 0; j < (w>>3); j++)
        {          
            AE_L8X4U_IP(y, pInY, 4 * sizeof(uint8_t));
            AE_L8X4U_IP(u, pInU, 4 * sizeof(uint8_t));
            AE_L8X4U_IP(v, pInV, 4 * sizeof(uint8_t));
