...
void imgnorm_gu8(void * restrict outImg, const void * restrict inImg, const imgsize_t* sz, int minInt, int maxInt)
{
    ...
    for (i = 0; i<h; i++)
    {
        ...
        for (j = 0; j<(w >> 3); j++)
        {
            ...
            //save result 
            t0 = AE_SEL8X8I(AE_MOVINT8X8_FROMINT16X4(y0), AE_MOVINT8X8_FROMINT16X4(y1), 25); //Extract even
            AE_SA8X8_IP(t0, al_out, pOut);
        }
        AE_SA64POS_FP(al_out, pOut);
        for (j = (w&~7); j < w; j++)
        {
            uint8_t tmp = in[i*istride + j];
            p0 = AE_MOVDA16(tmp); 
