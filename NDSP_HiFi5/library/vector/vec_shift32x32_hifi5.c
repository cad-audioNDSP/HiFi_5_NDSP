...
)
{
    ...
    if (t>=0)
    {
        scale=AE_SLAA32S(1,XT_MIN(t,31));
        if (N<=7)
        {
            __Pragma("no_unroll")
            for (n=0; n<N; n++) 
            {
                ...
                AE_MUL2P32X4S(x0,x1,x0,x0,scale,scale);
        ...
        AE_MUL2P32X4S(x0,x1,x0,x1,scale,scale);
        AE_MUL2P32X4S(x2,x3,x2,x3,scale,scale);
    ...
    scale=AE_SLAA32S(1,XT_MAX(t,-31)+32);
    if (N<=7)
    {
        __Pragma("no_unroll")
        for (n=0; n<N; n++) 
        {
            AE_L32_IP(x0,castxcc(ae_int32,x),sizeof(int32_t));
            AE_MUL2P32X4T(x0,x1,x0,x0,scale,scale);
            AE_S32_L_IP(x0,castxcc(ae_int32,y),sizeof(int32_t));
        }
        return ;
    }
    ...
    AE_LA32X2X2_IP(x0,x1,ax,pX);
    AE_LA32X2X2_IP(x2,x3,ax,pX);
    AE_MUL2P32X4T(x0,x1,x0,x1,scale,scale);
    AE_MUL2P32X4T(x2,x3,x2,x3,scale,scale);
    AE_SA32X2X2_IP(x0,x1,ay,pY);
    AE_SA32X2X2_IP(x2,x3,ay,pY);
    AE_SA128POS_FP(ay,pY);
    pX=(const ae_int32x4 *)(x+N0);
