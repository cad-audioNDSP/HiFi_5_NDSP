...
int32_t     vec_min32x32_fast (const int32_t    * restrict x, int N)
{
    ...
    ae_int32x2 x0,x1,x2,x3,min0,min1;
    int n;
    NASSERT_ALIGN16(x);
    NASSERT((N&3)==0);
    if (N <= 0) return 0;
    pX=(const ae_int32x4 *)x;
    min0=MAX_INT32; min1=MAX_INT32;
    for (n=0; n<(N>>3); n++) 
    {
        AE_L32X2X2_IP(x0,x1,pX,sizeof(ae_int32x4));
        AE_L32X2X2_IP(x2,x3,pX,sizeof(ae_int32x4));
        min0=AE_MIN32(min0,AE_MIN32(x0,x1));
        min1=AE_MIN32(min1,AE_MIN32(x2,x3));
    }
    if (N&7)
    {
        AE_L32X2X2_IP(x0,x1,pX,sizeof(ae_int32x4));
    ...
    min0=AE_MIN32(min0,min1);
    return XT_MIN(AE_MOVAD32_H(min0),AE_MOVAD32_L(min0));
}
