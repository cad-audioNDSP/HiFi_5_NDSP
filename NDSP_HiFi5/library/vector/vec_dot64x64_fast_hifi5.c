...
int64_t vec_dot64x64_fast (const int64_t * restrict x,const int64_t * restrict y,int N)
{
    ...
    ae_int32x2 y2, x2, y3, x3;
    ae_int64 xw0, yw0, xw1, yw1;
    ae_int64 W0,  W1, ACC;
    ae_ep e0,e1;
    const ae_int64x2 *  restrict  px = (const ae_int64x2 *) x;
    const ae_int64x2 *  restrict  py = (const ae_int64x2 *) y;
    NASSERT_ALIGN16(x);
    NASSERT_ALIGN16(y);
    NASSERT((N&3)==0);
    if(N<=0) return 0;
    ...
    for (n=0; n<(N>>2); n++)
    {
        AE_L64X2_IP(xw0,xw1,px,sizeof(ae_int64x2)); x0 = AE_MOVINT32X2_FROMINT64(xw0); x1 = AE_MOVINT32X2_FROMINT64(xw1);
        AE_L64X2_IP(yw0,yw1,py,sizeof(ae_int64x2)); y0 = AE_MOVINT32X2_FROMINT64(yw0); y1 = AE_MOVINT32X2_FROMINT64(yw1);
        AE_L64X2_IP(xw0,xw1,px,sizeof(ae_int64x2)); x2 = AE_MOVINT32X2_FROMINT64(xw0); x3 = AE_MOVINT32X2_FROMINT64(xw1);
        AE_L64X2_IP(yw0,yw1,py,sizeof(ae_int64x2)); y2 = AE_MOVINT32X2_FROMINT64(yw0); y3 = AE_MOVINT32X2_FROMINT64(yw1);
        //  Q63xQ63->Q63   
        AE_MULAAD32USEP_HL_LH (e0,W0,x0,y0);
        AE_MULAAD32USEP_HL_LH (e0,W0,x1,y1);
        AE_MULAAD32USEP_HL_LH (e0,W0,x2,y2);
        AE_MULAAD32USEP_HL_LH (e0,W0,x3,y3);
        AE_MULA32EP_HH(e1,W1,x0,y0);
        AE_MULA32EP_HH(e1,W1,x1,y1);
        AE_MULA32EP_HH(e1,W1,x2,y2);
        AE_MULA32EP_HH(e1,W1,x3,y3);
    }
