...
float32_t scl_cosinef(float32_t x)
{
    ...
    xtfloatx2 x0, x1, y0;
    /* Reducted input value and its 2nd power*/
    xtfloatx2 p, p2;
    ...
    ptbl = (xtfloat*)(xtbool2_extract_0(b_cs) ? polysinf_tbl: polycosf_tbl);
    ...
    {
        {
...

            {
                ...
                */
                ...
                y0 = _c;
                XT_MOVT_S(y0, _s, AE_MOVBA(ji));

                /* Determine the input sign. */
                ss = XT_AND(ss, 0x80000000);
                XT_MOVNEZ_S(y0, XT_NEG_S(y0), ss >> 31);
