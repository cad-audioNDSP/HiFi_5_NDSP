...
{
  NASSERT( M > 0 );
  return ( ALIGNED_SIZE( sizeof(bqriir32x32_df1_t), sizeof(void *) )
           + // 4 state elements for each of M DFI sections.
           ALIGNED_SIZE( 4*M*sz_i32, sz_i32*4 )
           + // 6 SOS coefficients for each of M sections
           ALIGNED_SIZE( 6*M*sz_i32, sz_i32*4 ));
...
{  
  ...
  bqriir        = (bqriir32x32_df1_ptr_t)ALIGNED_ADDR( ptr, sizeof(void *) );
  ptr           = bqriir + 1;
  sectState     = (ae_int32x2*)ALIGNED_ADDR( ptr, sz_i32*4 );
  ptr           = sectState + 2*M;
  sectCoef_gsos = (int32_t*)ALIGNED_ADDR( ptr, sz_i32*4 );
  ...
  for ( m=0; m<M; m++ )
  {
    AE_S32X2_IP(AE_ZERO32(),sectState,sz_i32*2);
    AE_S32X2_IP(AE_ZERO32(),sectState,sz_i32*2);
  }
...
{
    int M = ((bqriir32x32_df1_ptr_t)_bqriir)->M;
    return (M/4)*3;
} // bqriir32x32_df1_groupDelay()
