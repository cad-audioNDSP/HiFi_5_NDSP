...
#include "stereo_bqriir32x32_df1_nd_common.h"
...
{
  NASSERT( M > 0 );
  return (ALIGNED_SIZE(sizeof(stereo_bqriir32x32_df1_nd_t), 4)
           ...
           ALIGNED_SIZE( 2*6*M*sizeof(int32_t), 4*sz_i32 ));
} /* stereo_bqriir32x32_df1_nd_alloc() */

/* Initialization routine for iir filters. Returns: handle to the object */
stereo_bqriir32x32_df1_handle_t stereo_bqriir32x32_df1_nd_init(void * objmem, int M,
                                                             ...
                                                             int16_t         gainr )
{
    stereo_bqriir32x32_df1_nd_ptr_t stereo_bqriir;
  ...
  ptr = objmem;
  stereo_bqriir = (stereo_bqriir32x32_df1_nd_ptr_t)ALIGNED_ADDR(ptr, 4);
  ptr           = stereo_bqriir + 1;
  sectState     = (ae_int32x2*)ALIGNED_ADDR( ptr, 4*sz_i32 );
  ptr           = sectState + 2*M*2;
  sectCoef_gsos = (int32_t*)ALIGNED_ADDR( ptr, 4*sz_i32 );
  ptr           = sectCoef_gsos + 6*M*2;

  NASSERT((int8_t*)ptr - (int8_t*)objmem <= (int)stereo_bqriir32x32_df1_nd_alloc(M));

  // Initialize the filter instance.
  stereo_bqriir->magic      = STEREO_BQRIIR32X32_DF1_ND_MAGIC;
  stereo_bqriir->M         = M;
  stereo_bqriir->gainl     = gainl;
  stereo_bqriir->gainr     = gainr;
  stereo_bqriir->coef_gsos = sectCoef_gsos;
  ...
  for ( m=0; m<M; m++ )
  {
    ...
    g = coef_gl[m];
    sectCoef_gsos[12*m+0] = mpy32x16r( coef_sosl[5*m+2], g ); // b2 Q30
    sectCoef_gsos[12*m+1] = mpy32x16r( coef_sosl[5*m+1], g ); // b1 Q30
    sectCoef_gsos[12*m+2] =            coef_sosl[5*m+4];      // a2 Q30
    sectCoef_gsos[12*m+3] =            coef_sosl[5*m+3];      // a1 Q30
    sectCoef_gsos[12*m+4] = mpy32x16r( coef_sosl[5*m+1], g ); // b1 Q30
    sectCoef_gsos[12*m+5] = mpy32x16r( coef_sosl[5*m+0], g ); // b0 Q30

    g = coef_gr[m];
    sectCoef_gsos[12*m+ 6] = mpy32x16r( coef_sosr[5*m+2], g ); // b2 Q30
    sectCoef_gsos[12*m+ 7] = mpy32x16r( coef_sosr[5*m+1], g ); // b1 Q30
    sectCoef_gsos[12*m+ 8] =            coef_sosr[5*m+4];      // a2 Q30
    sectCoef_gsos[12*m+ 9] =            coef_sosr[5*m+3];      // a1 Q30
    sectCoef_gsos[12*m+10] = mpy32x16r( coef_sosr[5*m+1], g ); // b1 Q30
    sectCoef_gsos[12*m+11] = mpy32x16r( coef_sosr[5*m+0], g ); // b0 Q30

    AE_S32X2_IP(AE_ZERO32(),sectState, 2*sz_i32);
    AE_S32X2_IP(AE_ZERO32(),sectState, 2*sz_i32);
    AE_S32X2_IP(AE_ZERO32(),sectState, 2*sz_i32);
    AE_S32X2_IP(AE_ZERO32(),sectState, 2*sz_i32);
  }
