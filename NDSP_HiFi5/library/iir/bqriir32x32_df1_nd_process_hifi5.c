...
#include "bqriir32x32_df1_nd_common.h"
...
-------------------------------------------------------------------------*/
void bqriir32x32_df1_nd( bqriir32x32_df1_nd_handle_t _bqriir,
                      void    * restrict       s,
                      int32_t * restrict       r,
                const int32_t *                x , int N)
{
  bqriir32x32_df1_nd_ptr_t bqriir = (bqriir32x32_df1_nd_ptr_t)_bqriir;
  ...
  for ( m=0; m<M/2; m++ )
  {
    ...
    for ( n=0; n<(N>>1); n++ )
    {
      ...
      AE_PKSRF32( st1, acc00, 1 );

      AE_MULAAF2D32S_HH_LL( acc01, acc10,  cf0_b10, cf1_b10, x01 , st1);
      AE_MULSSF2D32S_HH_LL( acc01, acc10, cf0_a21, cf1_a21, st1 , st2);

      AE_PKSRF32( st2, acc10, 1 );

      acc11 = AE_MULF32S_HH( cf1_b21, st1 );

      AE_PKSRF32( st1, acc01, 1 );

      AE_MULAAFD32S_HH_LL( acc11, cf1_b10, st1 );
      AE_MULSSFD32S_HH_LL( acc11, cf1_a21, st2 );
      AE_PKSRF32( st2, acc11, 1 );

      AE_SA32X2_IP( st2, alr, pr );
    }
