...
#include "bqriir32x16_df2_nd_common.h"
...
bqriir32x16_df2_nd_handle_t bqriir32x16_df2_nd_init( void * objmem,  int M,
                                               ...
                                               int16_t         gain )
{
  bqriir32x16_df2_nd_ptr_t bqriir;
  ...
  bqriir    = (bqriir32x16_df2_nd_ptr_t)ALIGNED_ADDR( ptr, 4 );
  ptr       = bqriir + 1;
  ...
  NASSERT( (int8_t*)ptr - (int8_t*)objmem <= (int)bqriir32x16_df2_nd_alloc(M ) );
  ...
  for ( m=0; m<M; m++ )
  {
    ...
    sectCoef[8*m+0] = AE_NEG16S_scalar( a2 ); 
    sectCoef[8*m+1] = AE_NEG16S_scalar( a1 );
    sectCoef[8*m+2] = 0;
    sectCoef[8*m+3] = g14; // Q14
