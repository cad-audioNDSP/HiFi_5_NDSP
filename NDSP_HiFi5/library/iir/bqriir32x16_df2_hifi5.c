...
size_t bqriir32x16_df2_alloc( int M )
{
    ...
    return bqriir32x16_df2_nd_alloc(M);
#else
  NASSERT( M > 0 );

  return ( ALIGNED_SIZE( sizeof(bqriir32x16_df2_t), sizeof(void *) )
...
{
#if (ALG32X16_DF2_ND == 1)
    return bqriir32x16_df2_nd_init(objmem, M, coef_sos, coef_g, gain);
#else
  ...
  bqriir    = (bqriir32x16_df2_ptr_t)ALIGNED_ADDR( ptr, sizeof(void *) );
  ptr       = bqriir + 1;
...
{
#if (ALG32X16_DF2_ND == 1)
    return bqriir32x16_df2_nd_groupDelay(_bqriir);
#else
    return 0;
#endif
} // bqriir32x16_df2_groupDelay()
