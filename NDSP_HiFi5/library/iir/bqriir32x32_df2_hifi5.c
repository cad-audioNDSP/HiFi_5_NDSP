...
{
  NASSERT( M > 0 );
  return ( ALIGNED_SIZE( sizeof(bqriir32x32_df2_t), sizeof(void *) )
           + // 4 state elements for each of M DFII sections
           ALIGNED_SIZE( 4*M*sz_i32, sizeof(ae_int32x4) )
           + // 1 gain + 5 SOS coefficients for each of M sections
           ALIGNED_SIZE( 6*M*sz_i32, sizeof(ae_int32x4) ));
                                               ...
                                               int16_t         gain )
{
  ...
  ptr = objmem;
  bqriir       = (bqriir32x32_df2_ptr_t)ALIGNED_ADDR( ptr, sizeof(void *) );
  ptr          = bqriir + 1;
  sectState    = (ae_int32x2 *)ALIGNED_ADDR( ptr, sizeof(ae_int32x4) );
  ptr          = sectState + 2*M;
  sectCoef_gsos = (int32_t *)ALIGNED_ADDR( ptr, sizeof(ae_int32x4) );
  ...
  for ( m=0; m<M; m++ )
  {
    sectCoef_gsos[6*m+0] = ((int32_t)coef_g[m])<<15; // gain,Q30
    sectCoef_gsos[6*m+1] = coef_sos[5*m+2]; // b2,Q30
    sectCoef_gsos[6*m+2] = coef_sos[5*m+1]; // b1,Q30
    sectCoef_gsos[6*m+3] = coef_sos[5*m+0]; // b0,Q30
    sectCoef_gsos[6*m+4] = coef_sos[5*m+4]; // a2,Q30
    sectCoef_gsos[6*m+5] = coef_sos[5*m+3]; // a1,Q30

    AE_S32X2_IP(AE_ZERO32(), sectState, sizeof(ae_int32x2));
    AE_S32X2_IP(AE_ZERO32(), sectState, sizeof(ae_int32x2));
  }
...
{
    int M = ((bqriir32x32_df2_ptr_t)_bqriir)->M;
    return (M/4)*4;
} // bqriir32x32_df2_groupDelay()
