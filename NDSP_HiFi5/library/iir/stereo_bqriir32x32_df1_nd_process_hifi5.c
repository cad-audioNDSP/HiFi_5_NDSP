...
void stereo_bqriir32x32_df1_nd(stereo_bqriir32x32_df1_nd_handle_t _bqriir,
                             void    * restrict       s,
                             int32_t * restrict       r,
                       const int32_t *                x, int N )
{
    stereo_bqriir32x32_df1_nd_ptr_t stereo_bqriir = (stereo_bqriir32x32_df1_nd_ptr_t)_bqriir;
  ...
  NASSERT( stereo_bqriir && stereo_bqriir->magic == STEREO_BQRIIR32X32_DF1_ND_MAGIC && r && x );
  if (N<=0) return;
  NASSERT( N%2==0 );
