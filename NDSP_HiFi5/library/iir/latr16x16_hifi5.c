...
static void latr1_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
                        int N, int M )
{
  ...
  *delLine = AE_SEL16_7632(dl,dl);

} /* latr1_proc() */

/* Order 2 lattice filter data processing function. */
static void latr2_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
...
static void latr3_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
...
static void latr4_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
...
static void latr5_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
...
static void latr6_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
                        int N, int M )
{
  ...
  // Q31 <- Q15 + 16
  s_g = AE_MOVDA32X2( ((int32_t)scale)<<16, ((int32_t)scale)<<16 );
...
static void latr7_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
...
static void latr8_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
...
static void latrX_proc( int16_t * restrict r,      // r[N]     [out   ] Q15
                  const int16_t * restrict x,      // x[N]     [in    ] Q15
                        int16_t *          delLine,// dline[M] [in/out] Q14
                  const int16_t *          coef,   // coef[M]  [in    ] Q15
                        int16_t            scale,  // scale    [in    ] Q15
                        int N, int M )
{
  const ae_int16x4 * restrict Pin;
        ae_int16x4 * restrict Pout;
  const ae_int16x4 * restrict Pdl_ld;
        ae_int16x4 * restrict Pdl_st;
  const ae_int16x4 * restrict Pcoef;
  ae_int16x4 X, DL, CF;
  ...
  for ( n=0; n<N; n++ )
  {
    ...
    Pdl_st = (      ae_int16x4 *)(delLine+M-1);
    Pdl_ld = (const ae_int16x4 *)Pdl_st;
    Pcoef  = (const ae_int16x4 *)(coef+M-1);
