...
size_t bqriir32x16_df1_alloc( int M )
{
    ...
    return bqriir32x16_df1_nd_alloc(M);
#else
  NASSERT(  M > 0 );
  return ( ALIGNED_SIZE( sizeof(bqriir32x16_df1_t), sizeof(void *) )
           + // 4 state elements for each of M DFI sections.
           ALIGNED_SIZE( 4*M*sz_i32, 4*sz_i32 )
...
{
#if (ALG32X16_DF1_ND == 1)
    return bqriir32x16_df1_nd_init(objmem, M, coef_sos, coef_g, gain);
#else
  ...
  ptr = objmem;
  bqriir    = (bqriir32x16_df1_ptr_t)ALIGNED_ADDR( ptr, sizeof(void *) );
  ptr       = bqriir + 1;
  ...
  for ( m=0; m<M; m++ )
  {
    ...
    AE_L16_IP(g,castxcc(ae_int16,coef_g),sz_i16);

    AE_L16_IP(b0,cs,sz_i16); 
    AE_L16_IP(b1,cs,sz_i16); 
    AE_L16_IP(b2,cs,sz_i16); 
    AE_L16_IP(a1,cs,sz_i16); 
    AE_L16_IP(a2,cs,sz_i16); 
...
{
#if (ALG32X16_DF1_ND == 1)
    return bqriir32x16_df1_nd_groupDelay(_bqriir);
#else
    return 0;
#endif
} /* bqriir32x16_df1_groupDelay() */
