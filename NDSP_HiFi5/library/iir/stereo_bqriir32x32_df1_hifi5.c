...
size_t stereo_bqriir32x32_df1_alloc( int M )
{
    ...
    return stereo_bqriir32x32_df1_nd_alloc(M);
#else
  NASSERT( M > 0 );
  return ( ALIGNED_SIZE( sizeof(stereo_bqriir32x32_df1_t), sizeof(void *) )
...
{
#if (ALG_STEREO_32X32_DF1_ND == 1)
    return stereo_bqriir32x32_df1_nd_init(objmem, M, coef_sosl, coef_gl, gainl, coef_sosr, coef_gr, gainr);
#else
  ...
  ptr = objmem;
  stereo_bqriir = (stereo_bqriir32x32_df1_ptr_t)ALIGNED_ADDR( ptr, sizeof(void *) );
  ptr           = stereo_bqriir + 1;
...
{
#if (ALG_STEREO_32X32_DF1_ND == 1)
    return stereo_bqriir32x32_df1_nd_groupDelay(_bqriir);
#else
    return 0;
#endif
} /* stereo_bqriir32x32_df1_groupDelay() */
