...
typedef struct tag_bqriir16x16_df2_nd_t
{
...
} bqriir16x16_df2_nd_t, *bqriir16x16_df2_nd_ptr_t;
...
size_t bqriir16x16_df2_nd_alloc( int M )
{
  ...
  return ( ALIGNED_SIZE( sizeof(bqriir16x16_df2_nd_t), 4 )
...
bqriir16x16_df2_nd_handle_t bqriir16x16_df2_nd_init( void * objmem,  int M, 
                                               ...
                                               int16_t         gain )
{
  bqriir16x16_df2_nd_ptr_t bqriir;
  ...
  bqriir    = (bqriir16x16_df2_nd_ptr_t)ALIGNED_ADDR( ptr, 4 );
  ptr       = bqriir + 1;
  ...
  ASSERT( (int8_t*)ptr - (int8_t*)objmem <= (int)bqriir16x16_df2_nd_alloc(M ) );
...
{
    return 0;
} /* bqriir16x16_df2_nd_groupDelay() */

void bqriir16x16_df2_nd(bqriir16x16_df2_nd_handle_t _bqriir,
    void    * restrict       s,
    int16_t * restrict       r,
    const int16_t *                x, int N)
{
    bqriir16x16_df2_nd_ptr_t bqriir = (bqriir16x16_df2_nd_ptr_t)_bqriir;
    ...
    int32_t x0, x1;
    ae_int32x2 X0, X1;
    ae_int32x2 td, Ad2d1, Bd2d1, Cd2d1, Dd2d1, ABd, BCd, CDd, ABd1, BCd1, CDd1;
    ae_int16x4 outy;
    ae_int16x4 d1, d2, A_xd2d1d0, B_xd2d1d0, C_xd2d1d0, D_xd2d1d0;
    ae_int16x4 A_0b2b1b0, A_g0a2a1, A_0ga2a1,
               B_0b2b1b0, B_g0a2a1, B_0ga2a1,
               C_0b2b1b0, C_g0a2a1, C_0ga2a1,
               D_0b2b1b0, D_g0a2a1, D_0ga2a1;
    ae_int64   accA, accB, accC, accD;
    ae_int64   accA1, accB1, accC1, accD1;
    ae_int16x4 Ab2b1b0_0, Bb2b1b0_0, Cb2b1b0_0, Db2b1b0_0;
    ae_int32x2 ty;
    xtbool4    bmask;
    int M, gain;
    int m, n;
    ...
    // Perform data block processing for each section, by 4 sections.
    ...
    for (m = 0; m<((M - 1) >> 2); m++)
    {
        ...
        AE_L16X4_IP(C_0b2b1b0, Cf, 4 * sz_i16);
        AE_L16X4_IP(C_g0a2a1, Cf, 4 * sz_i16);
        AE_L16X4_IP(D_0b2b1b0, Cf, 4 * sz_i16);
        AE_L16X4_IP(D_g0a2a1, Cf, 4 * sz_i16);        
        Ab2b1b0_0 = AE_SEL16_6543(A_0b2b1b0, A_0b2b1b0);        
        Bb2b1b0_0 = AE_SEL16_6543(B_0b2b1b0, B_0b2b1b0);       
        Cb2b1b0_0 = AE_SEL16_6543(C_0b2b1b0, C_0b2b1b0);        
        Db2b1b0_0 = AE_SEL16_6543(D_0b2b1b0, D_0b2b1b0);
        A_0ga2a1 = AE_SEL16_5410(AE_SHORTSWAP(A_g0a2a1), A_g0a2a1);
        B_0ga2a1 = AE_SEL16_5410(AE_SHORTSWAP(B_g0a2a1), B_g0a2a1);
        C_0ga2a1 = AE_SEL16_5410(AE_SHORTSWAP(C_g0a2a1), C_g0a2a1);
        D_0ga2a1 = AE_SEL16_5410(AE_SHORTSWAP(D_g0a2a1), D_g0a2a1);
        ...
        d1 = AE_L16_I(St, 4 * sz_i16);
        d2 = AE_L16_I(St, 5 * sz_i16);
        C_xd2d1d0 = AE_SEL16_5140(d1, d2);
        Cd2d1 = AE_SEXT32X2D16_10(C_xd2d1d0);
        d1 = AE_L16_I(St, 6 * sz_i16);
        d2 = AE_L16_I(St, 7 * sz_i16);
        D_xd2d1d0 = AE_SEL16_5140(d1, d2);
        Dd2d1 = AE_SEXT32X2D16_10(D_xd2d1d0);
        ...
        for (n = 0; n<(N >> 1); n++)
        {
            ...
            X1 = AE_MOVDA32X2(x1, x1);

            //-----------------------------------------------
            // Section 0 sample 0
            td = Ad2d1;
            // Q30 <- Q15*Q14 + 1
            accA = AE_MULZAAAAQ32X16(X0, Ad2d1, A_g0a2a1);                               
            // Q15 <- Q30 - 16 + 1
            AE_PKSR32(Ad2d1, accA, 2);           
            // Q30 <- Q15*Q14 + 1
            accA1 = AE_MULZAAAAQ32X16(X1, Ad2d1, A_0ga2a1);  
            // Q15 <- Q30 - 16 + 1         
            AE_PKSR32(Ad2d1, accA1, 2);
            // Q30 <- Q15*Q14 + 1
            AE_MULZAAAA2Q32X16(accA, accA1, td, Ad2d1, Ab2b1b0_0, A_0b2b1b0);
            // Q15 <- Q30 - 16 + 1
            AE_PKSR32(ABd, accA, 2);            
            AE_PKSR32(ABd1, accA1, 2);

            //-----------------------------------------------
            // Section 1 sample 0, 1
            td = Bd2d1;
            accB = AE_MULZAAAAQ32X16(ABd, Bd2d1, B_0ga2a1);        
            AE_PKSR32(Bd2d1, accB, 2);
            accB1 = AE_MULZAAAAQ32X16(ABd1, Bd2d1, B_0ga2a1);
            AE_PKSR32(Bd2d1, accB1, 2);
            AE_MULZAAAA2Q32X16(accB, accB1, td, Bd2d1, Bb2b1b0_0, B_0b2b1b0);
            AE_PKSR32(BCd, accB, 2);
            AE_PKSR32(BCd1, accB1, 2);

            //-----------------------------------------------
            // Section 2 sample 0, 1
            td = Cd2d1;
            accC = AE_MULZAAAAQ32X16(BCd, Cd2d1, C_0ga2a1);            
            AE_PKSR32(Cd2d1, accC, 2);            
            accC1 = AE_MULZAAAAQ32X16(BCd1, Cd2d1, C_0ga2a1);
            AE_PKSR32(Cd2d1, accC1, 2);
            AE_MULZAAAA2Q32X16(accC, accC1, td, Cd2d1, Cb2b1b0_0, C_0b2b1b0);
            AE_PKSR32(CDd, accC, 2);
            AE_PKSR32(CDd1, accC1, 2);

            //-----------------------------------------------
            // Section 3 sample 0, 1
            td = Dd2d1;
            accD = AE_MULZAAAAQ32X16(CDd, Dd2d1, D_0ga2a1);
            AE_PKSR32(Dd2d1, accD, 2);           
            accD1 = AE_MULZAAAAQ32X16(CDd1, Dd2d1, D_0ga2a1);
            AE_PKSR32(Dd2d1, accD1, 2);
            AE_MULZAAAA2Q32X16(accD, accD1, td, Dd2d1, Db2b1b0_0, D_0b2b1b0);

            // format and store the output sample.
            // Q15 <- Q29 + 2 - 16 w/ asym. rounding and saturation
            ty = AE_MOVINT32X2_FROMINT64(accD);
            AE_PKSR32(ty, accD, 2);
            ...
            // format and store the output sample.
            // Q15 <- Q29 + 2 - 16 w/ asym. rounding and saturation
            ty = AE_MOVINT32X2_FROMINT64(accD1);
            AE_PKSR32(ty, accD1, 2);
            outy = AE_SAT16X4(ty, ty);
            AE_S16_0_IP(outy, R, sz_i16);
        }
        ...
        AE_S16_0_IP(d1, St, sz_i16);
        AE_S16_0_IP(d2, St, sz_i16);
        d2 = AE_SAT16X4(Cd2d1, Cd2d1);
        d1 = AE_SEL16_4321(d2, d2);
        AE_S16_0_IP(d1, St, sz_i16);
        AE_S16_0_IP(d2, St, sz_i16);
        d2 = AE_SAT16X4(Dd2d1, Dd2d1);
        d1 = AE_SEL16_4321(d2, d2);
        AE_S16_0_IP(d1, St, sz_i16);
        AE_S16_0_IP(d2, St, sz_i16);
    ...
    M = M - ((M-1)&(~3));
    //------------------------------------------------------------------------
    // Pass signal through the last biquads (1-4) and apply the total gain.
    //------------------------------------------------------------------------
    if ( M == 4)
    {
        //
        ...
        for (n = 0; n<(N >> 1); n++)
        {
            ...
            // Apply total gain, format and store the output sample.
            // Q(15+gain) <- Q(29 + gain - 14) w/ rounding and saturation
            ty = AE_MOVINT32X2_FROMF64(accD1);
            ty = AE_SRAA32RS(ty, 14 - gain);
            outy = AE_SAT16X4(ty, ty);
            AE_S16_0_IP(outy, R, sz_i16);
        }
    ...
    else if ( M == 3)
    {
        ...
        //
        // Pass the block of input samples through 3 sections. n-th sample at the
        // output of a biquad section r[n] and section state elements (d1,d2) update
        // are given by the following formulae:
        //   d0   = g*x[n] - a1*d1 - a2*d2
        //   r[n] = b0*d0 + b1*d1 + b2*d2;
        ...
        for (n = 0; n<(N >> 1); n++)
        {
            ...
            // Apply total gain, format and store the output sample.
            // Q(15+gain) <- Q(29 + gain - 14) w/ rounding and saturation
            ty = AE_MOVINT32X2_FROMF64(accC1);
            ty = AE_SRAA32RS(ty, 14 - gain);
            outy = AE_SAT16X4(ty, ty);
            AE_S16_0_IP(outy, R, sz_i16);
        }
    ...
    else if ( M == 2)
    {
        ...
        for (n = 0; n<(N >> 1); n++)
        {
            ...
            // Apply total gain, format and store the output sample.
            // Q(15+gain) <- Q(29 + gain - 14) w/ rounding and saturation
            ty = AE_MOVINT32X2_FROMF64(accB1);
            ty = AE_SRAA32RS(ty, 14 - gain);
            outy = AE_SAT16X4(ty, ty);
            AE_S16_0_IP(outy, R, sz_i16);
        }
    ...
    else if ( M == 1)
    {
        ...
        //
        // Pass the block of input samples through 1 section. n-th sample at the
        // output of a biquad section r[n] and section state elements (d1,d2) update
        // are given by the following formulae:
        //   d0   = g*x[n] - a1*d1 - a2*d2
        //   r[n] = b0*d0 + b1*d1 + b2*d2;
        ...
        for (n = 0; n<(N >> 1); n++)
        {
            ...
            // Apply total gain, format and store the output sample.
            // Q(15+gain) <- Q(29 + gain - 14) w/ rounding and saturation
            ty = AE_MOVINT32X2_FROMF64(accA1);
            ty = AE_SRAA32RS(ty, 14 - gain);
            outy = AE_SAT16X4(ty, ty);
            AE_S16_0_IP(outy, R, sz_i16);
        }
