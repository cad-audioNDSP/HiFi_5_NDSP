...
{
  NASSERT(  M > 0 );
  return ( ALIGNED_SIZE( sizeof(stereo_bqriir32x16_df1_nd_t), 4 )
           + // 4 state elements for each of M DFI sections for the two channels
           ALIGNED_SIZE( 2*4*M*sizeof(int32_t), 4*sz_i32 )
           + // 8 coefficients for each of M sections for the two channels
           ALIGNED_SIZE( 2*8*M*sizeof(int16_t), 8*sz_i16 ) );
} /* stereo_bqriir32x16_df1_nd_alloc() */

/* Initialization routine for iir filters. Returns: handle to the object */
stereo_bqriir32x16_df1_nd_handle_t stereo_bqriir32x16_df1_nd_init(void * objmem, int M,
                                                             ...
                                                             int16_t         gainr )
{
    stereo_bqriir32x16_df1_nd_ptr_t stereo_bqriir;
  ae_int16 * restrict sc;
  const ae_int16 * restrict csl;
  const ae_int16 * restrict csr;
  ...
  ptr = objmem;
  stereo_bqriir = (stereo_bqriir32x16_df1_nd_ptr_t)ALIGNED_ADDR(ptr, 4);
  ptr           = stereo_bqriir + 1;
  sectState     = (ae_int32x2 *)ALIGNED_ADDR( ptr, 4*sz_i32 );
  ptr           = sectState + 2*M*2;
  sectCoef      = (int16_t *)ALIGNED_ADDR( ptr, 8*sz_i16 );
  ptr           = sectCoef + 8*M*2;

  NASSERT((int8_t*)ptr - (int8_t*)objmem <= (int)stereo_bqriir32x16_df1_nd_alloc(M));

  // Initialize the filter instance.
  stereo_bqriir->magic  = STEREO_BQRIIR32X16_DF1_ND_MAGIC;
  stereo_bqriir->M      = M;
  stereo_bqriir->gainl  = gainl;
  stereo_bqriir->gainr  = gainr;
  stereo_bqriir->coef  = sectCoef;
  stereo_bqriir->state = (int32_t*)sectState;

  // Copy filter coefficients for M sections, zero the state elements.
  sc =( ae_int16 *)sectCoef;
  csl=(const ae_int16 *)(coef_sosl);
  csr=(const ae_int16 *)(coef_sosr);
  ...
  for ( m=0; m<M; m++ )
  {
    ...
    AE_L16_IP(g,castxcc(ae_int16,coef_gl),sz_i16);

    AE_L16_IP(b0,csl,sz_i16); 
    AE_L16_IP(b1,csl,sz_i16); 
    AE_L16_IP(b2,csl,sz_i16); 
    AE_L16_IP(a1,csl,sz_i16); 
    AE_L16_IP(a2,csl,sz_i16); 

    //
    // pack: g,g,a2,a1; b0,0,b2,b1
    //

    AE_S16_0_IP(  g, sc, sz_i16 );
    AE_S16_0_IP(  g, sc, sz_i16 );
    AE_S16_0_IP( a2, sc, sz_i16 );
    AE_S16_0_IP( a1, sc, sz_i16 );

    AE_S16_0_IP( b0,  sc, sz_i16 );
    AE_S16_0_IP( z16, sc, sz_i16 );
    AE_S16_0_IP( b2,  sc, sz_i16 );
    AE_S16_0_IP( b1,  sc, sz_i16 );

    //
    // Right channel
    //

    AE_L16_IP(g,castxcc(ae_int16,coef_gr),sz_i16);

    AE_L16_IP(b0,csr,sz_i16); 
    AE_L16_IP(b1,csr,sz_i16); 
    AE_L16_IP(b2,csr,sz_i16); 
    AE_L16_IP(a1,csr,sz_i16); 
    AE_L16_IP(a2,csr,sz_i16); 
    ...
    AE_S32X2_IP( z32, sectState, sz_i32*2 );
    AE_S32X2_IP( z32, sectState, sz_i32*2 );
    AE_S32X2_IP( z32, sectState, sz_i32*2 );
    AE_S32X2_IP( z32, sectState, sz_i32*2 );
  }
