...
{
  NASSERT( M > 0 );
  return ( ALIGNED_SIZE( sizeof(bqriir32x32_df2_nd_t), 4 )
...
bqriir32x32_df2_nd_handle_t bqriir32x32_df2_nd_init( void * objmem, int M, 
                                               ...
                                               int16_t         gain )
{
  bqriir32x32_df2_nd_ptr_t bqriir;
  ...
  ptr = objmem;
  bqriir       = (bqriir32x32_df2_nd_ptr_t)ALIGNED_ADDR( ptr, 4 );
  ptr          = bqriir + 1;
  ...
  NASSERT( (int8_t*)ptr - (int8_t*)objmem <= (int)bqriir32x32_df2_nd_alloc(M ) );
