...
#if SMALLER_CODESIZE 
{
  ...
  do
  {
      ...
      {
          M=4;
          AE_S32X2X2_I(MIN_INT32,MIN_INT32,(ae_int32x4*)buff,0);
          __Pragma("no_unroll")
          __Pragma("no_simd")
          for (n=0; n<(N&3); n++) buff[n]=((int32_t*)pX)[n];
          pX=(const ae_int32x4*)buff;
          savedY=(int32_t*)pY;
          pY=(ae_int32x4*)buff;
      }
  }
  ...
  __Pragma("no_unroll")
  __Pragma("no_simd")
  for (n=0; n<(N&3); n++) savedY[n]=buff[n];
  __Pragma("no_reorder")
