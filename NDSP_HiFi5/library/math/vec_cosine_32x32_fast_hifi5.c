...
#if SMALLER_CODESIZE
{
  ...
  ae_int32x2 y2, y3, z2, z3, t2, t3;
  ae_int32x2 p0, p1, p2, p3, p4, p5;
  ae_int32x2 x0, x1, x0_2, x1_2, x0_3, x1_3;
  ae_int32x2 x2, x3, x2_2, x3_2, x2_3, x3_3;
  ...
  for (n=0; n<(N>>3); n++)
  {
    ...
    x0 = AE_ADD32(x0, 0x40000000);
    x1 = AE_ADD32(x1, 0x40000000);
    x2 = AE_ADD32(x2, 0x40000000);
    x3 = AE_ADD32(x3, 0x40000000);
    ...
    AE_MULF2P32X4RAS(x2_2, x3_2, x2,   x3,   x2, x3);//x^2
    AE_MULF2P32X4RAS(x2_3, x3_3, x2_2, x3_2, x2, x3);//x^3

    t2 = t3 = t0 = t1 = p0; y2 = y3 = y0 = y1 = p1;
    AE_MULAF2P32X4RAS(y0, y1, x0_2, x1_2, t0, t1); t0 = y0; t1 = y1;
    AE_MULAF2P32X4RAS(y2, y3, x2_2, x3_2, t2, t3); t2 = y2; t3 = y3;
    y2 = y3 = y0 = y1 = p2;
    AE_MULAF2P32X4RAS(y0, y1, x0_2, x1_2, t0, t1); t0 = y0; t1 = y1;
    AE_MULAF2P32X4RAS(y2, y3, x2_2, x3_2, t2, t3); t2 = y2; t3 = y3;
    y2 = y3 = y0 = y1 = p3;
    ...
    AE_MULF2P32X4RAS(y2, y3, x2, x3, p5, p5);
    AE_MULAF2P32X4RAS(y0, y1, x0_3, x1_3, t0, t1); //Q28
    AE_MULAF2P32X4RAS(y2, y3, x2_3, x3_3, t2, t3); //Q28
    AE_MUL2P32X4S(z0, z1, y0, y1, 0x8, 0x8); //Q31
    AE_MUL2P32X4S(z2, z3, y2, y3, 0x8, 0x8); //Q31
    AE_S32X2X2_IP(z0, z1, pY, 4 * sizeof(ae_int32));
    AE_S32X2X2_IP(z2, z3, pY, 4 * sizeof(ae_int32));
  }
  N&=7;
  if (N) vec_tail32x32((int32_t*)pY,(const int32_t*)pX,vec_cosine32x32_fast,N);
} /* vec_cosine32x32_fast() */
#else
{
  int n;
        ae_int32x4 * restrict pY = (      ae_int32x4 *)y;
