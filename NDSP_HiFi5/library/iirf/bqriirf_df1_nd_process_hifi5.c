...
DISCARD_FUN(void,bqriirf_df1_nd,( bqriirf_df1_nd_handle_t restrict _bqriir,
                      float32_t *       z,
...
void bqriirf_df1_nd(bqriirf_df1_nd_handle_t restrict _bqriir,
    float32_t *                   z,
    const float32_t *                   x, int N)
{
    bqriirf_df1_nd_t    * restrict state;
    ...
    xtfloatx2 dx01, dx11, dy01, dy11, dz1, t1;
    ...
    NASSERT(x);
    NASSERT(z);
    state = (bqriirf_df1_nd_t*)(_bqriir);
    NASSERT(state);
    NASSERT(state->st);
    NASSERT(state->cf);
    M = state->M;
    ...
    pb = (const xtfloatx2*)(state->cf + 0 * M);
    ...
    for (m = 0; m < (M >> 2); m++)
    {
        ...
        {
            /* load coefficients */
            AE_LSX2X2_X(b00, b01, (const xtfloatx4*)pb, 0 * sizeof(float32_t));
            AE_LSX2X2_X(b10, b11, (const xtfloatx4*)pb, 4 * sizeof(float32_t));
            AE_LSX2X2_X(b20, b21, (const xtfloatx4*)pb, 8 * sizeof(float32_t));
            AE_LSX2X2_X(a10, a11, (const xtfloatx4*)pb, 12 * sizeof(float32_t));
            AE_LSX2X2_X(a20, a21, (const xtfloatx4*)pb, 16 * sizeof(float32_t));
            ...
            t = XT_SEL32_HH_SX2(t, tz);
            /* Pass sample through first 2 biquads */
            dz0 = XT_MUL_SX2(dx10, b20);
            XT_MSUBN_SX2(dz0, a20, dy10);
            XT_MADDN_SX2(dz0, b10, dx00);
            XT_MSUBN_SX2(dz0, a10, dy00);
            XT_MADDN_SX2(dz0, b00, t);
            z0 = XT_SEL32_HH_SX2(tz, dz0);
            XT_MADDN_SX2(dz0, b00, z0);
            dx10 = dx00;
            dy10 = dy00;
            dx00 = XT_SEL32_HL_SX2(t, z0);
            dy00 = dz0;

            /* Pass sample through last 2 biquads */
            t1 = XT_SEL32_LH_SX2(dz0, tz);

            dz1 = XT_MUL_SX2(dx11, b21);
            XT_MSUBN_SX2(dz1, a21, dy11);
            XT_MADDN_SX2(dz1, b11, dx01);
            XT_MSUBN_SX2(dz1, a11, dy01);
            XT_MADDN_SX2(dz1, b01, t1);
            z1 = XT_SEL32_HH_SX2(tz, dz1);
            XT_MADDN_SX2(dz1, b01, z1);

            dx11 = dx01;
            dy11 = dy01;
            dx01 = XT_SEL32_HL_SX2(t1, z1);
            dy01 = dz1;
    ...
    if (M & 3)
    {
        ...
        for (m = 0; m < ((M&3)-1); m++)
        {
            ...
            XT_LSIP(dx1, castxcc(xtfloat, pDrd), sizeof(float32_t));
            ...
            XT_SSIP(dx1, castxcc(xtfloat, pDwr), sizeof(float32_t));
        ...
        //apply final scaling with the last biquad
        {
            xtfloat dx0, dx1, dy0, dy1, dz, t;
            xtfloat a1, a2, b0, b1, b2, sc0;
            sc0 = scale;
            pZ = (ae_int32x2 *)z;
            /* load delay lines */
            XT_LSIP(dx0, castxcc(xtfloat, pDrd), sizeof(float32_t));
...
void bqriirf_df1_nd( bqriirf_df1_nd_handle_t restrict _bqriir,
            ...
            const float32_t *                   x, int N)
{
#define VLEN 1  /* vector length */
    bqriirf_df1_nd_t *state;
