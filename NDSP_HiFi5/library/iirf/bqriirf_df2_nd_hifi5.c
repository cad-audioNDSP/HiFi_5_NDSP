...
DISCARD_FUN(size_t,bqriirf_df2_nd_alloc,( int M ))
DISCARD_FUN(size_t, bqriirf_df2_nd_groupDelay, (bqriirf_df2_nd_handle_t  _bqriir))
DISCARD_FUN(bqriirf_df2_nd_handle_t,bqriirf_df2_nd_init,( void * objmem, int M,
...
{
  NASSERT( M >= 0 );
  return sizeof(bqriirf_df2_nd_t)+7*M*sizeof(float32_t)+14;
} // bqriirf_df2_nd_alloc()
...
#define VLEN 1  /* vector length */
    ...
    for (m = 0; m<M; m += VLEN)
    {
        ...
        else
        {
            for (p = 0; p < P; p++)
            {
                cf[0 + (p + m) * 5] = coef_sos[0 + (p + m) * 5]; //b0
                cf[1 + (p + m) * 5] = coef_sos[1 + (p + m) * 5]; //b1
                cf[2 + (p + m) * 5] = coef_sos[2 + (p + m) * 5]; //b2
                cf[3 + (p + m) * 5] = coef_sos[3 + (p + m) * 5]; //a1
                cf[4 + (p + m) * 5] = coef_sos[4 + (p + m) * 5]; //a2
            }
        }
    }
    return iir;
    ...
    int16_t         gain)
{
    ...
    for (m = 0; m<M; m++)
    {
        cf[0 + m * 5] = coef_sos[0 + (m)* 5]; //b0
        cf[1 + m * 5] = coef_sos[1 + (m)* 5]; //b1
        cf[2 + m * 5] = coef_sos[2 + (m)* 5]; //b2
        cf[3 + m * 5] = coef_sos[3 + (m)* 5]; //a1
        cf[4 + m * 5] = coef_sos[4 + (m)* 5]; //a2
    }
    return iir;
} // bqriirf_df2_nd_init()
