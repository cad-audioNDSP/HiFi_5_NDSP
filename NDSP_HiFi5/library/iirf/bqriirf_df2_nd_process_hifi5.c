...
#include "common.h"
#include "bqriirf_df2_nd.h"
#include "common_fpu.h"

#if (HAVE_VFPU==0 && HAVE_FPU==0)
DISCARD_FUN(void,bqriirf_df2_nd,( bqriirf_df2_nd_handle_t _bqriir,
...
void bqriirf_df2_nd( bqriirf_df2_nd_handle_t _bqriir,
                  float32_t * restrict       z,
            const float32_t *                x, int N)
{
    bqriirf_df2_nd_t *state;
    ...
    NASSERT(x);
    NASSERT(z);
    state=(bqriirf_df2_nd_t*)(_bqriir);
    NASSERT(state);
    NASSERT(state->st);
    NASSERT(state->cf);
    M=state->M;
    ...
    /* Process one biquad section at a time */
    pb1 =( const xtfloatx2*)((float32_t *)pb0+1);
    pb2 =( const xtfloatx2*)((float32_t *)pb1+1);
    pa1 =( const xtfloatx2*)((float32_t *)pb2+1);
    pa2 =( const xtfloatx2*)((float32_t *)pa1+1);
    ...
    for(m = 0; m < (M-1); m++)
    {
        ...
        XT_LSIP(b0,castxcc(xtfloat,pb0),5*sizeof(float32_t));
        XT_LSIP(b1,castxcc(xtfloat,pb1),5*sizeof(float32_t));
        XT_LSIP(b2,castxcc(xtfloat,pb2),5*sizeof(float32_t));
        XT_LSIP(a1,castxcc(xtfloat,pa1),5*sizeof(float32_t)); a1=XT_NEG_S(a1);
        XT_LSIP(a2,castxcc(xtfloat,pa2),5*sizeof(float32_t));
        /* processing loop  */
        for (n = 0; n < N; n++)
        {
    ...
    //perform final scaling while processing the last biquad 
    if (M > 0)
    {
        xtfloatx2 ft;
        xtfloat dx0,dy0,dy1,dy2;
