...
DISCARD_FUN(size_t, bqriirf_df2_groupDelay, (bqriirf_df2_handle_t  _bqriir))
...
{
  NASSERT( M >= 0 );
  return sizeof(bqriirf_df2_t)+8*M*sizeof(float32_t)+2*(sizeof(xtfloat)*4-1);
} // bqriirf_df2_alloc()
...
#define VLEN 4  /* vector length */
    ...
    int m;
    st = (float32_t*)((((uintptr_t)objmem)+(sizeof(xtfloat)*4-1))&~(sizeof(xtfloat)*4-1));
    cf = (float32_t*)(((uintptr_t)(st+3*M)+(sizeof(xtfloat)*4-1))&~(sizeof(xtfloat)*4-1));
...
{
    int M = ((bqriirf_df2_ptr_t)_bqriir)->M;
