...
#include "stereo_bqriirf_df1_nd_common.h"

#if (HAVE_VFPU==0 && HAVE_FPU==0)
DISCARD_FUN(void,stereo_bqriirf_df1_nd,( stereo_bqriirf_df1_nd_handle_t _bqriir,
                                      float32_t * restrict z,
                                      const float32_t * x, int N))
#elif (HAVE_VFPU)

void stereo_bqriirf_df1_nd(stereo_bqriirf_df1_handle_t _bqriir,
    float32_t *       restrict  z,
    const float32_t *                 x, int N)
{
    stereo_bqriirf_df1_nd_ptr_t restrict state;
    ...
    NASSERT(x);
    NASSERT(z);
    state=(stereo_bqriirf_df1_nd_ptr_t)(_bqriir);
    NASSERT(state);
    NASSERT(state->st);
    NASSERT(state->cf);
    M=state->M;
    ...
    for (m = 0; m < (M>>1); m++)
    {
        ...
        dx01 = XT_LSX2I(pD,  8*sizeof(float32_t));
        ...
        for (n = 0; n < N; n++)
        {
            ...
            XT_MADDN_SX2(dz1, b11, dx01);
            XT_MSUBN_SX2(dz1, a21, dy11);
            XT_MADDN_SX2(dz1, b01, dz0);
        ...
        XT_SSX2IP(dx01, pD, 2*sizeof(float32_t));
...
} /* stereo_bqriirf_df1_nd() */

#else
void stereo_bqriirf_df1_nd( stereo_bqriirf_df1_nd_handle_t          _bqriir,
                         float32_t *                 restrict z,
                   const float32_t *                          x, int N)
{
#define VLEN 2  /* vector length */
    stereo_bqriirf_df1_nd_t *state;
    const xtfloat *cf;
    ...
    int32_t sl,sr;
    NASSERT(_bqriir);
    state=(stereo_bqriirf_df1_nd_t*)(_bqriir);
    if(N<=0) return;
    NASSERT(N%2==0);
    NASSERT(x);
    NASSERT(z);
