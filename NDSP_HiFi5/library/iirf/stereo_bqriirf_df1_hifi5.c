...
DISCARD_FUN(size_t,stereo_bqriirf_df1_alloc,( int M ))
DISCARD_FUN(size_t, stereo_bqriirf_df1_groupDelay, (stereo_bqriirf_df1_handle_t  _bqriir))
DISCARD_FUN(stereo_bqriirf_df1_handle_t,stereo_bqriirf_df1_init,( void * objmem, int M,
                                                            const float32_t * coef_sosl,
                                                            int16_t           gainl,
                                                            const float32_t * coef_sosr,
                                                            int16_t           gainr ))
#elif (HAVE_VFPU)
...
{
	NASSERT( M >= 0 );
	return sizeof(stereo_bqriirf_df1_t)+18*M*sizeof(float32_t)+(sizeof(xtfloatx4)-1);
} /* stereo_bqriirf_df1_alloc() */

// Given a memory area of sufficient size, initialize a filter instance and 
// return a handle to it.
stereo_bqriirf_df1_handle_t stereo_bqriirf_df1_init( void * objmem, int M,
                                                     const float32_t * coef_sosl,
                                                     int16_t           gainl,
                                                     const float32_t * coef_sosr,
                                                     int16_t           gainr )
{
	stereo_bqriirf_df1_ptr_t iir;
	float32_t * cf; /* 10*M filter coefficients  */
	float32_t * st; /* 4*M stereo delay elements */
	int m;
    NASSERT( objmem && coef_sosl && coef_sosr );
    NASSERT( -48 <= gainl && gainl <= 15 && -48 <= gainr && gainr <= 15 );
	NASSERT( M >= 0 );

	st = (float32_t*)((((uintptr_t)objmem)+(sizeof(xtfloatx4)-1))&~(sizeof(xtfloatx4)-1));
	cf = st+8*M;
	iir = (stereo_bqriirf_df1_ptr_t)(cf+10*M);
	iir->M=M;
	...
	for(m=0; m<M; m++) 
	{
		cf[10*m+2*0+0]=coef_sosl[0+m*5]; //b0
		cf[10*m+2*0+1]=coef_sosr[0+m*5]; //b0
		cf[10*m+2*1+0]=coef_sosl[1+m*5]; //b1
		cf[10*m+2*1+1]=coef_sosr[1+m*5]; //b1
		cf[10*m+2*2+0]=coef_sosl[2+m*5]; //b2
		cf[10*m+2*2+1]=coef_sosr[2+m*5]; //b2
		cf[10*m+2*3+0]=coef_sosl[3+m*5]; //a1
		cf[10*m+2*3+1]=coef_sosr[3+m*5]; //a1
		cf[10*m+2*4+0]=coef_sosl[4+m*5]; //a2
		cf[10*m+2*4+1]=coef_sosr[4+m*5]; //a2
	}
	return iir;
} /* stereo_bqriirf_df1_init() */

//Returns algorithmic delay
size_t stereo_bqriirf_df1_groupDelay(stereo_bqriirf_df1_handle_t _bqriir)
{
    int M = ((stereo_bqriirf_df1_ptr_t)_bqriir)->M;
    return 3*(M/4) + XT_MAX(0, (M&3)-1);
}/* stereo_bqriirf_df1_groupDelay() */

#else
size_t stereo_bqriirf_df1_alloc( int M )
{
	NASSERT( M >= 0 );
	return sizeof(stereo_bqriirf_df1_t)+18*M*sizeof(float32_t)+7;
} /* stereo_bqriirf_df1_alloc() */
                                                     ...
                                                     int16_t gainr )
{
    ...
    cf = st+8*M;
    iir = (stereo_bqriirf_df1_t*)(cf+10*M);
    iir->M=M;
    iir->st=(complex_float*)st;
    iir->cf=cf;
    iir->gainl=gainl;
    ...
    /* clean up and copy coefficients */
    for(m=0; m<8*M; m++) st[m]=0.f;
	for(m=0; m<5*M; m++) { cf[2*m+0]=coef_sosl[m]; cf[2*m+1]=coef_sosr[m]; }
    return iir;
} /* stereo_bqriirf_df1_init() */
