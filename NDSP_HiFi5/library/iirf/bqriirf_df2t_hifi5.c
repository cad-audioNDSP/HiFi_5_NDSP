...
DISCARD_FUN(size_t, bqriirf_df2t_groupDelay, (bqriirf_df2t_handle_t  _bqriir))
...
{
  NASSERT( M >= 0 );
  return sizeof(bqriirf_df2t_t)+8*M*sizeof(float32_t)+2*(sizeof(xtfloatx4)-1);
} // bqriirf_df2t_alloc()
...
#define VLEN 4  /* vector length */
    ...
    cf = (float32_t*)(((uintptr_t)(st+3*M)+(sizeof(xtfloatx4)-1))&~(sizeof(xtfloatx4)-1));
...
{
    int M = ((bqriirf_df2t_ptr_t)_bqriir)->M;
    return M;
} // bqriirf_df2t_groupDelay()
...
{
  NASSERT( M >= 0 );
  return sizeof(bqriirf_df2t_t)+8*M*sizeof(float32_t)+14;
} // bqriirf_df2t_alloc()
                                               ...
                                               int16_t         gain )
{
    ...
    cf = (float32_t*)(((uintptr_t)(st+3*M)+7)&~7);
    iir = (bqriirf_df2t_t*)(cf+5*M);
    iir->M=M;
    iir->st=st;
    iir->cf=cf;
