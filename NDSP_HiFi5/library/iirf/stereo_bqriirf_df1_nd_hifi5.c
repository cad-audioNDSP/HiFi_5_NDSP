...
DISCARD_FUN(size_t,stereo_bqriirf_df1_nd_alloc,( int M ))
DISCARD_FUN(size_t, stereo_bqriirf_df1_nd_groupDelay, (stereo_bqriirf_df1_nd_handle_t  _bqriir))
DISCARD_FUN(stereo_bqriirf_df1_nd_handle_t,stereo_bqriirf_df1_nd_init,( void * objmem, int M,
...
{
	NASSERT( M >= 0 );
    return sizeof(stereo_bqriirf_df1_nd_t)+18 * M*sizeof(float32_t)+7;
} /* stereo_bqriirf_df1_nd_alloc() */

// Given a memory area of sufficient size, initialize a filter instance and 
// return a handle to it.
stereo_bqriirf_df1_nd_handle_t stereo_bqriirf_df1_nd_init(void * objmem, int M,
                                                     const float32_t * coef_sosl,
                                                     int16_t           gainl,
                                                     const float32_t * coef_sosr,
                                                     int16_t           gainr )
{
    stereo_bqriirf_df1_nd_ptr_t iir;
	...
	cf = st+8*M;
    iir = (stereo_bqriirf_df1_nd_ptr_t)(cf + 10 * M);
	iir->M=M;
	...
	for(m=0; m<M; m++) 
	{
		cf[2*(m+0*M)+0]=coef_sosl[1+m*5]; //b1
		cf[2*(m+0*M)+1]=coef_sosr[1+m*5]; //b1
		cf[2*(m+1*M)+0]=coef_sosl[2+m*5]; //b2
		cf[2*(m+1*M)+1]=coef_sosr[2+m*5]; //b2
		cf[2*(m+2*M)+0]=coef_sosl[4+m*5]; //a2
		cf[2*(m+2*M)+1]=coef_sosr[4+m*5]; //a2
		cf[2*(m+3*M)+0]=coef_sosl[0+m*5]; //b0
		cf[2*(m+3*M)+1]=coef_sosr[0+m*5]; //b0
		cf[2*(m+4*M)+0]=coef_sosl[3+m*5]; //a1
		cf[2*(m+4*M)+1]=coef_sosr[3+m*5]; //a1
	}
	return iir;
} /* stereo_bqriirf_df1_nd_init() */

//Returns algorithmic delay
size_t stereo_bqriirf_df1_nd_groupDelay(stereo_bqriirf_df1_nd_handle_t _bqriir)
{
    //int M = ((stereo_bqriirf_df1_nd_ptr_t)_bqriir)->M;
    return 0;
}/* stereo_bqriirf_df1_nd_groupDelay() */
                                                     ...
                                                     int16_t gainr )
{
    ...
    st = (float32_t*)((((uintptr_t)objmem)+7)&~7);
    cf = st+8*M;
    iir = (stereo_bqriirf_df1_nd_t*)(cf+10*M);
    iir->M=M;
    iir->st=(complex_float*)st;
    iir->cf=cf;
    iir->gainl=gainl;
