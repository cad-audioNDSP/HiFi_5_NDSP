...
#include "common.h"
#include "bqciirf_df1_nd.h"
#include "common_fpu.h"

#if (HAVE_VFPU==0 && HAVE_FPU==0)
DISCARD_FUN(void,bqciirf_df1_nd,( bqciirf_df1_nd_handle_t _bqciir,
...
void bqciirf_df1_nd(bqciirf_df1_nd_handle_t _bqciir,
    complex_float * restrict   z,
    const complex_float *            x, int N)
{
    bqciirf_df1_nd_t   *restrict state;
    ...
    NASSERT(x);
    NASSERT(z);
    state=(bqciirf_df1_nd_t*)(_bqciir);
    NASSERT(state);
    NASSERT(state->st);
    NASSERT(state->cf);
    M=state->M;
    ...
    for (m = 0; m < (M>>1); m++)
    {
        ...
        dx01 = XT_LSX2I(pD, 4*sizeof(complex_float));
        ...
        XT_SSX2IP(dx01, pD, sizeof(complex_float));
