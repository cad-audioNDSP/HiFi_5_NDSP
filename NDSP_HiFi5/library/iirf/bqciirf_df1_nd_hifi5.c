...
DISCARD_FUN(size_t,bqciirf_df1_nd_alloc,( int M ))
DISCARD_FUN(size_t, bqciirf_df1_nd_groupDelay, (bqciirf_df1_nd_handle_t  _bqriir))
DISCARD_FUN(bqciirf_df1_nd_handle_t,bqciirf_df1_nd_init,( void * objmem, int M,
...
{
  NASSERT( M >= 0 );
  return sizeof(bqciirf_df1_nd_t)+13*M*sizeof(float32_t)+7;
} // bqciirf_df1_nd_alloc()
                                               ...
                                               int16_t         gain )
{
    ...
    for(m=0; m<M; m++) 
    {
        cf[m+0*M]=coef_sos[1+m*5]; //b1
        cf[m+1*M]=coef_sos[2+m*5]; //b2
        cf[m+2*M]=coef_sos[4+m*5]; //a2
        cf[m+3*M]=coef_sos[0+m*5]; //b0
        cf[m+4*M]=coef_sos[3+m*5]; //a1
    }
    return iir;
} // bqciirf_df1_nd_init()
...
{
    return 0;
} // bqciirf_df1_nd_grpupDelay()

#else
// use another layout for scalar FPU
size_t bqciirf_df1_nd_alloc( int M )
{
  if (M<=0) return 0;
  return sizeof(bqciirf_df1_nd_t)+13*M*sizeof(float32_t)+7;
