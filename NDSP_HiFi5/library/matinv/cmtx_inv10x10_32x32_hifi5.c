...
int  cmtx_inv10x10_32x32(void* pScr, complex_fract32 *x, int qX) 
{
    ...
    for (k=0; k<N; k++)
    {
        ...
        //NASSERT(e>0);
        {
            ...
            {
                ae_valignx2 aC=AE_ZALIGN128();
                ae_valignx2 aB=AE_ZALIGN128();
                ae_valignx2 aBrd,aCrd;
                ae_int32x2 Bk0,Bk1,Bk2,Bk3,Bk4;
                ae_int32x2 Ck0,Ck1,Ck2,Ck3,Ck4;
                pB=(B+k*N+off);
                pC=(ae_int32x2*)(((uintptr_t)pB)+(int)((uintptr_t)C-(uintptr_t)B));
                pT=(T+k);
                AE_ADDCIRC32X2_XC1(pT,sizeof(ae_int32x2));
                aBrd=AE_LA128_PP(pB);
                AE_LA32X2X2_IP(Bk0,Bk1,aBrd,castxcc(ae_int32x4,pB));
                AE_LA32X2X2_IP(Bk2,Bk3,aBrd,castxcc(ae_int32x4,pB));
                AE_L32X2_XC(Bk4,pB,(N-4)*sizeof(ae_int32x2));
                aCrd=AE_LA128_PP(pC);
                AE_LA32X2X2_IP(Ck0,Ck1,aCrd,castxcc(ae_int32x4,pC));
                AE_LA32X2X2_IP(Ck2,Ck3,aCrd,castxcc(ae_int32x4,pC));
                AE_L32X2_IP(Ck4,pC,sizeof(ae_int32x2));
                pBwr=pB;
                for (i=0; i<N-1; i++)
                {
                    ae_int32x2 Bin0,Bin1,Bin2,Bin3,Bin4;
                    ae_int32x2 Cin0,Cin1,Cin2,Cin3,Cin4;
                    AE_L32X2_XC1(Ti,pT,sizeof(ae_int32x2));
                    Ti=AE_NEG32S(Ti);
                    pC=(ae_int32x2*)(((uintptr_t)pB)+(int)((uintptr_t)C-(uintptr_t)B));
                    pCwr=pC;
                    aBrd=AE_LA128_PP(pB);
                    AE_LA32X2X2_IP(Bin0,Bin1,aBrd,castxcc(ae_int32x4,pB));
                    AE_LA32X2X2_IP(Bin2,Bin3,aBrd,castxcc(ae_int32x4,pB));
                    AE_L32X2_XC(Bin4,pB,(N-4)*sizeof(ae_int32x2));
                    aCrd=AE_LA128_PP(pC);
                    AE_LA32X2X2_IP(Cin0,Cin1,aCrd,castxcc(ae_int32x4,pC));
                    AE_LA32X2X2_IP(Cin2,Cin3,aCrd,castxcc(ae_int32x4,pC));
                    AE_L32X2_IP(Cin4,pC,sizeof(ae_int32x2));
                    ...
                    AE_MULAFC32RAS(Cin4,Ck4,Ti);

                    AE_SA32X2X2_IP(Cin0,Cin1,aC,castxcc(ae_int32x4,pCwr));
                    AE_SA32X2X2_IP(Cin2,Cin3,aC,castxcc(ae_int32x4,pCwr));
                    AE_SA128POS_FP(aC,pCwr);
                    AE_S32X2_IP(Cin4,pCwr,1*sizeof(ae_int32x2));

                    AE_SA32X2X2_IP(Bin0,Bin1,aB,castxcc(ae_int32x4,pBwr));
                    AE_SA32X2X2_IP(Bin2,Bin3,aB,castxcc(ae_int32x4,pBwr));
                    AE_SA128POS_FP(aB,pBwr);
                    AE_S32X2_XC (Bin4,pBwr,(N-4)*sizeof(ae_int32x2));
                }
                AE_L32X2_XC1(Ti,pT,sizeof(ae_int32x2));
                ...
                Ck4=AE_MULFC32RAS(Ck4,Ti);
                pCwr=(ae_int32x2*)(((uintptr_t)pBwr)+(int)((uintptr_t)C-(uintptr_t)B));
                AE_SA32X2X2_IP(Ck0,Ck1,aC,castxcc(ae_int32x4,pCwr));
                AE_SA32X2X2_IP(Ck2,Ck3,aC,castxcc(ae_int32x4,pCwr));
                AE_SA128POS_FP(aC,pCwr);
                AE_S32X2_IP(Ck4,pCwr,1*sizeof(ae_int32x2));

                AE_SA32X2X2_IP(Bk0,Bk1,aB,castxcc(ae_int32x4,pBwr));
                AE_SA32X2X2_IP(Bk2,Bk3,aB,castxcc(ae_int32x4,pBwr));
                AE_SA128POS_FP(aB,pBwr);
                AE_S32X2_XC (Bk4,pBwr,(N-4)*sizeof(ae_int32x2));
...
{ 
    return  (2*10*10+10)*sizeof(complex_fract32); 
}
