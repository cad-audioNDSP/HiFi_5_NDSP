...
/* ------------------------------------------------------------------------ */
/*
 * Real Matrix Inversion 6x6, 32-bit fixed point API
...
int  mtx_inv6x6_32x32  (void* pScr, int32_t *x, int qX) 
{
    ...
    // copy input
    {
        ...
        for (k=0; k<N/2; k++) 
        {
            ...
            AE_LA32X2X2_IP(b2,c0,ax,castxcc(ae_int32x4,pX));
            AE_LA32X2X2_IP(c1,c2,ax,castxcc(ae_int32x4,pX));
            AE_S32X2X2_IP(b0,b1,castxcc(ae_int32x4,pB),sizeof(ae_int32x4));
            AE_S32X2X2_IP(b2,AE_ZERO32(),castxcc(ae_int32x4,pB),sizeof(ae_int32x4));
            AE_S32X2X2_IP(c0,c1,castxcc(ae_int32x4,pB),sizeof(ae_int32x4));
            AE_S32X2X2_IP(c2,AE_ZERO32(),castxcc(ae_int32x4,pB),sizeof(ae_int32x4));
...
//            NASSERT(e>0);
            ...
            for (i=0; i<N-1; i++)
            {
                ...
                AE_MULSF2P32X4RAS(Bin2,Cin2,Bk2,Ck2,Ti,Ti);
            ...
            AE_MULF2P32X4RAS(Bk2,Ck2,Bk2,Ck2,Ti,Ti);
    ...
    // copy back to the output
    {
        ...
        for (k=0; k<N/2; k++) 
        {
            ...
            AE_SA32X2X2_IP(b2,c0,ax,castxcc(ae_int32x4,pX));
