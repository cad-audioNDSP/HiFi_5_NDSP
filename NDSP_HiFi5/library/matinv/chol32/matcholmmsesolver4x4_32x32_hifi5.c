    ...
    const int32_t sigma2, int qRA,int qYBRA,int qXYR)
{
    ...
    tmatcholmmsesolver_32x32_getScratch (&scr, pScr, 4);
    pScr=scr.pEnd;
       matcholdecomp4x4_32x32(pScr, scr.R, scr.D, A, sigma2, qRA);
    matcholfwdsubst4x4_32x32(pScr, scr.y, scr.R, scr.D, A, B, qYBRA);
    matcholbkwsubst4x4_32x32(pScr, x, scr.R, scr.D, scr.y, qXYR);
}
...
size_t  matcholmmsesolver4x4_32x32_getScratchSize()
{
    tmatcholmmsesolver_32x32_Scratch scr;
    size_t sz;
    tmatcholmmsesolver_32x32_getScratch (&scr, NULL, 4);
    sz = MAX(MAX(    matcholdecomp4x4_32x32_getScratchSize(),
                  matcholfwdsubst4x4_32x32_getScratchSize()),
                  matcholbkwsubst4x4_32x32_getScratchSize());
    return sz+scr.sz;
}
