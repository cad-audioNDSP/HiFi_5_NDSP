...
#elif 0
{
    ...
    for (m=0; m<N; m+=2)
    {
        ...
        for (n=m; n<N; n++)
        {
            ...
            ep_im0=AE_MOVEA(0);
            ep_im1=AE_MOVEA(0);
            pAn=(const ae_int32x2 *)&A[n];
            pAm=(const ae_int32x2 *)&A[m];
            for (k=0; k<N; k++)
            {
                akmreim1=AE_L32X2_I(pAm,sizeof(ae_int32x2));
                AE_L32X2_XP(akmreim0,pAm,N*sizeof(ae_int32x2));
                AE_L32X2_XP(aknreim ,pAn,N*sizeof(ae_int32x2));
                AE_MULAAD32EP_HH_LL(ep_re0,B_re0,akmreim0,aknreim);
                AE_MULAAD32EP_HH_LL(ep_re1,B_re1,akmreim1,aknreim);
                t=AE_SEL32_LH(aknreim,AE_NEG32S(aknreim));
                AE_MULAAD32EP_HH_LL(ep_im0,B_im0,akmreim0,t);
                AE_MULAAD32EP_HH_LL(ep_im1,B_im1,akmreim1,t);
            }
            ...
            /* -> Q(2*qA)->Q(2*qR-4) */
            AE_S64_I (B_im0,pZ0,      1*sizeof(ae_int64));
            AE_S64_X (B_re1,pZ0,    2*N*sizeof(ae_int64));
            AE_S64_X (B_im1,pZ0,(2*N+1)*sizeof(ae_int64));
            AE_S64_IP(B_re0,pZ0,      2*sizeof(ae_int64));

            AE_S64_I (AE_NEG64S(B_im0),pZ1,1*sizeof(ae_int64));
            AE_S64_I (         (B_re1),pZ1,2*sizeof(ae_int64));
            AE_S64_I (AE_NEG64S(B_im1),pZ1,3*sizeof(ae_int64));
...
#else
{
    ...
    for (m=0; m<N; m+=2)
    {
        ...
        for (n=m; n<N; n++)
        {
            ...
            ep_re0=AE_SEXT72(B_re0);
            ep_re1=AE_SEXT72(B_re1);
			AE_L32X2X2_XP(akmreim0, akmreim1, pAm, N*sizeof(ae_int32x2));
			AE_L32X2_XP(aknreim, pAn, N*sizeof(ae_int32x2));
            AE_MULAAD32EP_HH_LL(ep_re0,B_re0,akmreim0,aknreim);
            AE_MULAAD32EP_HH_LL(ep_re1,B_re1,akmreim1,aknreim);
            t=AE_SEL32_LH(aknreim,AE_NEG32S(aknreim));
            AE_MULZAAD32EP_HH_LL(ep_im0,B_im0,akmreim0,t);
            AE_MULZAAD32EP_HH_LL(ep_im1,B_im1,akmreim1,t);
			...
			AE_L32X2X2_XP(akmreim0, akmreim1, pAm, -(N - 1)*N*(int)sizeof(ae_int32x2));
			AE_L32X2_XP(aknreim, pAn, -(N - 1)*N*(int)sizeof(ae_int32x2));
            AE_MULAAD32EP_HH_LL(ep_re0,B_re0,akmreim0,aknreim);
            AE_MULAAD32EP_HH_LL(ep_re1,B_re1,akmreim1,aknreim);
            t=AE_SEL32_LH(aknreim,AE_NEG32S(aknreim));
            AE_MULAAD32EP_HH_LL(ep_im0,B_im0,akmreim0,t);
            ...
            B_im1=AE_SLAA64S(AE_SRAI72(ep_im1,B_im1,4),2*qRA);
            /* -> Q(2*qA)->Q(2*qR-4) */
			AE_S64X2_X(B_re1, B_im1, pZ0, 2 * N*sizeof(ae_int64));
			AE_S64X2_IP(B_re0, B_im0, pZ0, 2 * sizeof(ae_int64));
			AE_S64X2_I(B_re1, AE_NEG64S(B_im1), pZ1, 2 * sizeof(ae_int64));
			AE_S64X2_XP(B_re0, AE_NEG64S(B_im0), pZ1, 2 * N*sizeof(ae_int64));
		}
    }
}
#endif
...
size_t  cmatcholpreprocess10x10_32x32_getScratchSize() { return 0; }
