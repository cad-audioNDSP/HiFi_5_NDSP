...
/* ------------------------------------------------------------------------ */
/*
 * complex Matrix Gauss-Jordan Elimination for linear equation problem 4x4, 32-bit 
   fixed point API
...
int  cmtx_gjelim4x4_32x32  (void* pScr, complex_fract32 *y, const complex_fract32* A,const complex_fract32 * x, int qA, int qX)
{
    ...
    aX=AE_LA128_PP(pX);
    {
        ...
        AE_LA32X2X2_IP(c2,c3,aX,castxcc(ae_int32x4,pX));
        AE_S32X2X2_IP(c0,c1,castxcc(ae_int32x4,pC),sizeof(ae_int32x4));
        AE_S32X2X2_IP(c2,c3,castxcc(ae_int32x4,pC),sizeof(ae_int32x4));
    }
    ...
    for (k=0; k<N; k++)
    {
        ...
        // find normalization factor
        {
            ...
            pB=(B+k);
            {
                ...
                AE_L32X2_XP(b3,pB,sizeof(ae_int32x2)*N);
                AE_S32X2X2_I(b0,b1,(ae_int32x4*)T,0*sizeof(ae_int32x4));
                AE_S32X2X2_I(b2,b3,(ae_int32x4*)T,1*sizeof(ae_int32x4));
            }
            Tk=AE_L32X2_X((ae_int32x2*)T,k*sizeof(ae_int32x2));
            AE_S32X2_X(AE_SLAA32S(AE_MOVDA32X2(1,0),qB),(ae_int32x2*)T,k*sizeof(ae_int32x2));
            d2=AE_MULZAAD32_HH_LL(Tk,Tk);
            {
                ae_int32x2 Tn0,Tn1,Tn2,Tn3;
                AE_L32X2X2_I(Tn0,Tn1,(const ae_int32x4*)T,0*sizeof(ae_int32x4));
                AE_L32X2X2_I(Tn2,Tn3,(const ae_int32x4*)T,1*sizeof(ae_int32x4));
                n2=            AE_MULZAAD32_HH_LL(Tn0,Tn0);
                n2=AE_MAX64(n2,AE_MULZAAD32_HH_LL(Tn1,Tn1));
                n2=AE_MAX64(n2,AE_MULZAAD32_HH_LL(Tn2,Tn2));
                n2=AE_MAX64(n2,AE_MULZAAD32_HH_LL(Tn3,Tn3));
            }
            ...
            Tk=AE_SLAA32S(Tk,e_den+e_rden-1);
            {
                ...
                Tn2=AE_MULFC32RAS(AE_SLAA32S(Tn2,e_num),Tk);
                Tn3=AE_MULFC32RAS(AE_SLAA32S(Tn3,e_num),Tk);
                AE_S32X2X2_I(Tn0,Tn1,(ae_int32x4*)T,0*sizeof(ae_int32x4));
                AE_S32X2X2_I(Tn2,Tn3,(ae_int32x4*)T,1*sizeof(ae_int32x4));
            }
           e=e_den-e_num+1;
        }
...
{
    return  (4*4+2*4)*sizeof(complex_fract32);  
}
