...
/* ------------------------------------------------------------------------ */
/*
 * complex Matrix Gauss-Jordan Elimination for linear equation problem 6x6, 32-bit 
   fixed point API
...
int  cmtx_gjelim6x6_32x32  (void* pScr, complex_fract32 *y, const complex_fract32* A,const complex_fract32 * x, int qA, int qX)
{
    ...
    for (k=0; k<N; k++)
    {
        ...
        pC=(ae_int32x2 *)C;
        {
            ...
            for(n=0; n<(N); n++) 
            {
                ae_int32x2 b0,b1,b2,b3,b4,b5,c0;
        ...
        else // k=2..5
        {
            ...
            for (n=0; n<N; n++)
            {
                ae_int32x2 b2,b3,b4,b5,c0;
                AE_L32X2X2_IP(b2,b3,castxcc(ae_int32x4,pB),  sizeof(ae_int32x4));
                AE_L32X2X2_IP(b4,b5,castxcc(ae_int32x4,pB),2*sizeof(ae_int32x4));
                AE_L32X2_IP(c0,castxcc(ae_int32x2,pC),sizeof(ae_int32x2));
                AE_MUL2P32X4(b2,b3,b2,b3,expB_coef,expB_coef);
                AE_MUL2P32X4(b4,b5,b4,b5,expB_coef,expB_coef);
                c0=AE_MULP32X2(c0,expC_coef);
                AE_S32X2X2_IP(b2,b3,castxcc(ae_int32x4,pBwr),  sizeof(ae_int32x4));
                AE_S32X2X2_IP(b4,b5,castxcc(ae_int32x4,pBwr),2*sizeof(ae_int32x4));
                AE_S32X2_IP(c0,castxcc(ae_int32x2,pCwr),sizeof(ae_int32x2));
            }
            __Pragma("no_reorder");
            pC=(ae_int32x2 *)C;
            ...
            pBwr=(B+k*N+2);
            pT=(T+k);
            AE_L32X2X2_I (Bk4,Bk5,(const ae_int32x4*)pB,1*sizeof(ae_int32x4));
            AE_L32X2X2_XC(Bk2,Bk3,castxcc(ae_int32x4,pB),N*sizeof(ae_int32x2));
            AE_ADDCIRC32X2_XC1(pT,sizeof(ae_int32x2));
            pBwr=pB;
            esh_coef=AE_SLAA32S(0x40000000,-e+1);
            for (i=0; i<N-1; i++)
            {
                ae_int32x2 Bin2,Bin3,Bin4,Bin5;
                Ti=AE_L32X2_I(pT,0);
                Ti=AE_NEG32S(Ti);
                Cin=AE_L32X2_X (pT,(int)((uintptr_t)C-(uintptr_t)T));
                Cin=AE_SLAA32S(Cin,-e);
                ...
                AE_S32X2_X (Cin,pT,(int)((uintptr_t)C-(uintptr_t)T));
                AE_ADDCIRC32X2_XC1(pT,sizeof(ae_int32x2));
                AE_L32X2X2_I (Bin4,Bin5,(const ae_int32x4*)pB,1*sizeof(ae_int32x4));
                AE_L32X2X2_XC(Bin2,Bin3,castxcc(ae_int32x4,pB),N*sizeof(ae_int32x2));
                AE_MULF2P32X4RAS(Bin2,Bin3,Bin2,Bin3,esh_coef,esh_coef);
                AE_MULF2P32X4RAS(Bin4,Bin5,Bin4,Bin5,esh_coef,esh_coef);
                AE_MULAFC32RAS(Bin2,Bk2,Ti);
                AE_MULAFC32RAS(Bin3,Bk3,Ti);
                AE_MULAFC32RAS(Bin4,Bk4,Ti);
                AE_MULAFC32RAS(Bin5,Bk5,Ti);
                AE_S32X2X2_I (Bin4,Bin5,(ae_int32x4*)pBwr,1*sizeof(ae_int32x4));
                AE_S32X2X2_XC(Bin2,Bin3,castxcc(ae_int32x4,pBwr),N*sizeof(ae_int32x2));
            }
            Ti=AE_L32X2_I(pT,0);
            Ckn=AE_MULFC32RAS(Ckn,Ti);
            AE_S32X2_X (Ckn,pT,(int)((uintptr_t)C-(uintptr_t)T));
            ...
            Bk4=AE_MULFC32RAS(Bk4,Ti);
            Bk5=AE_MULFC32RAS(Bk5,Ti);
            AE_S32X2X2_I (Bk4,Bk5,(ae_int32x4*)pBwr,1*sizeof(ae_int32x4));
            AE_S32X2X2_XC(Bk2,Bk3,castxcc(ae_int32x4,pBwr),N*sizeof(ae_int32x2));
            pBwr=(B+k);
            for (i=0; i<N; i++)  AE_S32X2_XP(AE_ZERO32(),pBwr,N*sizeof(ae_int32x2));
            __Pragma("no_reorder");
        }
    ...
    // copy back to the output
    {
        ...
        AE_L32X2X2_I(c4,c5,(const ae_int32x4*)C,2*sizeof(ae_int32x4));
        AE_SA32X2X2_IP(c0,c1,aX,castxcc(ae_int32x4,pY));
        AE_SA32X2X2_IP(c2,c3,aX,castxcc(ae_int32x4,pY));
        AE_SA32X2X2_IP(c4,c5,aX,castxcc(ae_int32x4,pY));
...
{ 
    return  (6*6+2*6)*sizeof(complex_fract32);  
}
