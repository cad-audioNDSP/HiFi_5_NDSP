...
/* ------------------------------------------------------------------------ */
/*
 * Real Matrix Gauss-Jordan Elimination for linear equation problem 8x8, 32-bit 
   fixed point API
...
int  mtx_gjelim8x8_32x32  (void* pScr, int32_t *y, const int32_t* A,const int32_t * x, int qA, int qX)
{
    ...
    for (k=0; k<N; k++)
    {
        ...
        for(n=0; n<(N/2); n++) 
        {
            ...
            minnsaB0=AE_MIN16(minnsaB0,AE_NSA32X4(b0,b1));
            minnsaB1=AE_MIN16(minnsaB1,AE_NSA32X4(b2,b3));
            AE_L32X2_IP(c0,pC,sizeof(ae_int32x2));
        ...
        // find normalization factor
        {
            ...
            AE_S32_L_X(AE_SLAA32S(1,qB),(ae_int32*)pT,k*sizeof(int32_t));
            {
                ...
                maxnum=AE_MAX32(AE_MAXABS32S(t0,t1),AE_MAXABS32S(t2,t3));
        ...
        // Gauss-Jordan elimination: might be made using curcular addressing on B/C
        {
            ...
            AE_L32X2X2_XC (Bk0,Bk1,castxcc(ae_int32x4,pB),N/2*sizeof(ae_int32x2));
            ...
            for (i=0; i<N-1; i++)
            {
                ...
                AE_L32X2X2_XC (Bin0,Bin1,castxcc(ae_int32x4,pB),N/2*sizeof(ae_int32x2));
                ...
                AE_S32X2X2_XC (Bin0,Bin1,castxcc(ae_int32x4,pBwr),N/2*sizeof(ae_int32x2));
            }
            Ti=AE_L32_I((const ae_int32*)pT,0);
            ...
            AE_S32X2X2_XC (Bk0,Bk1,castxcc(ae_int32x4,pBwr),N/2*sizeof(ae_int32x2));
            pBwr=(ae_int32x2*)(B+k);
    ...
    // copy back to the output
    {
        ...
        AE_L32X2X2_I  (c0,c1,(const ae_int32x4*)pC,0*sizeof(ae_int32x4));
        AE_L32X2X2_I  (c2,c3,(const ae_int32x4*)pC,1*sizeof(ae_int32x4));
        AE_SA32X2X2_IP(c0,c1,aY,castxcc(ae_int32x4,pY));
        AE_SA32X2X2_IP(c2,c3,aY,castxcc(ae_int32x4,pY));
