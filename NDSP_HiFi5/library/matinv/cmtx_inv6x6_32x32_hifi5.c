       ...
       // NASSERT(e>0);
        {
            ...
            for (off=0; off<N; off+=N/2)
            {
                ...
                AE_L32X2_XC(Bk2,pB,(N-2)*sizeof(ae_int32x2));
                aCrd=AE_LA128_PP(pC);
                AE_LA32X2X2_IP(Ck0,Ck1,aCrd,castxcc(ae_int32x4,pC));
                AE_L32X2_IP(Ck2,pC,sizeof(ae_int32x2));
                pBwr=pB;
                for (i=0; i<N-1; i++)
                {
                    ae_int32x2 Bin0,Bin1,Bin2;
                    ae_int32x2 Cin0,Cin1,Cin2;
                    ...
                    AE_L32X2_XC(Bin2,pB,(N-2)*sizeof(ae_int32x2));
                    aCrd=AE_LA128_PP(pC);
                    AE_LA32X2X2_IP(Cin0,Cin1,aCrd,castxcc(ae_int32x4,pC));
                    AE_L32X2_IP(Cin2,pC,sizeof(ae_int32x2));
                    ...
                    AE_S32X2_IP(Cin2,pCwr,1*sizeof(ae_int32x2));

                    AE_SA32X2X2_IP(Bin0,Bin1,aB,castxcc(ae_int32x4,pBwr));
                    AE_SA128POS_FP(aB,pBwr);
                    AE_S32X2_XC (Bin2,pBwr,(N-2)*sizeof(ae_int32x2));
                }
                AE_L32X2_XC1(Ti,pT,sizeof(ae_int32x2));
                ...
                AE_SA128POS_FP(aC,pCwr);
                AE_S32X2_IP(Ck2,pCwr,1*sizeof(ae_int32x2));

                AE_SA32X2X2_IP(Bk0,Bk1,aB,castxcc(ae_int32x4,pBwr));
                AE_SA128POS_FP(aB,pBwr);
                AE_S32X2_XC (Bk2,pBwr,(N-2)*sizeof(ae_int32x2));
           }
           __Pragma("no_reorder")
        }
        pB=(B+k);
...
{ 
    return   (2*6*6+6)*sizeof(complex_fract32); 
}
