...
void  mtx_gjelim10x10f(void* pScr, float32_t *y, const float32_t* A,const float32_t * x) 
{
    ...
    // copy input
    {
        ...
        for (k=0; k<N/2; k++) 
        {
            ...
            xtfloatx2 x6,x7,x8,x9,xa,xb;
            AE_LASX2X2_IP(x0,x1,aA,castxcc(xtfloatx4,A));
            AE_LASX2X2_IP(x2,x3,aA,castxcc(xtfloatx4,A));
            AE_LASX2X2_IP(x4,x6,aA,castxcc(xtfloatx4,A));
            AE_LASX2X2_IP(x7,x8,aA,castxcc(xtfloatx4,A));
            AE_LASX2X2_IP(x9,xa,aA,castxcc(xtfloatx4,A));
            {
                ae_int32x2 tt;
                AE_L32_IP(tt,castxcc(ae_int32,x),sizeof(xtfloat));
                x5=XT_AE_MOVXTFLOATX2_FROMINT32X2(tt);
                AE_L32_IP(tt,castxcc(ae_int32,x),sizeof(xtfloat));
                xb=XT_AE_MOVXTFLOATX2_FROMINT32X2(tt);
            }
            AE_SSX2X2_IP(x0,x1,pAwr,sizeof(xtfloatx4));
            AE_SSX2X2_IP(x2,x3,pAwr,sizeof(xtfloatx4));
            AE_SSX2X2_IP(x4,x5,pAwr,sizeof(xtfloatx4));
            AE_SSX2X2_IP(x6,x7,pAwr,sizeof(xtfloatx4));
            AE_SSX2X2_IP(x8,x9,pAwr,sizeof(xtfloatx4));
            AE_SSX2X2_IP(xa,xb,pAwr,sizeof(xtfloatx4));
        }
    }
    __Pragma("no_reorder")
    /* Gauss elimination */
    for(k=0; k<N; k++)
    {
        ...
        xtfloatx2 Amax0,Amax1,Amax2,Amax3,Amax4,Amax5;
        xtfloatx2 amax;
...
size_t mtx_gjelim10x10f_getScratchSize   () { const int N=10; return(N+2)*N*sizeof(float32_t);  }
#elif (HAVE_FPU)
// code for scalar FPU

void  mtx_gjelim10x10f  (void* pScr, float32_t * restrict y, const float32_t* restrict A,const float32_t * restrict x) 
{
    const int N=10;
    int k,n;
...
size_t mtx_gjelim10x10f_getScratchSize   () { const int N=10; return  N*(N+2)*sizeof(float32_t);  }
#else
DISCARD_FUN(void, mtx_gjelim10x10f, (void* pScr, float32_t *y, const float32_t* A, const float32_t * x))
