...
void cmtx_inv10x10f(void * pScr,complex_float* x) 
{
    ...
    for(k=0; k<N; k++)
    {
        ...
        xtfloatx2 Bn0,Bn1,Bn2,Bn3,Bn4,Bn5,Bn6,Bn7,Bn8,Bn9;
        xtfloatx2 Bk0,Bk1,Bk2,Bk3,Bk4,Bk5,Bk6,Bk7,Bk8,Bk9;
        ...
        AE_LSX2X2_I (Bk2,Bk3,pAwr, 6*sizeof(xtfloatx4));
        AE_LSX2X2_I (Bk4,Bk5,pAwr, 7*sizeof(xtfloatx4));
        AE_LSX2X2_X (Bk6,Bk7,pAwr, 8*sizeof(xtfloatx4));
        AE_LSX2X2_X (Bk8,Bk9,pAwr, 9*sizeof(xtfloatx4));
        ...
        AE_LSX2X2_I (Bn2,Bn3,pA, 6*sizeof(xtfloatx4));
        AE_LSX2X2_I (Bn4,Bn5,pA, 7*sizeof(xtfloatx4));
        AE_LSX2X2_X (Bn6,Bn7,pA, 8*sizeof(xtfloatx4));
        AE_LSX2X2_X (Bn8,Bn9,pA, 9*sizeof(xtfloatx4));
        __Pragma("no_reorder")
        ...
        AE_SSX2X2_I (Bk2,Bk3,pA, 6*sizeof(xtfloatx4));
        AE_SSX2X2_I (Bk4,Bk5,pA, 7*sizeof(xtfloatx4));
        AE_SSX2X2_X (Bk6,Bk7,pA, 8*sizeof(xtfloatx4));
        AE_SSX2X2_X (Bk8,Bk9,pA, 9*sizeof(xtfloatx4));
        __Pragma("no_reorder")
        ...
        MULMUX_SX2X2(Bk8,Bk9,Bn8,Bn9,rden,rden, 0); MADDMUXQ_S (Bk8,Bk9,Bn8,Bn9,rden, 1);
        xtfloatx4* saved_pA=pAwr;
        ...
        AE_SSX2X2_I (Bk2,Bk3,pAwr, 6*sizeof(xtfloatx4));
        AE_SSX2X2_I (Bk4,Bk5,pAwr, 7*sizeof(xtfloatx4));
        AE_SSX2X2_X (Bk6,Bk7,pAwr, 8*sizeof(xtfloatx4));
        AE_SSX2X2_X (Bk8,Bk9,pAwr, 9*sizeof(xtfloatx4));
        AE_SSX2X2_XC(Ak0,Ak1,pAwr, N*sizeof(xtfloatx4));
        pA=pAwr;
        /* elimination: separated on 2 phases for better scheduling
           Note: to avoid extra copying of pivot column we have to
           modify right part first
        */
        __Pragma("no_reorder")

        pT=(const xtfloatx2*)pA;
        pT+=k;
        for (p=1; p>=0; p--)
        {
            pA=saved_pA;
            pA+=p*(N/2);
            AE_LSX2X2_I (Ak2,Ak3,pA, 1*sizeof(xtfloatx4));
            AE_LSX2X2_I (Ak4,Ak5,pA, 2*sizeof(xtfloatx4));
            AE_LSX2X2_I (Ak6,Ak7,pA, 3*sizeof(xtfloatx4));
            AE_LSX2X2_I (Ak8,Ak9,pA, 4*sizeof(xtfloatx4));
            AE_LSX2X2_XC(Ak0,Ak1,pA, N*sizeof(xtfloatx4));
            pAwr=pA;
            for (n=0; n<N-1; n++)
            {
                XT_LSX2XC (Ank,pT,2*N*sizeof(complex_float));
                AE_LSX2X2_I (An2,An3,pA, 1*sizeof(xtfloatx4));
                AE_LSX2X2_I (An4,An5,pA, 2*sizeof(xtfloatx4));
                AE_LSX2X2_I (An6,An7,pA, 3*sizeof(xtfloatx4));
                AE_LSX2X2_I (An8,An9,pA, 4*sizeof(xtfloatx4));
            ...
            AE_ADDCIRC_XC(castxcc(ae_int64,pT),2*N*sizeof(complex_float));
        }
        __Pragma("no_reorder")
    }
    ...
    for(n=0; n<N; n++)
    {
        ...
        AE_LSX2X2_IP(An6,An7,pA, 1*sizeof(xtfloatx4));
        AE_LSX2X2_XP(An8,An9,pA, (1+N/2)*sizeof(xtfloatx4));
        AE_SASX2X2_IP(An0,An1,ax,pAwr);
        AE_SASX2X2_IP(An2,An3,ax,pAwr);
        AE_SASX2X2_IP(An4,An5,ax,pAwr);
        AE_SASX2X2_IP(An6,An7,ax,pAwr);
        AE_SASX2X2_IP(An8,An9,ax,pAwr);
    }
    AE_SA128POS_FP(ax,pAwr);

}
size_t cmtx_inv10x10f_getScratchSize() { return 2*10*10*sizeof(complex_float); }
#elif (HAVE_FPU)
// code for scalar FPU
...
void cmtx_inv10x10f(void * pScr,complex_float* restrict x) 
{
    const int N=10;
    int p,n,m,k;
    complex_float r;
    ...
    for(n=0; n<N; n++)
    {
        ae_int32x2 x0,x1,x2,x3,x4,x5,x6,x7,x8,x9;
        ...
        AE_LA32X2X2_IP(x8,x9,aX,castxcc(ae_int32x4,pArd));
        ...
        AE_S32X2X2_IP(x8,x9,castxcc(ae_int32x4,pAwr),sizeof(ae_int32x4));
    ...
    for(n=0; n<N; n++)
    {
        ...
        AE_L32X2X2_IP(x6,x7,castxcc(ae_int32x4,pArd),sizeof(ae_int32x4));
        AE_L32X2X2_XP(x8,x9,castxcc(ae_int32x4,pArd),(N/2+1)*sizeof(ae_int32x4));
        ...
        AE_SA32X2X2_IP(x8,x9,aX,castxcc(ae_int32x4,pAwr));
    }
    AE_SA128POS_FP(aX,pAwr);
}
size_t cmtx_inv10x10f_getScratchSize() { const int N=10; return 2*N*N*sizeof(complex_float); }
#else
DISCARD_FUN(void, cmtx_inv10x10f, (void* pScr, complex_float* x))
