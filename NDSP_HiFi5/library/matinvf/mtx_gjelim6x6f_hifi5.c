...
void  mtx_gjelim6x6f  (void* pScr, float32_t *y, const float32_t* A,const float32_t * x) 
{
    ...
    WUR_AE_CEND0  ((uintptr_t)(B+N*(N+2)));
    ...
    // copy input
    {
        ...
        for (k=0; k<N; k++) 
        {
            ...
            XT_LASX2IP(x0,aA,castxcc(xtfloatx2,A));
            XT_LASX2IP(x1,aA,castxcc(xtfloatx2,A));
            XT_LASX2IP(x2,aA,castxcc(xtfloatx2,A));
            {
                ae_int32x2 tt;
                AE_L32_IP(tt,castxcc(ae_int32,x),sizeof(xtfloat));
                x3=XT_AE_MOVXTFLOATX2_FROMINT32X2(tt);
            }
    ...
    for(k=0; k<N; k++)
    {
        ...
        for(n=k; n<N; n++)
        {
          ...
          {
              ae_int32x2 tt;
              AE_L32_XP(tt,castxcc(ae_int32,pA),(N+2)*sizeof(xtfloat));
    ...
    // copy back to the output
    {
        ...
        pA=(xtfloatx4*)(B+N);
        XT_LSX2XP(x0,castxcc(xtfloatx2,pA),(N+2)*sizeof(float32_t));
        XT_LSX2XP(x1,castxcc(xtfloatx2,pA),(N+2)*sizeof(float32_t));
        XT_LSX2XP(x2,castxcc(xtfloatx2,pA),(N+2)*sizeof(float32_t));
        XT_LSX2XP(x3,castxcc(xtfloatx2,pA),(N+2)*sizeof(float32_t));
        XT_LSX2XP(x4,castxcc(xtfloatx2,pA),(N+2)*sizeof(float32_t));
        XT_LSX2XP(x5,castxcc(xtfloatx2,pA),(N+2)*sizeof(float32_t));
        XT_SASX2IP(AE_SEL32_HH_SX2(x0,x1),aY,castxcc(xtfloatx2,y));
        XT_SASX2IP(AE_SEL32_HH_SX2(x2,x3),aY,castxcc(xtfloatx2,y));
        XT_SASX2IP(AE_SEL32_HH_SX2(x4,x5),aY,castxcc(xtfloatx2,y));
        AE_SA64POS_FP(aY,y);
    }
}

size_t mtx_gjelim6x6f_getScratchSize   () { const int N=6; return(N+2)*N*sizeof(float32_t);  }
#elif (HAVE_FPU)
// code for scalar FPU

void  mtx_gjelim6x6f  (void* pScr, float32_t * restrict y, const float32_t* restrict A,const float32_t * restrict x) 
{
    const int N=6;
    int k,n;
...
size_t mtx_gjelim6x6f_getScratchSize   () { const int N=6; return  N*(N+2)*sizeof(float32_t);  }
#else
DISCARD_FUN(void, mtx_gjelim6x6f, (void* pScr, float32_t *y, const float32_t* A, const float32_t * x))
