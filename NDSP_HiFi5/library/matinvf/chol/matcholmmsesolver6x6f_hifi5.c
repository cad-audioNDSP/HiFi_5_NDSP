    ...
    const float32_t       sigma2)
{
    ...
    matcholfwdsubst6x6f(pScr, y, R, D, A, B);
    matcholbkwsubst6x6f(pScr, x, R, D, y);
}
...
size_t  matcholmmsesolver6x6f_getScratchSize()
{
    ...
    s_fw = matcholfwdsubst6x6f_getScratchSize();
    s_bk = matcholbkwsubst6x6f_getScratchSize();
...
#else
DISCARD_FUN(void, matcholmmsesolver6x6f, (void * pScr,
    float32_t * x,
    const float32_t * A,
    const float32_t * B,
    const float32_t sigma2))
