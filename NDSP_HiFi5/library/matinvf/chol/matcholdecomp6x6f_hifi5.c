...
{
    static const f_realcholFwdrecf f_Fwdrec[6] = { realcholFwdrec1f, realcholFwdrec2f, realcholFwdrec3f, realcholFwdrec4f, realcholFwdrec5f, realcholFwdrec6f };
    int n;
    ...
    // compute  A'*A 
    matcholpreprocess6x6f(NULL, prep, A, sigma2);
...
#if (HAVE_VFPU)
    {
      ...
      XT_SSX2IP(0, castxcc(xtfloatx2, T1), 2 * (sizeof(float32_t)));
      ...
      XT_SSIP(0, castxcc(xtfloat, T0), (sizeof(float32_t)));
    }
#else
    {
    ...
    for (n = 0; n<6; n++)
    {
        ...
        f_Fwdrec[n](y, R, D, prep + n, 6);
        // update n-th diagonal element
        realcholDiagUpdf(y, D + n, prep + n * 6 + n, n);
    }
}
...
#else
DISCARD_FUN(void, matcholdecomp6x6f, (void * pScr,
    float32_t * R,
    float32_t * D,
    const float32_t * A,
    const float32_t sigma2))
