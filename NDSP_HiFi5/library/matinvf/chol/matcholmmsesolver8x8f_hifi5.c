    ...
    const float32_t       sigma2)
{
    ...
    matcholfwdsubst8x8f(pScr, y, R, D, A, B);
    matcholbkwsubst8x8f(pScr, x, R, D, y);
}
...
size_t  matcholmmsesolver8x8f_getScratchSize()
{
    ...
    s_fw = matcholfwdsubst8x8f_getScratchSize();
    s_bk = matcholbkwsubst8x8f_getScratchSize();
...
#else
DISCARD_FUN(void, matcholmmsesolver8x8f, (void * pScr,
    float32_t * x,
    const float32_t * A,
    const float32_t * B,
    const float32_t sigma2))
