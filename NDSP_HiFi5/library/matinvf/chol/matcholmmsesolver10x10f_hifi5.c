    ...
    const float32_t       sigma2)
{
    ...
    matcholfwdsubst10x10f(pScr, y, R, D, A, B);
    matcholbkwsubst10x10f(pScr, x, R, D, y);
}
...
size_t  matcholmmsesolver10x10f_getScratchSize()
{
    ...
    s_fw = matcholfwdsubst10x10f_getScratchSize();
    s_bk = matcholbkwsubst10x10f_getScratchSize();
...
#else
DISCARD_FUN(void, matcholmmsesolver10x10f, (void * pScr,
    float32_t * x,
    const float32_t * A,
    const float32_t * B,
    const float32_t sigma2))
