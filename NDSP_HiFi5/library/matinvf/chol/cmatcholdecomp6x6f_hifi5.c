...
{
    static const f_cplxcholFwdrecf f_Fwdrec[6] = { cplxcholFwdrec1f, cplxcholFwdrec2f, cplxcholFwdrec3f, cplxcholFwdrec4f, cplxcholFwdrec5f, cplxcholFwdrec6f };
    int n;
    ...
    // compute A'*A
    cmatcholpreprocess6x6f(NULL, prep, A, sigma2);
    ...
    for (n = 0; n<6; n++)
    {
        ...
        cplxcholDiagUpdf(y, D + n, prep + n*7, n);
    }
}
...
#else
DISCARD_FUN(void, cmatcholdecomp6x6f, (void * pScr,
    complex_float * R,
    complex_float * D,
    const complex_float * A,
    const float32_t sigma2))
