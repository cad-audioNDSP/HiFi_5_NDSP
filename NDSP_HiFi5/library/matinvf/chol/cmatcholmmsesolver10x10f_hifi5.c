    ...
    const float32_t       sigma2)
{
    ...
    cmatcholfwdsubst10x10f(pScr, y, R, D, A, B);
    cmatcholbkwsubst10x10f(pScr, x, R, D, y);
}
...
size_t  cmatcholmmsesolver10x10f_getScratchSize()
{
    ...
    s_fw = cmatcholfwdsubst10x10f_getScratchSize();
    s_bk = cmatcholbkwsubst10x10f_getScratchSize();
    s_max = s_dc > s_fw ? s_dc : s_fw;
    s_max = s_max > s_bk ? s_max : s_bk;
    return SD + SR + SY + s_max;
}
#else
DISCARD_FUN(void, cmatcholmmsesolver10x10f, (void * pScr,
    complex_float * x,
    const complex_float * A,
    const complex_float * B,
    const float32_t sigma2))
