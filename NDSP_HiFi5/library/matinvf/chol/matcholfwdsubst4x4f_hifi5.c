...
Z[N*P]   matrix NxP
*/
static void realcomputeAB4f(float32_t* Z, const float32_t* A, const float32_t* B)
{
	...
	const float32_t * B)
{
	...
	// compute A'*B
	realcomputeAB4f(_Z, A, B);

	// forward recursion
	realcholFwdrec4f(y, R, D, _Z, 1);
}


size_t  matcholfwdsubst4x4f_getScratchSize()
{
	return 4 * sizeof(float32_t);
}
#elif (HAVE_FPU)
#define SZ_F32 (sizeof(float32_t))
/*
...
#else
DISCARD_FUN(void, matcholfwdsubst4x4f, (void * pScr,
	float32_t * y,
	const float32_t * R,
	const float32_t * D,
	const float32_t * A,
