...
{
    static const f_realcholFwdrecf f_Fwdrec[8] = { realcholFwdrec1f, realcholFwdrec2f, realcholFwdrec3f, realcholFwdrec4f, realcholFwdrec5f, realcholFwdrec6f, realcholFwdrec7f, realcholFwdrec8f };
    int n;
    ...
    // compute  A'*A 
    matcholpreprocess8x8f(NULL, prep, A, sigma2);
    ...
    for (n = 0; n<8; n++)
    {
        ...
        realcholDiagUpdf(y, D + n, prep + n * 8 + n, n);
    }
}
...
#else
DISCARD_FUN(void, matcholdecomp8x8f, (void * pScr,
    float32_t * R,
    float32_t * D,
    const float32_t * A,
    const float32_t sigma2))
