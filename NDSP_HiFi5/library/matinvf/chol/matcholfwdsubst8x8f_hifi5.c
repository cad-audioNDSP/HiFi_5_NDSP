...
Z[N*P]   matrix NxP
*/
static void realcomputeAB8f(float32_t* Z, const float32_t* A, const float32_t* B)
{
	...
	//for (m = 0; m<4; m++)
	{
		...
		MUL_SX2X2(Z0t, Z1t, A4, A5, B1, B1);
		MUL_SX2X2(Z2t, Z3t, A6, A7, B1, B1);
		...
		MADD_SX2X2(Z0t, Z1t, A4, A5, B1, B1);
		MADD_SX2X2(Z2t, Z3t, A6, A7, B1, B1);
	...
	const float32_t * B)
{
	...
	// compute A'*B
	realcomputeAB8f(_Z, A, B);

	// forward recursion
	realcholFwdrec8f(y, R, D, _Z, 1);
}


size_t  matcholfwdsubst8x8f_getScratchSize()
{
	return 8 * sizeof(float32_t);
}
#elif(HAVE_FPU)
#define SZ_F32 (sizeof(float32_t))
...
#else
DISCARD_FUN(void, matcholfwdsubst8x8f, (void * pScr,
	float32_t * y,
	const float32_t * R,
	const float32_t * D,
	const float32_t * A,
