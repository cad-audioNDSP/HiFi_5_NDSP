    ...
    const float32_t * y)
{
    ...
    const xtfloat * restrict pR5 = (const xtfloat*)(R + (6 * 5) / 2 + 0); //N*(N-1)/2
    const xtfloat * restrict pR4 = (const xtfloat*)(R + (6 * 5) / 2 + 1);
    const xtfloat * restrict pR3 = (const xtfloat*)(R + (6 * 5) / 2 + 2);
    const xtfloat * restrict pR2 = (const xtfloat*)(R + (6 * 5) / 2 + 3);
    const xtfloat * restrict pR1 = (const xtfloat*)(R + (6 * 5) / 2 + 4);
    const xtfloat * restrict pY = (const xtfloat*)(y + 5);
    const xtfloat * restrict pD = (const xtfloat*)(D + 5);
    xtfloat * restrict pX = (xtfloat*)(x + 5);
    xtfloat D0;
    xtfloat R1, R2, R3, R4, R5;
    ...
    XT_LSXP(R3, pR3, -SZ_F32 * 5);
    XT_MSUB_S(X3, X0, R3);
    XT_LSXP(R3, pR3, -SZ_F32 * 4);
    XT_MSUB_S(X3, X1, R3);
    XT_LSIP(R3, pR3, 0);
    XT_MSUB_S(X3, X2, R3);
    X3 = XT_MUL_S(X3, D0);
    ...
    XT_MSUB_S(X4, X0, R4);
    XT_LSXP(R4, pR4, -SZ_F32 * 4);
    XT_MSUB_S(X4, X1, R4);
    XT_LSXP(R4, pR4, -SZ_F32 * 3);
    XT_MSUB_S(X4, X2, R4);
    XT_LSIP(R4, pR4, 0);
    XT_MSUB_S(X4, X3, R4);
    X4 = XT_MUL_S(X4, D0);
    ...
    XT_MSUB_S(X5, X1, R5);
    XT_LSXP(R5, pR5, -SZ_F32 * 3);
    XT_MSUB_S(X5, X2, R5);
    XT_LSXP(R5, pR5, -SZ_F32 * 2);
    XT_MSUB_S(X5, X3, R5);
    XT_LSIP(R5, pR5, 0);
    XT_MSUB_S(X5, X4, R5);
    X5 = XT_MUL_S(X5, D0);
...
#else
DISCARD_FUN(void, matcholbkwsubst6x6f, (void * pScr,
    float32_t * x,
    const float32_t * R,
    const float32_t * D,
    const float32_t * y))
