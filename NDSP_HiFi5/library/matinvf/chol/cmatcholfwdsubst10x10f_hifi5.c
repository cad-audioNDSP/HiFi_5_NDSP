...
*/
void cplxcomputeAB10f(complex_float * Z,
	...
	const complex_float * B)
{
	...
	xtfloatx2 Z0t, Z1t, Z2t, Z3t, Z4t, Z5t, Z6t, Z7t, Z8t, Z9t;
	int m;



	Z0 = Z1 = Z2 = Z3 = Z4 = Z5 = Z6 = Z7 = Z8 = Z9 = XT_CONST_S(0);
	Z0t = Z1t = Z2t = Z3t = Z4t = Z5t = Z6t = Z7t = Z8t = Z9t = XT_CONST_S(0);
	for (m = 0; m<10; m += 2)
	{
		...
		AE_LSX2X2_IP(A8, A9, pA, 2 * SZ_CF32);
		...
		MADDMUX_SX2X2(Z8, Z9, A8, A9, B0, B0, 0);
		MADDMUX_SX2X2(Z8t, Z9t, A8, A9, B0, B0, 3);
	...
	AE_SSX2X2_IP(Z8, Z9, pZ, 2 * SZ_CF32);
    ...
    const complex_float * B)
{
    ...
    // compute A'*B
    cplxcomputeAB10f(_Z, A, B);

    // forward recursion
    cplxcholFwdrec10f(y, R, D, _Z, 1);
}


size_t  cmatcholfwdsubst10x10f_getScratchSize()
{
	return 2 * 10 * sizeof(float32_t);
}
#elif (HAVE_FPU)
#define SZ_F32 (sizeof(float32_t))
#define SZ_CF32 (2*SZ_F32)
    ...
    const complex_float * B)
{
    ...
    for (n = 0; n<10; n++)
    {
        ...
        for (m = 0; m<10; m++)
        {
            ...
            a_re = _A[2 * n + m * 10 * 2 + 0]; a_im = _A[2 * n + m * 10 * 2 + 1];
...
#else
    xtfloat Z_re, Z_im, A_re, A_im, B_re, B_im;
    xtfloat * restrict pZ = (xtfloat *)Z;
    const xtfloat * restrict pA = (const xtfloat *)A;
    const xtfloat * restrict pB = (const xtfloat *)B;
    int n, m;

    for (n = 0; n<10; n++)
    {
        Z_re = Z_im = XT_CONST_S(0);
        pB = (const xtfloat *)(B);
        pA = (const xtfloat *)(A + n);

        for (m = 0; m<10; m++)
        {
            XT_LSIP(A_re, pA, SZ_F32);
            XT_LSIP(A_im, pA, (10 * 2 - 1)*SZ_F32);
            XT_LSIP(B_re, pB, SZ_F32);
            XT_LSIP(B_im, pB, SZ_F32);
            XT_MADD_S(Z_re, A_re, B_re);
            XT_MADD_S(Z_re, A_im, B_im);
            XT_MADD_S(Z_im, A_re, B_im);
            XT_MSUB_S(Z_im, A_im, B_re);
        }
        XT_SSIP(Z_re, pZ, SZ_F32);
        XT_SSIP(Z_im, pZ, SZ_F32);
    }
#endif
}
/*-------------------------------------------------------------------------
...
#else
DISCARD_FUN(void, cmatcholfwdsubst10x10f, (void * pScr,
	complex_float * y,
	const complex_float * R,
	const complex_float * D,
	const complex_float * A,
