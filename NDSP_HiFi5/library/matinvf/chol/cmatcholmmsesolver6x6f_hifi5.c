    ...
    const float32_t       sigma2)
{
    ...
    cmatcholfwdsubst6x6f(pScr, y, R, D, A, B);
    cmatcholbkwsubst6x6f(pScr, x, R, D, y);
}
...
size_t  cmatcholmmsesolver6x6f_getScratchSize()
{
    ...
    s_fw = cmatcholfwdsubst6x6f_getScratchSize();
    s_bk = cmatcholbkwsubst6x6f_getScratchSize();
    s_max = s_dc > s_fw ? s_dc : s_fw;
    s_max = s_max > s_bk ? s_max : s_bk;
    return SD + SR + SY + s_max;
}
#else
DISCARD_FUN(void, cmatcholmmsesolver6x6f, (void * pScr,
    complex_float * x,
    const complex_float * A,
    const complex_float * B,
    const float32_t sigma2))
