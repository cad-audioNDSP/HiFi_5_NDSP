...
Z[N*P]   matrix NxP
*/
static void realcomputeAB6f(float32_t* Z, const float32_t* A, const float32_t* B)
{
	...
	xtfloatx2 A0, A1, A2, A3, A4, A5;
	xtfloatx2 B0, B1;
	xtfloatx2 Z0, Z1, Z2;
	xtfloatx2 Z0t, Z1t, Z2t;
	//int m;
	
	//Z0 = Z1 = Z2 = XT_CONST_S(0);
	//Z0t = Z1t = Z2t = XT_CONST_S(0);
	//for (m = 0; m<3; m++)
	{
		...
		MUL_SX2X2(Z2, Z0t, A2, A3, B0, B1);
		MUL_SX2X2(Z1t, Z2t, A4, A5, B1, B1);
		...
		MADD_SX2X2(Z2, Z0t, A2, A3, B0, B1);
		MADD_SX2X2(Z1t, Z2t, A4, A5, B1, B1);
	...
	XT_SSX2IP(Z2, castxcc(xtfloatx2, pZ), 2 * SZ_F32);
	...
	const float32_t * B)
{
	...
	// compute A'*B
	realcomputeAB6f(_Z, A, B);

	// forward recursion
	realcholFwdrec6f(y, R, D, _Z, 1);
}


size_t  matcholfwdsubst6x6f_getScratchSize()
{
	return 6 * sizeof(float32_t);
}
#elif(HAVE_FPU)
#define SZ_F32 (sizeof(float32_t))
/*
...
#else
DISCARD_FUN(void, matcholfwdsubst6x6f, (void * pScr,
	float32_t * y,
	const float32_t * R,
	const float32_t * D,
	const float32_t * A,
