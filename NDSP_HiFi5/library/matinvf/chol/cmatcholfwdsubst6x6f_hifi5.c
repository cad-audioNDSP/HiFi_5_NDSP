...
*/
void cplxcomputeAB6f(complex_float * Z,
	...
	const complex_float * B)
{
	...
	xtfloatx2 B0, B1;
	xtfloatx2 Z0, Z1, Z2, Z3, Z4, Z5;
	xtfloatx2 Z0t, Z1t, Z2t, Z3t, Z4t, Z5t;
	
	Z0 = Z1 = Z2 = Z3 = Z4 = Z5 = XT_CONST_S(0);
	Z0t = Z1t = Z2t = Z3t = Z4t = Z5t = XT_CONST_S(0);
	//for (m = 0; m<6; m++)
	{
	...
	const complex_float * B)
{
	...
	// compute A'*B
	cplxcomputeAB6f(_Z, A, B);

	// forward recursion
	cplxcholFwdrec6f(y, R, D, _Z, 1);
}


size_t  cmatcholfwdsubst6x6f_getScratchSize()
{
	return 2 * 6 * sizeof(float32_t);
}
    ...
    const complex_float * B)
{
    ...
    for (n = 0; n<6; n++)
    {
        ...
        for (m = 0; m<6; m++)
        {
            ...
            a_re = _A[2 * n + m * 6 * 2 + 0]; a_im = _A[2 * n + m * 6 * 2 + 1];
...
#else
DISCARD_FUN(void, cmatcholfwdsubst6x6f, (void * pScr,
	complex_float * y,
	const complex_float * R,
	const complex_float * D,
	const complex_float * A,
