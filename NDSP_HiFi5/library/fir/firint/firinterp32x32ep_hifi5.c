...
typedef struct tag_firinterp32x32ep_t
{
...
} firinterp32x32ep_t, *firinterp32x32ep_ptr_t;
...
size_t firinterp32x32ep_alloc( int D, int M )
{
  ...
  return ( ALIGNED_SIZE( sizeof( firinterp32x32ep_t ), 4 )
           + // Delay line
...
{
  firinterp32x32ep_ptr_t firinterp;
  void *              ptr;
  int32_t *           delLine;
  int                 delLen;
  int32_t *           coef;
  ...
  procFxn = ( D == 2 ? &firinterp32x32ep_D2_proc :
              D == 3 ? &firinterp32x32ep_D3_proc :
              D == 4 ? &firinterp32x32ep_D4_proc :
                       &firinterp32x32ep_DX_proc );

   //
  // Partition the memory block.
  //
  ptr       = objmem;
  firinterp = (firinterp32x32ep_ptr_t)ALIGNED_ADDR( ptr, 4 );
  ptr       = firinterp + 1;
  ...
  NASSERT( (int8_t*)ptr - (int8_t*)objmem <=
          (int)firinterp32x32ep_alloc( D, M ) );
                       ...
                       const int32_t * restrict          x, int N )
{
  ...
  firinterp32x32ep_ptr_t firinterp = (firinterp32x32ep_ptr_t)handle;
  if (N<=0) return;
