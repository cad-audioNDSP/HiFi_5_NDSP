...
int firinterp32x16_D4_proc( int32_t * restrict y,
                      ...
                      int wrIx, int D, int N, int M )
{
    ...
    for (n = 0; n < (N >> 2); n++)
    {
        ...
        for (m = 0; m < (M >> 2); m++)
        {
            h3 = AE_L16X4_X(pH, 3 * M * sizeof(int16_t));
            ...
            AE_MULA2Q32X16_FIR_L(q4, q5, d0, d1, d2, h1);
            AE_MULA2Q32X16_FIR_L(q6, q7, d1, d2, d3, h1);
            AE_MULA2Q32X16_FIR_L(q8, q9, d0, d1, d2, h2);
            AE_MULA2Q32X16_FIR_L(qa, qb, d1, d2, d3, h2);
            AE_MULA2Q32X16_FIR_L(qc, qd, d0, d1, d2, h3);
            AE_MULA2Q32X16_FIR_L(qe, qf, d1, d2, d3, h3);
        }

        d0 = AE_ROUND32X2F48SASYM(AE_SLAI64(q0, 3), AE_SLAI64(q4, 3));
        AE_PKSR32(d1, q8, 3); AE_PKSR32(d1, qc, 3);
        AE_SA32X2X2_IP(d0, d1, aY, pY);

        d0 = AE_ROUND32X2F48SASYM(AE_SLAI64(q1, 3), AE_SLAI64(q5, 3));
        AE_PKSR32(d1, q9, 3); AE_PKSR32(d1, qd, 3);
        AE_SA32X2X2_IP(d0, d1, aY, pY);

        d0 = AE_ROUND32X2F48SASYM(AE_SLAI64(q2, 3), AE_SLAI64(q6, 3));
        AE_PKSR32(d1, qa, 3); AE_PKSR32(d1, qe, 3);
        AE_SA32X2X2_IP(d0, d1, aY, pY);

        d0 = AE_ROUND32X2F48SASYM(AE_SLAI64(q3, 3), AE_SLAI64(q7, 3));
        AE_PKSR32(d1, qb, 3); AE_PKSR32(d1, qf, 3);
        AE_SA32X2X2_IP(d0, d1, aY, pY);
    }
