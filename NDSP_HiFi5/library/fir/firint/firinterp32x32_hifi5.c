...
size_t firinterp32x32_alloc( int D, int M )
{
    ...
    return (ALIGNED_SIZE(sizeof(firinterp32x32_t), 4)
            + // Delay line
            ALIGNED_SIZE(delLen*sz_i32, 16)
            + // Coefficients
            ALIGNED_SIZE(coefNum*sz_i32, 16));
} /* firinterp32x32_alloc() */

/* Initialize the interpolator structure. The delay line is zeroed. */
firinterp32x32_handle_t firinterp32x32_init( void * objmem, int D, int M,
                                             const int32_t * restrict h )
{
    ...
    ptr         = delLine + delLen;
    coef        = (int32_t*)ALIGNED_ADDR(ptr, 16);
    ptr         = coef + coefNum;

    NASSERT((int8_t*)ptr - (int8_t*)objmem <= (int)firinterp32x32_alloc(D, M));
...
{
    firinterp32x32_ptr_t firinterp = (firinterp32x32_ptr_t)handle;
