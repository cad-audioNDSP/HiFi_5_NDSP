                      ...
                      int wrIx, int D, int N, int M )
{
    ...
    for (n = 0; n < (N >> 3); n++)
    {
        ...
        for (m = 0; m < (M >> 2); m++)
        {
            ...
            AE_MULAFQ16X2_FIR_2(q10, q11, d0, d1, h2);
            AE_MULAFQ16X2_FIR_0(q12, q13, d0, d1, h2);
            AE_MULAFQ16X2_FIR_2(q14, q15, d1, d2, h2);
            AE_MULAFQ16X2_FIR_0(q16, q17, d1, d2, h2);
        }

        t0 = AE_TRUNCA32X2F64S(q0, q8, 32);
        t1 = AE_SAT32X2(q10, q1);
        d0 = AE_ROUND16X4F32SASYM(t0, t1);
        // Scale output samples
        d0 = AE_MULP16X16X4S(d0, 3);
        AE_SA16X4_IP(d0, aY, pY);

        t0 = AE_TRUNCA32X2F64S(q9, q11, 32);
        t1 = AE_SAT32X2(q2, qa);
        d0 = AE_ROUND16X4F32SASYM(t0, t1);
        // Scale output samples
        d0 = AE_MULP16X16X4S(d0, 3);
        ...
        t0 = AE_TRUNCA32X2F64S(q12, q3, 32);
        t1 = AE_SAT32X2(qb, q13);
        d0 = AE_ROUND16X4F32SASYM(t0, t1);
        // Scale output samples
        d0 = AE_MULP16X16X4S(d0, 3);
        AE_SA16X4_IP(d0, aY, pY);

        t0 = AE_TRUNCA32X2F64S(q4, qc, 32);
        t1 = AE_SAT32X2(q14, q5);
        d0 = AE_ROUND16X4F32SASYM(t0, t1);
        // Scale output samples
        d0 = AE_MULP16X16X4S(d0, 3);
        AE_SA16X4_IP(d0, aY, pY);

        t0 = AE_TRUNCA32X2F64S(qd, q15, 32);
        t1 = AE_SAT32X2(q6, qe);
        d0 = AE_ROUND16X4F32SASYM(t0, t1);
        // Scale output samples
        d0 = AE_MULP16X16X4S(d0, 3);
        ...
        t0 = AE_TRUNCA32X2F64S(q16, q7, 32);
        t1 = AE_SAT32X2(qf, q17);
        d0 = AE_ROUND16X4F32SASYM(t0, t1);
        // Scale output samples
        d0 = AE_MULP16X16X4S(d0, 3);
        AE_SA16X4_IP(d0, aY, pY);
