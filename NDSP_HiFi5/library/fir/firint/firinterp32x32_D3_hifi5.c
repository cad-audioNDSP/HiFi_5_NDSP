                      ...
                      int wrIx, int D, int N, int M )
{
    ...
    for (n = 0; n < (N >> 2); n++)
    {
        ...
        q8 = q9 = qa = qb = AE_ZERO64();

        __Pragma("loop_count min=1");
        for (m = 0; m < (M >> 2); m++)
        {
            AE_L32X2X2_X(h2, h5, pH, 2 * M * sizeof(int32_t));
            AE_L32X2X2_X(h1, h4, pH, 1 * M * sizeof(int32_t));
            AE_L32X2X2_IP(h0, h3, pH, 4 * sizeof(int32_t));
            ...
            AE_MULAFD32X2RA_FIR_L(q0, q1, d1, d2, h3);
            AE_MULAFD32X2RA_FIR_L(q2, q3, d2, d3, h3);
            AE_MULAFD32X2RA_FIR_L(q4, q5, d1, d2, h4);
            AE_MULAFD32X2RA_FIR_L(q6, q7, d2, d3, h4);
            AE_MULAFD32X2RA_FIR_L(q8, q9, d1, d2, h5);
            AE_MULAFD32X2RA_FIR_L(qa, qb, d2, d3, h5);
        }

        d0 = AE_ROUND32X2F48SASYM(q0, q4);
        d1 = AE_ROUND32X2F48SASYM(q8, q1);
        AE_MUL2P32X4S(d0, d1, d0, d1, 3, 3);
        AE_SA32X2X2_IP(d0, d1, aY, pY);

        d0 = AE_ROUND32X2F48SASYM(q5, q9);
        d1 = AE_ROUND32X2F48SASYM(q2, q6);
        AE_MUL2P32X4S(d0, d1, d0, d1, 3, 3);
        AE_SA32X2X2_IP(d0, d1, aY, pY);

        d0 = AE_ROUND32X2F48SASYM(qa, q3);
        d1 = AE_ROUND32X2F48SASYM(q7, qb);
        AE_MUL2P32X4S(d0, d1, d0, d1, 3, 3);
        AE_SA32X2X2_IP(d0, d1, aY, pY);
    }
