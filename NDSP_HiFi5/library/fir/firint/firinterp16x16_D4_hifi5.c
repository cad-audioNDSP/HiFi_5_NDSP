                      ...
                      int wrIx, int D, int N, int M )
{
    ...
    const ae_int16x4 *          S0;
    const ae_int16x4 *          S1;
    const ae_int16x4 *          pH0;
    const ae_int16x4 *          pH1;
    ...
    for (n = 0; n < (N >> 2); n++)
    {
        ...
        pH0 = (const ae_int16x4 *)h;
        pH1 = (const ae_int16x4 *)(h + M_);
        ...
        for (m = 0; m < (M >> 2); m++)
        {
            AE_L16X4_XC(d1, S0, 4 * sizeof(int16_t));

            h3 = AE_L16X4_X(pH1, 2 * M_ * sizeof(int16_t));
            AE_L16X4_IP(h1, pH1, 4 * sizeof(int16_t));
            h2 = AE_L16X4_X(pH0, 2 * M_ * sizeof(int16_t));
            AE_L16X4_IP(h0, pH0, 4 * sizeof(int16_t));

            AE_MULAFQ16X2_FIR_2(q0, q1, d0, d1, h0);
            AE_MULAFQ16X2_FIR_0(q2, q3, d0, d1, h0);
            AE_MULAFQ16X2_FIR_2(q4, q5, d0, d1, h1);
            AE_MULAFQ16X2_FIR_0(q6, q7, d0, d1, h1);

            AE_MULAFQ16X2_FIR_2(q8, q9, d0, d1, h2);
            AE_MULAFQ16X2_FIR_0(qa, qb, d0, d1, h2);
            AE_MULAFQ16X2_FIR_2(qc, qd, d0, d1, h3);
            AE_MULAFQ16X2_FIR_0(qe, qf, d0, d1, h3);

            AE_L16X4_XC(d0, S1, 4 * sizeof(int16_t));
        }

        t0 = AE_TRUNCA32X2F64S(q0, q4, 34);
        t1 = AE_TRUNCA32X2F64S(q8, qc, 34);
        AE_SA16X4_IP(AE_ROUND16X4F32SASYM(t0, t1), aY, pY);
        t0 = AE_TRUNCA32X2F64S(q1, q5, 34);
        t1 = AE_TRUNCA32X2F64S(q9, qd, 34);
        AE_SA16X4_IP(AE_ROUND16X4F32SASYM(t0, t1), aY, pY);
        t0 = AE_TRUNCA32X2F64S(q2, q6, 34);
        t1 = AE_TRUNCA32X2F64S(qa, qe, 34);
        AE_SA16X4_IP(AE_ROUND16X4F32SASYM(t0, t1), aY, pY);
        t0 = AE_TRUNCA32X2F64S(q3, q7, 34);
        t1 = AE_TRUNCA32X2F64S(qb, qf, 34);
        AE_SA16X4_IP(AE_ROUND16X4F32SASYM(t0, t1), aY, pY);
    }
    AE_SA64POS_FP(aY, pY);
