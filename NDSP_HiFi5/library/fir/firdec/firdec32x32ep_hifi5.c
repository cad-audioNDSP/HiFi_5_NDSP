...
typedef struct tag_firdec32x32ep_t
{
...
} firdec32x32ep_t, *firdec32x32ep_ptr_t;
...
size_t firdec32x32ep_alloc( int D, int M )
{
  ...
  return ( ALIGNED_SIZE( sizeof( firdec32x32ep_t ), 4 )
           + // Delay line
...
firdec32x32ep_handle_t firdec32x32ep_init( void * objmem, int D,
                                       int M, const int32_t * restrict h )
{
  int inc;
  firdec32x32ep_ptr_t firdec;
  void *            ptr;
  int32_t *         delLine;
  int               delLen;
  int               coefNum;
  ...
  coefNum  = bM + 4;
  delLen = bM + 4*D;
  procFxn = ( D == 2 ? fir32x32ep_D2_proc :
              D == 3 ? fir32x32ep_D3_proc :
              D == 4 ? fir32x32ep_D4_proc :
                       fir32x32ep_DX_proc );
  //
  ...
  inc=2-2*inc;
  ptr       = objmem;
  firdec    = (firdec32x32ep_ptr_t)(ALIGNED_ADDR( ptr, 4 ));
  ptr       = firdec + 1;
  ...
  NASSERT( (int8_t*)ptr - (int8_t*)objmem <= (int)firdec32x32ep_alloc( D,M ) );
...
void firdec32x32ep_process( firdec32x32ep_handle_t _firdec,
                          int32_t * restrict       y,
                    const int32_t *                x, int N )
{

  firdec32x32ep_ptr_t firdec = (firdec32x32ep_ptr_t)_firdec;
  if (N<=0) return;
  NASSERT(N%8==0);
  //
