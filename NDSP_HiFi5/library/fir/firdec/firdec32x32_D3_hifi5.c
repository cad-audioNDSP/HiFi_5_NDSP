               ...
               int wrIx, int D, int N, int M)
{
    ...
    const ae_int32x4  *          pH0;
    const ae_int32x4  *          pH1;
    ...
    ae_int32x2 d8, d9, da, db, dc, dd, de;
    ...
    for (n = 0; n < (N >> 3); n++)
    {
        ...
        AE_L32X2X2_IP(d8, d9, pX, 4 * sizeof(int32_t));
        AE_L32X2X2_IP(da, db, pX, 4 * sizeof(int32_t));
        ...
        AE_S32X2X2_XC(d8, d9, pDw, 4 * sizeof(int32_t));
        AE_S32X2X2_XC(da, db, pDw, 4 * sizeof(int32_t));

        pH0 = (const ae_int32x4 *)h;
        pH1 = (const ae_int32x4 *)(h + 1);
        aH1 = AE_LA128_PP(pH1);
        AE_ADDCIRC_XC(pDr, 24 * sizeof(int32_t)); pD0 = (const ae_int32x4 *)pDr;
        ...
        for (m = 0; m < (M >> 3) + 1; m++)
        {
            ...
            AE_L32X2_XC(de, castxcc(ae_int32x2, pD3), 4 * sizeof(int32_t));

            AE_L32X2X2_IP(h0, h1, pH0, 4 * sizeof(int32_t));
            AE_LA32X2X2_IP(h2, h3, aH1, pH1);
            AE_MULAAF2D32RA_HH_LL(q0, q1, d0, d2, h0, h2);
            AE_MULAAF2D32RA_HH_LL(q2, q3, d3, d5, h0, h2);
            AE_MULAAF2D32RA_HH_LL(q4, q5, d6, d8, h0, h2);
            AE_MULAAF2D32RA_HH_LL(q6, q7, d9, db, h0, h2);
            AE_MULAAF2D32RA_HH_LL(q0, q1, d1, d3, h1, h3);
            AE_MULAAF2D32RA_HH_LL(q2, q3, d4, d6, h1, h3);
            AE_MULAAF2D32RA_HH_LL(q4, q5, d7, d9, h1, h3);
            AE_MULAAF2D32RA_HH_LL(q6, q7, da, dc, h1, h3);

            AE_L32X2X2_IP(h0, h1, pH0, 4 * sizeof(int32_t));
            AE_LA32X2X2_IP(h2, h3, aH1, pH1);
            AE_MULAAF2D32RA_HH_LL(q0, q1, d2, d4, h0, h2);
            AE_MULAAF2D32RA_HH_LL(q2, q3, d5, d7, h0, h2);
            AE_MULAAF2D32RA_HH_LL(q4, q5, d8, da, h0, h2);
            AE_MULAAF2D32RA_HH_LL(q6, q7, db, dd, h0, h2);
            AE_MULAAF2D32RA_HH_LL(q0, q1, d3, d5, h1, h3);
            AE_MULAAF2D32RA_HH_LL(q2, q3, d6, d8, h1, h3);
            AE_MULAAF2D32RA_HH_LL(q4, q5, d9, db, h1, h3);
            AE_MULAAF2D32RA_HH_LL(q6, q7, dc, de, h1, h3);
