               ...
               int wrIx, int D, int N, int M)
{
    ...
    for (n = 0; n < (N >> 3); n++)
    {
        ...
        pH0 = (const ae_int16x8 *)h;
        pH1 = (const ae_int16x8 *)(h + 5);
        aH1 = AE_LA128_PP(pH1);
        AE_ADDCIRC_XC(castxcc(ae_int64, pDr), 24 * sizeof(int32_t));

        q0 = q1 = q2 = q3 = q4 = q5 = q6 = q7 = AE_ZERO64();

        AE_L16X4X2_IP(h0, h1, pH0, 8 * sizeof(int16_t));
        h2 = AE_SEL16I(h0, h1, 5);

        AE_L32X2X2_XC(d0, d1, pDr, 4 * sizeof(int32_t));
        AE_L32X2X2_XC(d2, d3, pDr, 4 * sizeof(int32_t));
        AE_L32X2X2_XC(d4, d5, pDr, 4 * sizeof(int32_t));
        AE_L32X2X2_XC(d6, d7, pDr, 4 * sizeof(int32_t));
        AE_L32X2X2_XC(d8, d9, pDr, 4 * sizeof(int32_t));
        AE_L32X2X2_XC(da, db, pDr, 4 * sizeof(int32_t));
        dc = AE_L32X2_I((ae_int32x2 *)pDr, 0);

        AE_MULAAAAQ32X16(q0, d0, d1, h0);
        AE_MULAAAAQ32X16(q2, d3, d4, h0);
        AE_MULAAAAQ32X16(q4, d6, d7, h0);
        AE_MULAAAAQ32X16(q6, d9, da, h0);
        AE_MULAAAA2Q32X16(q1, q0, d2, d3, h2, h1);
        AE_MULAAAA2Q32X16(q3, q2, d5, d6, h2, h1);
        AE_MULAAAA2Q32X16(q5, q4, d8, d9, h2, h1);
        AE_MULAAAA2Q32X16(q7, q6, db, dc, h2, h1);
        ...
        {
            AE_L16X4X2_IP(h0, h1, pH0, 8 * sizeof(int16_t));
            AE_LA16X4X2_IP(h2, h3, aH1, pH1);

            AE_L32X2X2_XC(d8, d9, pDr, 4 * sizeof(int32_t));
            AE_L32X2X2_XC(da, db, pDr, 4 * sizeof(int32_t));
            dc = AE_L32X2_I((ae_int32x2 *)pDr, 0);

            AE_MULAAAA2Q32X16(q1, q0, d0, d1, h2, h0);
            AE_MULAAAA2Q32X16(q3, q2, d3, d4, h2, h0);
            AE_MULAAAA2Q32X16(q5, q4, d6, d7, h2, h0);
            AE_MULAAAA2Q32X16(q7, q6, d9, da, h2, h0);
            AE_MULAAAA2Q32X16(q1, q0, d2, d3, h3, h1);
            AE_MULAAAA2Q32X16(q3, q2, d5, d6, h3, h1);
            AE_MULAAAA2Q32X16(q5, q4, d8, d9, h3, h1);
            AE_MULAAAA2Q32X16(q7, q6, db, dc, h3, h1);
