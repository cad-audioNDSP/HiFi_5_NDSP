...
/* Data processing function for a factor 3 decimating FIR filter. */
 int fir32x32ep_D3_proc( int32_t * restrict y,
                                  int32_t * delayLine,
                                  int32_t delayLen,
                          ...
                          int wrIx, int D, int N, int M )
{
  ...
  for ( n=0; n<(N>>2); n++ )
  {
    ...
    for ( m=0; m<(M>>2)+1; m++ )
    {
      ...
      AE_MULAAD32EP_HH_LL (ep1,q1,d0,c0);
      AE_MULAAD32EP_HH_LL (ep2,q2,t3,c0);
      AE_MULAAD32EP_HH_LL (ep3,q3,d3,c0);
      AE_MULAAD32EP_HH_LL (ep0,q0,t1,c1);
      AE_MULAAD32EP_HH_LL (ep1,q1,d1,c1);
      AE_MULAAD32EP_HH_LL (ep2,q2,t4,c1);
      AE_MULAAD32EP_HH_LL (ep3,q3,d4,c1);
