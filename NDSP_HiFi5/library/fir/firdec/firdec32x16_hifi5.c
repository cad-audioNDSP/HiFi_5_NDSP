...
size_t firdec32x16_alloc( int D,  int M )
{
    ...
    return (ALIGNED_SIZE(sizeof(firdec32x16_t), 4)
            + // Delay line
            ALIGNED_SIZE(delayLen*sz_i32, 16)
            + // Coefficients
            ALIGNED_SIZE(coefNum*sz_i16, 16));
} /* firdec32x16_alloc() */

/* Initialize the decimator structure. The delay line is zeroed. */
firdec32x16_handle_t firdec32x16_init( void * objmem, int D, int M,
                                       const int16_t * restrict h )
{
    ...
    NASSERT((int8_t*)ptr - (int8_t*)objmem <= (int)firdec32x16_alloc(D, M));
