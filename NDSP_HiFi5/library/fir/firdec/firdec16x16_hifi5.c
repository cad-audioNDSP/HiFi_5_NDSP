...
typedef struct tag_firdec16x16_t
{
...
size_t firdec16x16_alloc( int D, int M )
{
    ...
    return (ALIGNED_SIZE(sizeof(firdec16x16_t), 4)
            + // Delay line
            ALIGNED_SIZE(delayLen*sz_i16, 16)
            + // Coefficients
            ALIGNED_SIZE(coefNum*sz_i16, 16));
} /* firdec16x16_alloc() */

/* Initialize the decimator structure. The delay line is zeroed. */
firdec16x16_handle_t firdec16x16_init( void * objmem, int D, int M,
                                       const int16_t * restrict h )
{
    ...
    // Begin by a few zeros to make the number of coefficients a multiple of 8.
    for (m = M; m < bM; m++)
    {
        *coefB++ = 0;
    }

    // Pad the coefficients with 16 zeros (one at the beginning, 15 following
...
{
    firdec16x16_ptr_t firdec = (firdec16x16_ptr_t)handle;
