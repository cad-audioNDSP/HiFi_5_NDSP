...
size_t firdec32x32_alloc( int D,  int M )
{
    ...
    return (ALIGNED_SIZE(sizeof(firdec32x32_t), 4)
            + // Delay line
            ALIGNED_SIZE(delayLen*sz_i32, 32)
            + // Coefficients
            ALIGNED_SIZE(coefNum*sz_i32, 32))
             + (1*sz_i32); //extra element for better performance
} /* firdec32x32_alloc() */

/* Initialize the decimator structure. The delay line is zeroed. */
firdec32x32_handle_t firdec32x32_init( void * objmem, int D, int M,
                                       const int32_t * restrict h )
{
    ...
    ptr     = firdec + 1;
    delLine = (int32_t*)(ALIGNED_ADDR(ptr, 32));
    ptr     = delLine + delLen + 1;
    coef    = (int32_t*)(ALIGNED_ADDR(ptr, 32));
    ptr     = coef + coefNum;
...
{
    firdec32x32_ptr_t firdec = (firdec32x32_ptr_t)handle;
