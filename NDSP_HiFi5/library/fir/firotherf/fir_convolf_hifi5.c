                ...
                int N, int M )
{
    ...
    for (n = 0; n < ((N+7)&~7); n += 8)
    {
        ...
        pX3 = pX;
        AE_ADDCIRC_XC(castxcc(ae_int64, pX3), 3 * sizeof(float32_t));
        pX4 = pX3;
        AE_ADDCIRC_XC(castxcc(ae_int64, pX4), 4 * sizeof(float32_t));
        pX = pX2;
        AE_LASX2X2POS_PC(aX0, pX0);
        ...
        /* preload data from x */
        AE_LASX2X2_IC(x0, x2, aX0, pX0);
        x1 = XT_SEL32_LH_SX2(x0, x2);
        AE_LSX2X2_I(x3, x5, pX3, 0);

        __Pragma("loop_count min=1");
        for (m = 0; m < (M >> 2); m++)
        {
            /* load data from x */
            AE_LASX2X2_IC(x4, x6, aX1, pX1);
            AE_LASX2X2_IC(x8, xa, aX2, pX2);
            AE_LSX2X2_XC(x7, x9, pX4, 4 * sizeof(float32_t));
            /* load data from y */
            AE_L64X2_IP(t1, t0, castxcc(ae_int64x2, pY), -4 * (int)sizeof(float32_t));
            y0 = AE_MOVXTFLOATX2_FROMINT32X2(AE_MOVINT32X2_FROMINT64(t0));
            y1 = AE_MOVXTFLOATX2_FROMINT32X2(AE_MOVINT32X2_FROMINT64(t1));
            ...
            AE_LASX2X2_IC(x0, x2, aX0, pX0);
            AE_LSX2X2_XC(xa, x1, pX3, 4 * sizeof(float32_t));
            AE_LSX2X2_I(x3, x5, pX3, 0);
        }
        /* reduction add */
