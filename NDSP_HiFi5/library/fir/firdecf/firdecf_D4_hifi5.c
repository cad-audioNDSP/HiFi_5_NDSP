...
int fir_decimaf_4x(float32_t * restrict y,
             ...
             int wrIx, int D, int N, int M)
{
    ...
    for (n = 0; n < (N >> 2); n++)
    {
        ...
        AE_ADDCIRC_XC(pDr, 32 * sizeof(float32_t)); pD0 = (const xtfloatx4 *)pDr;
        ...
        for (m = 0; m < (M >> 3) + 1; m++)
        {
            ...
            AE_LSX2X2_XC(d8, d9, pD2, 8 * sizeof(float32_t));

            AE_LSX2X2_IP(h0, h1, pH, 4 * sizeof(float32_t));
            MADD_SX2X2(q0, q1, d0, d2, h0, h0);
            MADD_SX2X2(q2, q3, d4, d6, h0, h0);
            MADD_SX2X2(q4, q5, d1, d3, h1, h1);
            MADD_SX2X2(q6, q7, d5, d7, h1, h1);

            AE_LSX2X2_IP(h0, h1, pH, 4 * sizeof(float32_t));
            MADD_SX2X2(q8, q9, d2, d4, h0, h0);
            MADD_SX2X2(qa, qb, d6, d8, h0, h0);
            MADD_SX2X2(qc, qd, d3, d5, h1, h1);
            MADD_SX2X2(qe, qf, d7, d9, h1, h1);
...
#elif (HAVE_FPU)
// for scalar FPU
int fir_decimaf_4x(float32_t * restrict z,
