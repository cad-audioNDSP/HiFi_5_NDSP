...
#if !HAVE_HPFPU
DISCARD_FUN(void,conv2d_gen_11x7_fp16,(void* pScr, float16_t *z, const float16_t * x, const float16_t * y, int P, int Q))
size_t conv2d_gen_11x7_fp16_getScratchSize (int P, int Q) 
{ 
    (void)P,(void)Q;
    return 0;
}
#elif HAVE_HPFPU
void conv2d_gen_11x7_fp16(void* pScr, float16_t* z, const float16_t* x, const float16_t* y, int P, int Q)
{
#   define M 11
#   define N 7
    ...
    for (i = 0; i < P + M - 1; i += 1)
    {
        ...
        if (mmax-mmin == 1)
        {
            ...
            if (Q&7)
            {
                AE_LAVHX4X2_XP(Y02, Y03, al0, pY0, (Q&7)*sizeof(xthalf));
                ...
                AE_SAVHX4X2_XP(z0, r0, alZ, pZ, (Q&7) * sizeof(xthalf));

                for (j = 0; j < (Q & 7); ++j)
                {
                    Y00 = AE_SELH_6543(Y00, Y01); Y01 = AE_SELH_6543(Y01, Y02); 
                    Y02 = AE_SELH_6543(Y02, Y03); Y03 = AE_SELH_6543(Y03, CONST_HX4(0));
                }
            }
        ...
        else
        {
            ...
            if (mmax-mmin!=M)
            {
                ...
                // last itteration
                {
                    ...
                    pY0 = pY;
                    pZ = (xthalfx8*)(z + (i + 0) * (N + Q - 1));
                    pTread = (xthalfx8*)((xthalfx4*)(pScr)+2 * M);
            ...
            else
            {
                ...
                //first 3 iter
                {
                    ...
                    if(Q&7)
                    {
                        ...
                        AE_LHX4X2_IP(W0123, W456z, pW, sizeof(xthalfx8));
                        AE_LAVHX4X2_XP(Y12, Y13, al1, pY1, (Q & 7) * sizeof(xthalf));
                        MULACNVL_HX4X2(z0, z1, W0123, Y10, Y11);
                        MULACNVH_HX4X2(z2, z3, W0123, Y11, Y12);
                        MULACNVL_HX4X2(z0, z1, W456z, Y11, Y12);
                        MULACNVH_HX4X2(z2, z3, W456z, Y12, Y12);
                        ...
                        AE_LHX4X2_XP(W0123, W456z, pW, -2 * (int)sizeof(xthalfx8));
                        AE_LAVHX4X2_XP(Y22, Y23, al2, pY2, (Q & 7) * sizeof(xthalf));
                        MULACNVL_HX4X2(z0, z1, W0123, Y20, Y21);
                        MULACNVH_HX4X2(z2, z3, W0123, Y21, Y22);
                        MULACNVL_HX4X2(z0, z1, W456z, Y21, Y22);
                        MULACNVH_HX4X2(z2, z3, W456z, Y22, Y22);
                        ...
                        for (j = 0; j < (Q & 7); ++j)
                        {
                            ...
                            Y10 = AE_SELH_6543(Y10, Y11); Y11 = AE_SELH_6543(Y11, Y12);
                            Y12 = AE_SELH_6543(Y12, Y13); Y13 = AE_SELH_6543(Y13, CONST_HX4(0));

                            Y20 = AE_SELH_6543(Y20, Y21); Y21 = AE_SELH_6543(Y21, Y22);
                            Y22 = AE_SELH_6543(Y22, Y23); Y23 = AE_SELH_6543(Y23, CONST_HX4(0));
                        }
                    }
...
size_t conv2d_gen_11x7_fp16_getScratchSize(int P, int Q)
{
    int M = 11;
    int N = 7;
    return (M * 2 + (Q+N-1)*4 + 8) * sizeof(xthalfx4);
...
size_t conv2d_gen_11x7_fp16_getScratchSize(int M, int N, int P, int Q)
{
    return 0;
}
...
void conv2d_gen_11x7_fp16(void* pScr, float16_t* z, const float16_t* x, const float16_t* y, int P, int Q)
{
    ...
    NASSERT(P >= 0 && P % 8 == 0);
    NASSERT(Q >= 0 && Q % 8 == 0);
    conv2d_gen_fp16(pScr, z, x, y, 11, 7, P, Q);
}
