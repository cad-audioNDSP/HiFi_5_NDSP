...
#if defined(AE_MUL4O8X16CNV_H) & defined(AE_SAV16X4X2_XP)  // NN extension 

void conv2d_3x3_8x16(void* pScr, int16_t* z, const int8_t* x, const int16_t* y, int rsh, int P, int Q)
{
#define M 3
#define N 3
        ...
        for (i = 0; i < P + M - 1; i += 1)
        {
            ...
            if (mmax - mmin == 3)
            {
...

                {
                    ...
                    AE_L16X4X2_IP(Y11, Y12, pY1, sizeof(ae_int16x8));
                    AE_L16X4X2_IP(Y21, Y22, pY2, sizeof(ae_int16x8));
    ...
    else
    {
        ...
        conv2d_3x3_16x16((int16_t*)pScr + 16, z, (const int16_t*)pScr, y, rsh, P, Q);
    }
#undef M
#undef N
}
...
size_t conv2d_3x3_8x16_getScratchSize(int P, int Q)
{
    if (Q>=128)
        return 4 * sizeof(ae_int8x8);
    else 
        return 16 * sizeof(int16_t)+ conv2d_3x3_16x16_getScratchSize(P,Q);
}
