...
void conv2d_11x7_8x16(void* pScr, int16_t* z, const int8_t* x, const int16_t* y, int rsh, int P, int Q)
{
#define M 11
#define N 7
    ...
    for (i = 0; i < P + M - 1; i += 1)
    {
        ...
        if (mmax - mmin == 11)
        {
            ...
            // first 4 iteration
            {
                ...
                Y00 = 0;
                AE_L16X4X2_XP(Y02, Y03, pY0, 2 * sizeof(ae_int16));
                Y01 = AE_SEL16_5432(Y00, Y02);
                al0 = AE_LA128_PP(pY0);

                Y10 = 0;
                AE_L16X4X2_XP(Y12, Y13, pY1, 2 * sizeof(ae_int16));
                Y11 = AE_SEL16_5432(Y10, Y12);
                al1 = AE_LA128_PP(pY1);

                Y20 = 0;
                AE_L16X4X2_XP(Y22, Y23, pY2, 2 * sizeof(ae_int16));
                Y21 = AE_SEL16_5432(Y20, Y22);
                al2 = AE_LA128_PP(pY2);

                Y30 = 0;
                AE_L16X4X2_XP(Y32, Y33, pY3, 2 * sizeof(ae_int16));
...
size_t conv2d_11x7_8x16_getScratchSize(int P, int Q)
{
    if (Q >= 128)
    {
        return conv2d_gen_11x7_8x16_getScratchSize(P,Q);
    }
    else
    {
        return 80 * sizeof(int16_t) + conv2d_11x7_16x16_getScratchSize(P, Q);
    }
}
...
void conv2d_11x7_8x16(void * pScr, int16_t * z, const int8_t * x, const int16_t * y, int rsh, int P, int Q)
{
    ...
    AE_L8X4F_IP(t0, x, 4 * sizeof(int8_t));
    ...
    conv2d_11x7_16x16((int16_t*)pScr + 80, z, (const int16_t*)pScr, y, rsh, P, Q);
}
