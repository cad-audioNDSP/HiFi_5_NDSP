...
/*
    ...
    Blockwise Adaptive LMS Algorithm for Real Data, 32x32-bit with 72-bit accumulator
...
-------------------------------------------------------------------------*/
void fir_blms32x32ep( int32_t * restrict e,
                    int32_t * restrict h,
              ...
              int N, int M )
{
    ...
    ae_int32x2 h0,h1,e0,e1;
    ae_int32x2 x0,x1,x5,x6,x7;
    ae_int64 q0,q1,q2,q3;
    ae_ep  eq0,eq1,eq2,eq3;
    int m, n;
    ...
    for ( n=0; n<N; n+=4 )
    {
        ...
        AE_L32X2_RIC(h0,H);
        AE_L32X2_RIC(h1,H);
        x5=AE_SEL32_LH(x0,x1);
        AE_MULZAAD32EP_HH_LL(eq0,q0,h0,x0);
        AE_L32X2_IP(x0,X,8);
        x6=AE_SEL32_LH(x1,x0);
        AE_MULAAD32EP_HH_LL (eq0,q0,h1,x1);
        AE_MULZAAD32EP_HH_LL(eq1,q1,h0,x5);
        AE_MULAAD32EP_HH_LL (eq1,q1,h1,x6);
        AE_MULZAAD32EP_HH_LL(eq2,q2,h0,x1);
        AE_MULAAD32EP_HH_LL (eq2,q2,h1,x0);
        AE_MULZAAD32EP_HH_LL(eq3,q3,h0,x6);
        AE_L32X2_IP(x1,X,8);
        x7=AE_SEL32_LH(x0,x1);
        AE_MULAAD32EP_HH_LL (eq3,q3,h1,x7);
        ...
        for ( m=0; m<(M>>2)-1; m++ )
        {
            ...
            AE_L32X2_RIC(h1,H);
            x5=AE_SEL32_LH(x0,x1);
            AE_MULAAD32EP_HH_LL(eq0,q0,h0,x0);
            AE_L32X2_IP(x0,X,8);
            x6=AE_SEL32_LH(x1,x0);
            AE_MULAAD32EP_HH_LL(eq0,q0,h1,x1);
            AE_MULAAD32EP_HH_LL(eq1,q1,h0,x5);
            AE_MULAAD32EP_HH_LL(eq1,q1,h1,x6);
            AE_MULAAD32EP_HH_LL(eq2,q2,h0,x1);
            AE_MULAAD32EP_HH_LL(eq2,q2,h1,x0);
            AE_MULAAD32EP_HH_LL(eq3,q3,h0,x6);
            AE_L32X2_IP(x1,X,8);
            x7=AE_SEL32_LH(x0,x1);
            AE_MULAAD32EP_HH_LL(eq3,q3,h1,x7);
        }
        q0=AE_SRAI72(eq0,q0,15);
        q1=AE_SRAI72(eq1,q1,15);
        q2=AE_SRAI72(eq2,q2,15);
        q3=AE_SRAI72(eq3,q3,15);
    ...
    for ( m=0; m<M; m+=4 )
    {
        ...
        q0=AE_MULF32S_HH(x0,v_fract);
        q1=AE_MULF32S_LL(x0,v_fract);
        q2=AE_MULF32S_HH(x1,v_fract);
        q3=AE_MULF32S_LL(x1,v_fract);
