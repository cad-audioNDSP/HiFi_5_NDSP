...
/*
  NatureDSP Signal Processing Library. FIR part
    Real polyphase analysis filter, floating point
...
DISCARD_FUN(size_t           , rppafirf_alloc  , (int M, int N));
DISCARD_FUN(rppafirf_handle_t, rppafirf_init   , (void * objmem, int M, int N, const float32_t* h));
DISCARD_FUN(void             , rppafirf_process, (rppafirf_handle_t handle, float32_t * y, const float32_t * x));
#else
/* Polyphase filter instance structure */
typedef struct tag_rppafirf_t
{
...
size_t rppafirf_alloc(int M, int N)
{
    ...
    return ALIGN_PAD + 2*M*N*sz_f32 + sizeof(rppafirf_t) + ADD_PAD_F32*sizeof(float32_t);
} /* rppafirf_alloc() */
...
rppafirf_handle_t rppafirf_init(void * objmem, int M, int N, const float32_t* h)
{
    ...
    ppfir = (rppafirf_t*)p; p = ppfir + 1;
#ifdef _DEBUG
    NASSERT((int8_t*)p - (int8_t*)objmem <= (int)rppafirf_alloc(M, N));
#endif
...
{
    rppafirf_t * ppfir = (rppafirf_t*)handle;
