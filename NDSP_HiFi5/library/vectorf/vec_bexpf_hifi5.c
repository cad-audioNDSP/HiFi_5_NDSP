...
#else
{
    ...
    if (N<=7)
    {
        maxval=XT_CONST_S(0);
        __Pragma("no_unroll")
        for (n=0; n<N; n++) maxval=XT_MAX_S(XT_ABS_S(x[n]),maxval);
        max0=maxval;
    }
    else
    {
        ...
        for (n=0; n<(N>>3); n++) 
        {
            ...
            ABS_SX2X2(x0,x1,x0,x1);
            ABS_SX2X2(x2,x3,x2,x3);
            max0=MAX_SX2(MAX_SX2(x1,x0),max0);
            max1=MAX_SX2(MAX_SX2(x3,x2),max1);
        }
        max0=MAX_SX2(max0,max1);
    }
    f=MAXNUM_SX2(max0,AE_SEL32_LH_SX2(max0,max0));
    b0 = XT_UN_S(f, f);
    b1 = XT_OEQ_S(f, 0.f);
    zero = xtbool2_extract_0(b0);
    bnan = xtbool2_extract_0(b1); 
    ...
    u0 = ((uint32_t)u0) >> 23;
    /* multiply denorm numbers by 2^24 */
    XT_MOVT_S(f,XT_MUL_S(f, 16777216.f),xtbool2_extract_0(AE_EQ32(u0,0)));
