...
*/
void fft16_32x16(int32_t *y, int32_t *x, const int16_t *tw)
{
    //const int N = 16;
    ...
    AE_L16X4X2_IP(tw1213, tw2122, ptwd, 2*sizeof(ae_int16x4));
    AE_L16X4X2_IP(tw2331, tw3233, ptwd, 2*sizeof(ae_int16x4));
    AE_MULFPC32X16X2RAS(x1, x5, x1, x5, tw0102);
    AE_MULFPC32X16X2RAS(x2, x6, x2, x6, tw0311);
    AE_MULFPC32X16X2RAS(x3, x7, x3, x7, tw1213);
    AE_MULFPC32X16X2RAS(x9, xD, x9, xD, tw2122);
    AE_MULFPC32X16X2RAS(xA, xE, xA, xE, tw2331);
    AE_MULFPC32X16X2RAS(xB, xF, xB, xF, tw3233);
    ...
    WUR_AE_SAR(1);
    DFT4X1RNG2(y0, y1, y2, y3);
    DFT4X1RNG2(y4, y5, y6, y7);
    DFT4X1RNG2(y8, y9, yA, yB);
    DFT4X1RNG2(yC, yD, yE, yF);
...
/*
   ...
   NOTE: y is input and output, x - temporary
*/
void fft32_32x16(int32_t *y, int32_t *x, const int16_t *tw)
{
    #define N 32
    int k;
     ...
     */
    {
        ...
        py0 = (ae_int32x4 *)y;
        py1 = py0 + 2*stride;
        ptwd = (const ae_int16x8 *)__fft8_tw1;
        AE_L16X4X2_I(tw1, tw2, ptwd, 0);
        AE_MOVSARA7X2(2, 1);
        ...
        AE_MULFPC32X16X2RAS(x3, xB, x3, xB, tw1);
        AE_MULFPC32X16X2RAS(x5, x7, x5, x7, tw2);
        AE_MULFPC32X16X2RAS(xD, xF, xD, xF, tw2);

        AE_ADDANDSUBRNG32_L(s0, d0, x0, x1);
        AE_ADDANDSUBRNG32_L(s1, d1, x2, x3);
        AE_ADDANDSUBRNG32_L(s2, d2, x4, x5);
        AE_ADDANDSUBRNG32_L(s3, d3, x6, x7);
        AE_ADDANDSUBRNG32_L(s4, d4, x8, x9);
        AE_ADDANDSUBRNG32_L(s5, d5, xA, xB);
