...
*/
void fft16_32x32(int32_t *y, int32_t *x, const int32_t *ptwd);/* N=16 */
void fft32_32x32(int32_t *y, int32_t *x, const int32_t *ptwd);/* N=32 */
...
-------------------------------------------------------------------------*/
int dct_32x32( int32_t * restrict y, int32_t * restrict x, dct_handle_t h, int scalingOpt )
{
  //
  ...
  //
    ...
    const ae_int32x4 * restrict p_cos;
    const ae_int32x4 * restrict p_w;
    ae_valignx2 al_y0;
    ae_valign al_z0, al_z2;
    ae_int32x2  vC0, vC2, vW0, vW1, vW2, vW3;
    ...
    {
    case 32:
        fft16_32x32(x, y, (const int32_t *)descr->fft_twd);
        break;
    case 64:
        fft32_32x32(x, y, (const int32_t *)descr->fft_twd);
        break;
    default:
        NASSERT(0);
    }
    ...
    p_cos = (const ae_int32x4 *)(descr->rfft_split_twd);
    p_w   = (const ae_int32x4 *)(descr->dct_twd);
    p_z0  = (      ae_int32x2 *)(y);
    ...
    WUR_AE_SAR(1);
    /*** process 1st sample ***/
    AE_L32X2_IP(vA0, castxcc(ae_int32x2,p_y0), sizeof(ae_int32x2));
    vW0 = AE_L32X2_X((ae_int32x2 *)p_w, N*sizeof(int32_t)-sizeof(ae_int32x2));
    vA1 = AE_SEL32_LH(vA0, vA0);
    vB0 = AE_ADDSUB32S(vA1, vA0);
    vB0 = AE_MULFP32X2RAS(vB0, vW0);

    AE_S32_L_IP(vB0, castxcc(ae_int32,p_z2), sizeof(ae_int32));
    AE_S32_H_IP(vB0, castxcc(ae_int32,p_z0), sizeof(ae_int32));

    al_y0 = AE_LA128_PP(p_y0);
    ...
    for (i = 1; i < (N>>3); i++)
    {
      /*
        ...
        T1=T1+Y0;
      */
      AE_LA32X2X2_IP(vA0, vA2, al_y0, p_y0);
      ...
      AE_L32X2X2_IP(vC0, vC2, p_cos, 2*sizeof(ae_int32x2));
      vB0 = AE_MULFC32RAS(vB0, vC0);
      vB2 = AE_MULFC32RAS(vB2, vC2);
      ...
      */
      AE_L32X2X2_IP(vW0, vW1, p_w, 2*sizeof(ae_int32x2));
      AE_L32X2X2_IP(vW2, vW3, p_w, 2*sizeof(ae_int32x2));
      vB0 = AE_MULFC32RAS(vA0, vW0);
      vB1 = AE_MULFC32RAS(vA1, vW1);
      vB2 = AE_MULFC32RAS(vA2, vW2);
      vB3 = AE_MULFC32RAS(vA3, vW3);
    ...
    vB0 = AE_SUBADD32S(vA0, vA1);
    vB1 = AE_ADDSUB32S(vA0, vA1);
    AE_L32X2_IP(vC0, castxcc(ae_int32x2,p_cos), sizeof(ae_int32x2));
    vB0 = AE_MULFC32RAS(vB0, vC0);
    ...
    */
    AE_L32X2_IP(vW0, castxcc(ae_int32x2,p_w), sizeof(ae_int32x2));
    vB3 = AE_MULFC32RAS(vA3, vW0);
