...
int stereo_ifft_cplx16x16_ie(complex_fract16* y, complex_fract16* x, const complex_fract16* twd, int twdstep, int N, int scalingOpt)
{
    ...
    */
    {
        ...
        for (i = 0; i < N4; i++)
        {
            ...
            /* load input data */
            AE_L32X2_XP(_x0, castxcc(ae_int32x2, px), N4 * 2 * sizeof(complex_fract16));
            AE_L32X2_XP(_x1, castxcc(ae_int32x2, px), N4 * 2 * sizeof(complex_fract16));
            AE_L32X2_XP(_x2, castxcc(ae_int32x2, px), N4 * 2 * sizeof(complex_fract16));
            AE_L32X2_XP(_x3, castxcc(ae_int32x2, px), sizeof(ae_int16x4)-3 * N4 * 2 * sizeof(complex_fract16));

            x0 = AE_MOVINT16X4_FROMF32X2(_x0);
            x1 = AE_MOVINT16X4_FROMF32X2(_x1);
            x2 = AE_MOVINT16X4_FROMF32X2(_x2);
            x3 = AE_MOVINT16X4_FROMF32X2(_x3);
    ...
    */
    ...
    {
        stridey *= 2;
        shift += ifft_16x16_stage_last_scl2_DFT8(NULL, (const int16_t*)x, (int16_t*)y, N, &stridey, 0, &bexp);
    }
    else/* radix-4 */
    {
        int stage_shift;
        ...
        for (i = 0; i < N4; i++)
        {
            ...
            /* store data */
            AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(x0), castxcc(ae_int32x2, py), N4 * 2 * sizeof(complex_fract16));
            AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(x1), castxcc(ae_int32x2, py), N4 * 2 * sizeof(complex_fract16));
            AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(x2), castxcc(ae_int32x2, py), N4 * 2 * sizeof(complex_fract16));
            AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(x3), castxcc(ae_int32x2, py), sizeof(ae_int16x4)-3 * N4 * 2 * sizeof(complex_fract16));
        }
        shift += stage_shift;
    }
    return shift;
} /* stereo_ifft_cplx16x16_ie() */
