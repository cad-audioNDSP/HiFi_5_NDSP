...
int fft_16x16_stage_last_scl2_DFT8(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
    ...
    for (j = 0; j < (numIter>>1); j++)
    {
        ...
        AE_S16X4X2_X (x1[0],x1[1], py0, stride*2*sizeof(int16_t));
        AE_S16X4X2_IP(x0[0],x0[1], py0, sizeof(ae_int16x8));
        AE_S16X4X2_X (x3[0],x3[1], py1, stride*2*sizeof(int16_t));
        AE_S16X4X2_IP(x2[0],x2[1], py1, sizeof(ae_int16x8));
        AE_S16X4X2_X (x5[0],x5[1], py2, stride*2*sizeof(int16_t));
        AE_S16X4X2_IP(x4[0],x4[1], py2, sizeof(ae_int16x8));
        AE_S16X4X2_X (x7[0],x7[1], py3, stride*2*sizeof(int16_t));
        AE_S16X4X2_IP(x6[0],x6[1], py3, sizeof(ae_int16x8));
    }
