...
int ifft_real16x16( int16_t* y, int16_t* x, fft_handle_t h, int scalingOpt )
{
  ...
  else
  {
    ...
    int bexp;
    {
      int i;
      ae_int16x8 * px;
      ae_int16x4 acc, tmp, tmp1, tmp2, tmp3;
      px = (ae_int16x8*)x;
      acc = AE_ZERO16();
      ...
      for (i = 0; i < (N >> 4); i++)   
      {
        AE_L16X4X2_IP(tmp, tmp1, px, sizeof(ae_int16x8));
        AE_L16X4X2_IP(tmp2, tmp3, px, sizeof(ae_int16x8));
        tmp = AE_ABS16S(tmp);
        tmp1 = AE_ABS16S(tmp1); 
        tmp2 = AE_ABS16S(tmp2);
        tmp3 = AE_ABS16S(tmp3);
        tmp1 = AE_OR16(tmp, tmp1);
        tmp2 = AE_OR16(tmp2, tmp3);
        tmp = AE_OR16(tmp1, tmp2);
        acc = AE_OR16(acc, tmp);
      }
#if 0
      if (N&8)  
      {
          AE_L16X4X2_IP(tmp, tmp1, castxcc(ae_int16x8, px), 2*sizeof(*px));
          tmp = AE_ABS16S(tmp); 
          acc = AE_OR16(acc, tmp);
          tmp1 = AE_ABS16S(tmp1); 
          acc = AE_OR16(acc, tmp1);
      }
      if (N&4) 
      {
