    ...
    if (stride / _v > (_v >> 2))
    {
        ...
        for (j = 0; j < (_v >> 2); j++)
        {
            ...
            for (i = 0; i < (stride / _v); i++)
            {
                ...
                AE_S16X4X2RNG_XP(x00, x01, castxcc(ae_int16x8, py0), _v * 2 * sizeof(int16_t));
                AE_S16X4X2RNG_XP(x10, x11, castxcc(ae_int16x8, py0), _v * 2 * sizeof(int16_t));
                AE_S16X4X2RNG_XP(x20, x21, castxcc(ae_int16x8, py0), _v * 2 * sizeof(int16_t));
                AE_S16X4X2RNG_XP(x30, x31, castxcc(ae_int16x8, py0), _v * 2 * sizeof(int16_t));
        ...
        for (i = 0; i < (stride / _v); i++)
        {
            ...
            for (j = 0; j < (_v >> 2); j++) 
            {
                ...
                */
                ...
                AE_S16X4X2RNG_XP(x10, x11, castxcc(ae_int16x8, py0), _v * 2 * sizeof(int16_t));
                AE_S16X4X2RNG_XP(x20, x21, castxcc(ae_int16x8, py0), _v * 2 * sizeof(int16_t));
                AE_S16X4X2RNG_XP(x30, x31, castxcc(ae_int16x8, py0), (4 - 3 * _v) * 2 * sizeof(int16_t));
            } /* for (i = 0; i < ((stride/_v)>>1); i++) */
        }   /* for (j = 0; j < (_v>>1); j++) */
    }   /* if (_v > stride ).. else.. */
