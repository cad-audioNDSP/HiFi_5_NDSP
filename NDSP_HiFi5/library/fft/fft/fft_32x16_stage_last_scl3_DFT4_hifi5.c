...
int fft_32x16_stage_last_scl3_DFT4(const int16_t *tw, const int32_t *x, int32_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  for (j = 0; j < (stride>>1); j++)
  {
    ...
    AE_S32X2X2_IP(x0, x4, castxcc(ae_int32x4, py0), 2 * sizeof(ae_int32x2));
    AE_S32X2X2_IP(x1, x5, castxcc(ae_int32x4, py1), 2 * sizeof(ae_int32x2));
    AE_S32X2X2_IP(x2, x6, castxcc(ae_int32x4, py2), 2 * sizeof(ae_int32x2));
    AE_S32X2X2_IP(x3, x7, castxcc(ae_int32x4, py3), 2 * sizeof(ae_int32x2));
  }
