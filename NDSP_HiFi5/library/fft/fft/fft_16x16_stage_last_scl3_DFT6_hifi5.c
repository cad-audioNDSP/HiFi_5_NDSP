...
/*
*  Last stage of FFT 16x16, radix-6, static scaling
*/
int fft_16x16_stage_last_scl3_DFT6(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
    ...
    { /* 23 cycles per pipeline stage in steady state with unroll=1 */
        ...
        AE_S16X4X2_XP(x00, x01, castxcc(ae_int16x8, py0), stride * sizeof(complex_fract16));
        AE_S16X4X2_XP(x10, x11, castxcc(ae_int16x8, py0), stride * sizeof(complex_fract16));
        AE_S16X4X2_XP(x20, x21, castxcc(ae_int16x8, py0), stride * sizeof(complex_fract16));
        AE_S16X4X2_XP(x30, x31, castxcc(ae_int16x8, py0), stride * sizeof(complex_fract16));
        AE_S16X4X2_XP(x40, x41, castxcc(ae_int16x8, py0), stride * sizeof(complex_fract16));
        AE_S16X4X2_XP(x50, x51, castxcc(ae_int16x8, py0), (4 - 5 * stride) * sizeof(complex_fract16));
    }
    *v *= 6;
    return shift;
} /* fft_16x16_stage_last_scl3_DFT6() */
