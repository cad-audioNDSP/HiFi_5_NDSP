...
void mfcc32x32_remdc( fract32 * y, const fract32 * x, int N )
{
    ...
    ae_int32x2 f0, f1, f2, f3;
    ae_int32x2 s, t, mean;
    ae_int64 acc0, acc1, g;
    ae_ep ep;
    int16_t r_exp;
    ...
    for ( n=0; n<(N>>3); n++ ) {
        AE_L32X2X2_I (f2, f3, pX, 1*sizeof(ae_int32x4));
        AE_L32X2X2_IP(f0, f1, pX, 2*sizeof(ae_int32x4));
        AE_MULAAAAQ32X16(acc0, f0, f1, 1);
        AE_MULAAAAQ32X16(acc1, f2, f3, 1);
    } /* n */
    /* 1 cycles per pipeline stage in steady state with unroll=1 */
    for ( n=0; n<((N>>1)&3); n++ ) {
        AE_L32X2_IP(f0, castxcc(ae_int32x2, pX), sizeof(ae_int32x2));
        AE_MULAAD32X16_H0_L1(acc0, f0, 1);
    } /* n */
    acc0 = AE_ADD64(acc0, acc1);
    /* r <- 1/N; Q(62-r_exp) */
    vec_recip32x32(&r_fract, &r_exp, &_N, 1);
    /* g <- acc*1/N; Q(31+x-r_exp) <- Qx*Q(62-r_exp) - 31 */
    s = AE_MOVINT32X2_FROMINT64(acc0); t = AE_MOVINT32X2_FROMINT32(AE_MOVDA32(r_fract));
    ...
    /* 1 cycles per pipeline stage in steady state with unroll=1 */
    for ( n=0; n<(N>>2); n++ ) {
        AE_L32X2X2_IP(f0, f1, pX, sizeof(ae_int32x4));
        f0 = AE_SUB32S(f0, mean); f1 = AE_SUB32S(f1, mean); 
        AE_S32X2X2_IP(f0, f1, pY, sizeof(ae_int32x4));
    } /* n */
    if (N&2) {
        f0 = AE_L32X2_I((ae_int32x2*)pX, 0);
        f0 = AE_SUB32S(f0, mean);
        AE_S32X2_I(f0, (ae_int32x2*)pY, 0);
    } /* (N&2) */
#else
    int64_t acc, f;
