...
PROFILE_EXTERN(logmelf_vec_complex2mag);
...
void mfccf_process( mfccf_handle_t handle, void * restrict pScr, float32_t * restrict cepstra, const float32_t * restrict speech )
{
    ...
    if (winLen!=hopLen) MFCC_COPY_32b(pf, pf+hopLen, winLen-hopLen);
    MFCC_COPY_32b(pf+winLen-hopLen, speech, hopLen);
    ...
    if (mfcc->stftWeights) {
        mfccf_vecmpy(f, mfcc->stftWeights, pf, winLen); pf = f;
    }
    ...
    if (f!=pf) MFCC_COPY_32b(f, pf, winLen);
    ...
    if (params->lifter>0) {
        ...
        MFCC_COPY_32b(cepstra, features, params->cepstraNum);
    }
    ...
    PROFILE_REPORT(logmelf_vec_complex2mag);
