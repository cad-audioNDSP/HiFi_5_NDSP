...
void mfcc32x32_process( mfcc32x32_handle_t handle, void * restrict pScr, fract32 * restrict cepstra, const fract32 * restrict speech )
{
    ...
    if (mfcc->stftWeights) {
        mfcc32x32_vecmpy(f, mfcc->stftWeights, f, winLen);
    }
    /* Pad the weighted signal with zeros to match the FFT size. */
    MFCC_ZERO_32b(f+winLen, params->fftSize-winLen);
...
{
    const ae_int32x4 * restrict _src = (ae_int32x4*)src;
    ae_int32x4 * restrict _dst = (ae_int32x4*)dst;
    ae_valignx2 src_u, dst_u;
    ae_int32x2 f0, f1;
    ae_int32x2 acc;
    int n, headLen;
    NASSERT_ALIGN(dst, 8);
    ...
    if (len>=4) {
        src_u = AE_LA128_PP(_src); dst_u = AE_ZALIGN128();
        AE_LA32X2X2_IP(f0, f1, src_u, _src);
        AE_SA32X2X2_IP(f0, f1, dst_u, _dst);
        AE_SA128POS_FP(dst_u, _dst);
        acc = AE_MAXABS32S(f0, f1);
        ...
        src_u = AE_LA128_PP(_src);
        /* 2 cycles per pipeline stage in steady state with unroll=2 */
        for ( n=0; n<(len>>2); n++ ) {
            AE_LA32X2X2_IP(f0, f1, src_u, _src);
            AE_SA32X2X2_IP(f0, f1, dst_u, _dst);
            acc = AE_MAX32(acc, AE_MAXABS32S(f0, f1));
        } /* n */
        AE_SA128POS_FP(dst_u, _dst);
    } else { /* (len<4) */
        f0 = AE_L32X2_I((ae_int32x2*)_src, 0);
        AE_S32X2_I(f0, (ae_int32x2*)_dst, 0);
        acc = AE_ABS32S(f0);
    } /* (len<4) */
    acc = AE_MAX32(acc, AE_SEL32_LH(acc, acc));
    return AE_MOVAD16_0(AE_NSA32X4(acc, acc));
} /* copy_bexp_32b() */
