...
void fileProc_stnd_rfft( tTestEngContext * context )
{
  ...
  *vecGetElem_fl32( &context->dataSet.Z, 0 ) = -HUGE_VALF;
  ...
  {
    printf( "fileProc_stnd_rfft(): failed to allocate in/out/ref vectors, N=%d\n", N );
  }
  /* Open input data file. */
  else if (fIn==NULL )
  {
    printf( "fileProc_stnd_rfft(): failed to open %s for reading\n", context_fft->fInName );
  }
  ...
  if ( res )
  {
    ...
    while ((n = fread(pin, inVec.szElem, lenFread, fIn)) > 0)
    {
      ...
      {
        printf( "fileProc_stnd_rfft(): failed to allocate xVec/yVec, "
      ...
      if ( !( ++context_fft->frameCnt % XY_VEC_REALLOC_PERIOD ) && !isAligned )
      {
        ...
        {
          printf( "fileProc_stnd_rfft(): vecsFree() failed for xVec/yVec, "
                  "frameCnt=%d, N=%d\n", context_fft->frameCnt, N );
          break;
        }
      ...
      {
        printf( "fileProc_stnd_rfft(): failed to read reference data, "
                "frameCnt=%d, N=%d\n", context_fft->frameCnt, N );
        res = 0; break;
      }
