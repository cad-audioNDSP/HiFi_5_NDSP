...
static const tTestDef testDefTblAccuracy[] =
{
  ...
  {
    FUNC_LIST(
    (tTestEngTarget)&scl_tanh_fp16,
    (tTestEngTarget)&scl_sigmoid_fp16),
    TEST_DESC(FMT_REAL | FMT_FLOAT16, TE_DIM_NUM_1, TE_ALIGN_NO, &te_loadFxn_vXvZacc, &te_math_processFxn_s_vXvZ_acc) },
  {
...
static const tTestDef testDefTblExc[] =
{
  /*  Stage 3  */
  {
    FUNC_LIST(
    (tTestEngTarget)&scl_tanh_fp16),
    TEST_DESC(FMT_REAL | FMT_FLOAT16, TE_DIM_NUM_1, TE_ALIGN_NO, &te_loadFxn_vXvZ, &te_math_processFxn_s_vXvZ_exc) },
	{
		FUNC_LIST(NULL), TEST_DESC(0, 0, 0, NULL, NULL) } /* End of table */
};
#undef TEST_DESC
...
#define DO_TEST_ACC(fxn, seqFile, extraFlags)                                                                   \
  if ( res || !breakOnError ) res &= ( 0 != testExec( testDefTblAccuracy,sizeof(testDefTblAccuracy)/sizeof(testDefTblAccuracy[0]), \
                                                      (tTestEngTarget)(fxn), "math3/" seqFile,                   \
...
#define DO_TEST_EXC(fxn, seqFile, extraFlags)                                                                   \
  ...
  if (optAccuracy)
  {
    ...
    * Stage 3
    */
    DO_TEST_ACC( &scl_tanh_fp16     , "accuracy_scl_tanh_fp16.seq"          , 0 );
  }
  ...
  if (optException)
  {
    ...
    * Stage 3
    */
    DO_TEST_EXC( &scl_tanh_fp16     , "exceptions_scl_tanh_fp16.seq"        , 0);
  }
  return (res);
}
