...
#if 0 //HiFi3/3z API
  {
    FUNC_LIST( (tTestEngTarget)&vec_poly4_24x24,(tTestEngTarget)&vec_poly8_24x24 ),
    TEST_DESC( FMT_REAL|FMT_FRACT32, TE_DIM_NUM_2, TE_ALIGN_NO, &te_loadFxn_poly, &te_processFxn_poly ) },
#endif
  {
...
int func_pfit1(int isFull, int isVerbose, int breakOnError)
{
  ...
  #define DO_TEST(fxn, seqFile)                                                                  \
    if ( res || !breakOnError ) res &= ( 0 != te_Exec( testDefTbl,                               \
                                                       ...
                                                       MAX_FUNC_NUM,                             \
                                                       (tTestEngTarget)(fxn), "fit1/" seqFile,    \
