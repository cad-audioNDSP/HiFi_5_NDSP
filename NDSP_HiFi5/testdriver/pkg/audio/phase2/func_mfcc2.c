...
static const tTestEngDesc htkdeltaf_desc     = HTKDELTA_TEST_DESC(FMT_REAL|FMT_FLOAT32, TE_ALIGN_YES);

#define DO_TEST( api, desc, seqFile )                                                                   \
    if ( res || !breakOnError ) res &= ( 0 != TestEngRun((tTestEngTarget)&api, &desc, "mfcc2/" seqFile,  \
...
int func_mfcc2(int isFull, int isVerbose, int breakOnError)
{
    ...
    DO_TEST(htkdeltaf  , htkdeltaf_desc, "htkdeltaf.seq");
    return (res);
}
