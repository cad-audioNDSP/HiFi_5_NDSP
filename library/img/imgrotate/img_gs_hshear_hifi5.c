...
void img_gs_hshear(void* pScr, void* inoutImg, imgsize_t * restrict inoutSz,int16_t beta, int fillColor)
{
  ...
  for (i=0; i<h; i++)
  {
    ...
    dst0 = (      ae_int16x8 *)((uintptr_t)pS);
    src0 = (const ae_int16x8 *)((uintptr_t)src + i*stride * sizeof(int16_t));
    src1 = (const ae_int16x8 *)((uintptr_t)src + (i*stride + 1) * sizeof(int16_t));
    aS = AE_LA128_PP(src1);
    for (j = 0; j<((w - 1)>>3); j++) //6/3
    {
      AE_L16X4X2_IP(x0, x1, src0, 2 * sizeof(ae_int16x4));
      AE_LA16X4X2_IP(x2, x3, aS, src1);

      AE_MULF2P32X16X4RAS(y0, y1, a0, a0, x0);
      AE_MULAF2P32X16X4RAS(y0, y1, b0, b0, x2);
      AE_MULF2P32X16X4RAS(y2, y3, a0, a0, x1);
      AE_MULAF2P32X16X4RAS(y2, y3, b0, b0, x3);

      x0 = AE_ROUND16X4F32SASYM(y0, y1);
      x1 = AE_ROUND16X4F32SASYM(y2, y3);
      AE_S16X4X2_IP(x0, x1, dst0, 2 * sizeof(ae_int16x4));
    }
    ...
    AE_S16_0_IP(x0, castxcc(ae_int16, dst0), sizeof(int16_t));
    src0 = (const ae_int16x8 *)((uintptr_t)pS);
    dst0 = (      ae_int16x8 *)((uintptr_t)src + (i * stride + k) * sizeof(int16_t) );
    aD = AE_ZALIGN128();
    for (j = 0; j<(w>>3); j++)
    {
      AE_L16X4X2_IP(x0, x1, src0, 2 * sizeof(ae_int16x4));
      AE_SA16X4X2_IP(x0, x1, aD, dst0);
    }
    ...
    x0 = fillColor;
    dst0 = (      ae_int16x8 *)((uintptr_t)src + (i * stride) * sizeof(int16_t) );
    for (j = 0; j<(k>>3); j++)
    {
    ...
    dst0 = (ae_int16x8 *)((uintptr_t)src + ((w + k) + i * stride) * sizeof(int16_t));
    aD = AE_ZALIGN128();
    for (j = 0; j<((W - (w + k)) >> 3); j++)
    {
      AE_SA16X4X2_IP(x0, x0, aD, dst0);
    }
    for (j = 0; j<((W - (w + k)) & 7); j++)
    {
      AE_S16_0_IP(x0, castxcc(ae_int16,dst0), sizeof(int16_t));
